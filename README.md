...
Each of the following compiles and runs end-to-end. See [`tests/e2e/cases/`](tests/e2e/cases/) for the full 652-case suite and [`docs/done.md`](docs/done.md) for the complete capability inventory.
...
| `runtime/tsc_runtime.{c,h}` | Full runtime (~10,150 LOC): strings, arrays, maps/sets/weak collections, regex, BigInt, exceptions, JSON, console, process, fs + fs.promises, path, Math, os, Date, Number, Buffer, URL, EventEmitter, Event/EventTarget, Error hierarchy, Promise (immediate), generators (eager), dns, net, child_process |
| `stdlib/lib.core.d.ts` | Type shim (~1,330 LOC; replaces `@types/node`) |
| `examples/` | 7 runnable demo programs |
| `tests/e2e/cases/` | 652 test dirs — `in.ts` + optional support files + `expected.stdout` or `expected.exitcode` |
