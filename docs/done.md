...
- Generated-C DCE uses exact static one-argument `Array.from([...])` Set constructor source lengths for string, numeric, and boolean Sets, enabling unused in-range `.with(...)` calls after duplicate elimination to be pruned. Test: `generated_c_dce_const`
