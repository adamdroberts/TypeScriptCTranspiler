...
- Standard class decorators on named classes are evaluated during module initialization. Direct decorator functions, decorator-factory closure results, and dynamic callable/proxy-backed decorators receive a dynamic context object with `kind: "class"`, `name`, `metadata`, and `addInitializer`; multiple decorator expressions on the same class evaluate top-down and invoke bottom-up. Class decorators can return AOT constructor replacement functions used by direct `new Class(...)` calls, including erased generic class constructors, stacked class decorator replacements compose in bottom-up invocation order while forwarding constructor arguments, and the original class value passed to decorators is constructible through `Reflect.construct(value, args)`. Broader class replacement edge cases remain deferred. Tests: `class_decorator_basic`, `class_decorator_factory`, `class_decorator_replacement`, `class_decorator_replacement_order`, `generic_class_decorator_replacement`, `decorator_add_initializer_basic`, `decorator_proxy_basic`, `decorator_replacement_order`, `decorator_factory_order`
