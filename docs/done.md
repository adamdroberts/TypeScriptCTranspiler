...
- Generated C omits provably unreferenced non-exported top-level function declarations, lifted top-level arrow/function-expression constants, conservative no-side-effect top-level class declarations, and side-effect-free top-level `const` literals before handing code to gcc. Tests: `generated_c_dce`, `generated_c_dce_lifted_arrow`, `generated_c_dce_class`, `generated_c_dce_const`
