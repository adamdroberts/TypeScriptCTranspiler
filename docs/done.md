...
- Generated-C DCE uses exact static numeric `Array.from(new Set(...))` length proofs with SameValueZero duplicate handling, including `NaN` and `Infinity`, enabling unused in-range `.with(...)` calls on multi-entry numeric sets to be pruned. Test: `generated_c_dce_const`
