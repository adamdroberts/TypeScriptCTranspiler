...
Verify all at once: `TSC2C_NO_GC=1 bun tests/e2e/run.ts` -> 672 passed.
...
- AOT closure diagnostics reject `eval`, `Function` / `new Function`, non-finite `require(variable)` without a static proof or `--dynamic-require-manifest`, unallow-listed literal native addon specifiers ending in `.node`, and unallow-listed literal package imports/requires whose installed package root contains `build/Release/*.node`, including transitive imports from emitted package sources under `node_modules`. Top-level finite const string `require(name)` specifiers, including string concatenation and template expressions whose parts are all statically known, are resolved into the AOT module graph. Side-effect-only `require(name)` statements also accept finite conditional alternatives and include every possible specifier as an eager AOT module edge; value-returning finite-set `require(name)` calls dispatch over the same finite alternatives for AOT-known `module.exports = ...` values and materialize AOT-known `module.exports.name = ...` named exports into dynamic objects. `--dynamic-require-manifest <path>` is an explicit compile-time allow-list for non-finite dynamic require sites; manifest entries use JSON shape `{ "requires": ["./specifier"] }`, enter the AOT module graph, and share the same runtime finite dispatch. `--unsafe-eval` is an explicit compile-time gate for runtime code compilation: it emits embedded Node bridge calls for `eval` and callable `Function` values, copies `runtime/tsc_node_embed.cc`, and requires discoverable `libnode` inputs when linking. `--native-addon-manifest <path>` is a separate compile-time allow-list for known native addons; manifest entries map literal specifiers to concrete `.node` files, lower to `tsc_node_native_addon(...)`, copy `runtime/tsc_node_embed.cc`, and require discoverable `libnode` inputs when linking without enabling unsafe eval. Tests/checks: `runtime_eval`, `runtime_function_call`, `runtime_function_constructor`, `dynamic_require`, `dynamic_require_manifest`, `dynamic_require_unknown`, `native_addon`, `native_addon_manifest_require`, `native_addon_package`, `node_modules_transitive_native_addon`, focused `--unsafe-eval --emit-c-only` bridge checks
