...
- `fs.statSync(path[, { bigint: false, throwIfNoEntry }], ...ignored)` and immediate-settled `fs.promises.stat(path[, { bigint: false, throwIfNoEntry }])` return a small typed `Stats` subset with `dev`, `ino`, `size`, `mode`, `nlink`, `uid`, `gid`, `rdev`, `blksize`, `blocks`, numeric `atimeMs` / `mtimeMs` / `ctimeMs` / `birthtimeMs` timestamps, Date-object `atime` / `mtime` / `ctime` / `birthtime` timestamps, `isFile(...ignored)`, `isDirectory(...ignored)`, `isSymbolicLink(...ignored)`, `isBlockDevice(...ignored)`, `isCharacterDevice(...ignored)`, `isFIFO(...ignored)`, and `isSocket(...ignored)`. Literal `{ throwIfNoEntry: false }` returns / fulfills with `undefined` for missing paths, and sync calls treat explicit `undefined` options as defaults while evaluating ignored extras. Tests: `fs_stat`, `fs_stats_kinds`, `fs_stat_options`, `fs_stat_ignored_arguments`, `fs_stats_times`, `fs_stats_metadata`
- `fs.lstatSync(path[, { bigint: false, throwIfNoEntry }], ...ignored)` and immediate-settled `fs.promises.lstat(path[, { bigint: false, throwIfNoEntry }])` return the same small typed `Stats` subset without following symlinks. Literal `{ throwIfNoEntry: false }` returns / fulfills with `undefined` for missing paths, and sync calls treat explicit `undefined` options as defaults while evaluating ignored extras. Tests: `fs_lstat`, `fs_stats_kinds`, `fs_stat_options`, `fs_stat_ignored_arguments`, `fs_stats_times`, `fs_stats_metadata`
