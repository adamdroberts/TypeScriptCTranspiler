...
- `fs.readdirSync(...)`, `fs.realpathSync(...)`, `fs.readlinkSync(...)`, `fs.mkdtempSync(...)`, and immediate `fs.promises.readdir(...)` / `realpath(...)` / `readlink(...)` / `mkdtemp(...)` accept earlier static `const` aliases for supported string `encoding` option values; realpath/readlink/mkdtemp additionally support literal `"hex"` / `"base64"` string and object encodings for string results. Tests: `fs_readdir_buffer_options`, `fs_link_path_encoding_options`, `fs_mkdtemp_encoding_options`, `fs_path_result_encoded_options`
