...
- Named class getter and setter accessors lower to C calls for instance and static property reads/writes, including string/number literal and const-literal computed accessor names. Plain setter assignments, numeric/string compound setter assignments, and logical setter assignments return the JavaScript assignment value. Tests: `class_accessors_basic`, `class_accessors_compound`, `class_accessors_logical`, `class_computed_accessors`
...
- Standard method decorators on named instance and static methods are evaluated during module initialization. Direct decorator functions and dynamic callable/proxy-backed decorators receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; multiple decorator expressions on the same method evaluate top-down and invoke bottom-up; static method decorators can return replacement functions used by direct `Class.method(...)` calls, and instance method decorators can return replacement functions used by direct `obj.method(...)` calls with the actual instance passed to typed class `this` receivers. Static and instance method replacements can call the original method value through `Reflect.apply(value, thisArg, args)`, and proxy-backed method decorators can return replacement functions through the dynamic decorator apply path. Literal and const-literal computed method names are covered; computed non-literal decorator names remain deferred. Tests: `method_decorator_basic`, `static_method_decorator_replacement`, `static_decorator_originals`, `instance_method_decorator_replacement`, `instance_method_decorator_this_receiver`, `instance_method_decorator_original`, `decorator_computed_members`, `decorator_add_initializer_basic`, `decorator_proxy_basic`, `decorator_proxy_replacement`, `decorator_replacement_order`, `decorator_factory_order`
- Standard field decorators on named instance and static fields are evaluated during module initialization. Direct decorator functions receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; returned initializer functions transform instance and static field initial values, and multiple returned initializers apply top-down after bottom-up decorator invocation. Literal and const-literal computed field names are covered; computed non-literal decorator names remain deferred. Tests: `field_decorator_basic`, `field_decorator_initializer`, `field_decorator_initializer_order`, `decorator_computed_members`, `decorator_proxy_basic`
- Standard getter and setter decorators on named instance and static accessors are evaluated during module initialization. Direct decorator functions receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; static getter decorators can return replacement getter functions used by direct `Class.property` reads, static setter decorators can return replacement setter functions used by direct `Class.property = value` writes, instance getter and setter decorators can return replacement functions used by direct `obj.property` reads/writes with the actual instance passed to typed class `this` receivers. Static and instance getter/setter replacements can call the original accessor value through `Reflect.apply(value, thisArg, args)`. Literal and const-literal computed accessor names are covered; computed non-literal decorator names remain deferred. Tests: `accessor_decorator_basic`, `static_getter_decorator_replacement`, `static_setter_decorator_replacement`, `static_decorator_originals`, `instance_getter_decorator_replacement`, `instance_getter_decorator_this_receiver`, `instance_getter_decorator_original`, `instance_setter_decorator_replacement`, `instance_setter_decorator_this_receiver`, `instance_setter_decorator_original`, `decorator_computed_members`, `decorator_proxy_basic`
...
| `proxy_descriptor_invariants` | Proxy getOwnPropertyDescriptor non-configurable descriptor, accessor identity, and non-extensible target invariant checks |
