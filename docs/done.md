...
- Generated-C DCE uses exact static string, numeric, boolean, and fresh object-key `Array.from(new Map(...))` lengths through literal entry arrays, and string-key lengths through `Object.entries(...)`, `Array.of(...)`, one-argument `Array.from(...)`, const entry arrays, and array-spread constructor sources, enabling unused in-range `.with(...)` calls after duplicate key elimination or fresh object-key uniqueness proofs to be pruned. Test: `generated_c_dce_const`
