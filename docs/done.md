...
- Supported `child_process` option objects treat explicit `undefined` property values as defaults across callback `exec`/`execFile`, sync `execSync`/`execFileSync`, and `spawnSync` subsets. Test: `child_process_option_property_undefined`
...
- Supported `child_process.spawnSync(...)` whole `stdio` tuples and string, numeric, null, and undefined entries can use earlier static `const` aliases. Tests: `child_process_spawn_sync_stdio_pipe`, `child_process_spawn_sync_stdio_ignore`, `child_process_spawn_sync_stdio_inherit`, `child_process_spawn_sync_stdio_fd`, `child_process_spawn_sync_stdio_stdin`, `child_process_spawn_sync_stdio_aliases`
...
| `crypto_random_uuid` | crypto.randomUUID version 4 UUID strings with bounded const-capable options and ignored extra arguments |
...
| `dns_lookup_option_property_undefined` | dns.lookup and dns.promises.lookup treat supported option properties set to undefined as defaults |
