...
- Generated-C DCE prunes unused `Promise.all(...)` / `Promise.allSettled(...)` / `Promise.any(...)` / `Promise.race(...)` over statically empty returned array, earlier-const/copied typed array, and earlier-const/copy-constructed typed Set sources, propagates those empty Set combinator states through `Promise.try(...)` callbacks, and prunes immediate Promise chains over earlier-const/copied typed array or earlier-const/copy-constructed typed Set empty combinator receivers. Test: `generated_c_dce_const`
