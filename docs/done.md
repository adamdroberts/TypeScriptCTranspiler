...
- Generated-C DCE prunes unused `Promise.all(...)` / `Promise.allSettled(...)` / `Promise.any(...)` / `Promise.race(...)` over statically empty returned array, earlier-const/copied typed array, and typed Set sources. Test: `generated_c_dce_const`
