...
- Synchronous `function*` declarations with ordinary `yield expr` statements and bounded `yield*` over arrays, strings, or dynamic iterable values lower to materialized array-backed `Iterator<T>` / `IterableIterator<T>` results consumable by existing `for...of` lowering. The materialized iterator supports `.next(value?)`, `.return(value)`, and `.throw(error)` cursor interactions; optional `.next(value)` inputs are evaluated and ignored in this eager subset. Async generators and lazy suspend/resume state machines remain deferred. Tests: `generator_functions`, `generator_next_argument`
