...
- Standard method decorators on named instance and static methods are evaluated during module initialization. Direct decorator functions receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; method replacement and computed non-literal decorator names remain deferred. Tests: `method_decorator_basic`, `decorator_add_initializer_basic`
