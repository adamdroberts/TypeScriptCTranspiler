...
- ES2024 static `Map.groupBy(items: T[] | Set<T>, keyFn: (value: T, index: number) => K): Map<K, T[]>` groups a typed array or typed Set into a typed `Map<K, T[]>` by the callback's return value. Set sources preserve insertion order for callback index assignment. The callback may be an inline arrow/function expression with an expression body or single-return block body, or a function reference (including generic functions); callbacks that declare `this: any` receive the JavaScript default `undefined` receiver. Tests: `map_group_by`, `group_by_this_param`
...
- ES2024 static `Object.groupBy(items: T[] | Set<T>, keyFn: (item: T, index: number) => string): unknown` groups a typed array or typed Set into a null-prototype dynamic object whose string keys map to dynamic arrays of boxed items. Set sources preserve insertion order for callback index assignment. The callback may be an inline arrow/function expression with an expression body or single-return block body, or a function reference; callbacks that declare `this: any` receive the JavaScript default `undefined` receiver. Elements must be types boxable into `tsc_value_t` (number, boolean, string, array, value). Tests: `object_group_by`, `group_by_this_param`
...
| `map_group_by` | ES2024 Map.groupBy over typed arrays and typed Set sources with arrow/block-body and named callback references |
...
| `object_group_by` | ES2024 Object.groupBy over typed arrays and typed Set sources returning a dynamic null-prototype object with expression/block-body callbacks |
