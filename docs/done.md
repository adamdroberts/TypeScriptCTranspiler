...
- Generated-C DCE also recognizes unused pure string `match(...)` / `search(...)` calls with statically valid string RegExp patterns, plus unused pure fresh-RegExp `test(...)` / `exec(...)` calls with string-coercible inputs and fresh-RegExp `toString()` / `toLocaleString()` / `valueOf()` calls. Test: `generated_c_dce_const`
