...
- Standard getter and setter decorators on named instance and static accessors are evaluated during module initialization. Direct decorator functions receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; static getter decorators can return replacement getter functions used by direct `Class.property` reads, static setter decorators can return replacement setter functions used by direct `Class.property = value` writes, and instance getter/setter decorators can return replacement functions used by direct `obj.property` reads/writes when the replacement does not declare a typed `this` receiver. Instance accessor `this` receiver/original-member bridging and computed non-literal decorator names remain deferred. Tests: `accessor_decorator_basic`, `static_getter_decorator_replacement`, `static_setter_decorator_replacement`, `instance_getter_decorator_replacement`, `instance_setter_decorator_replacement`, `decorator_proxy_basic`
...
| `instance_getter_decorator_replacement` | standard instance getter decorators returning replacement functions without typed this |
