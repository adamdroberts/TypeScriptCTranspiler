...
- ES2024 static `Object.groupBy(items: T[] | Set<T> | string, keyFn: (item: T, index: number) => string): unknown` groups a typed array, typed Set, or string source into a null-prototype dynamic object whose string keys map to dynamic arrays of boxed items. Set sources preserve insertion order and string sources use code-point order for callback index assignment. The callback may be an inline arrow/function expression with an expression body or single-return block body, or a function reference; callbacks that declare `this: any` receive the JavaScript default `undefined` receiver. Elements must be types boxable into `tsc_value_t` (number, boolean, string, array, value). Tests: `object_group_by`, `group_by_this_param`
...
- Generated-C DCE prunes unused `Object.groupBy(...)` / `Map.groupBy(...)` over statically empty returned array, typed Set, and string sources, plus unused `Map.groupBy(...)` over statically empty typed Map sources. Test: `generated_c_dce_const`
