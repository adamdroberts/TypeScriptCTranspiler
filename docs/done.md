...
- `fs.readdirSync(...)`, `fs.realpathSync(...)`, `fs.readlinkSync(...)`, `fs.mkdtempSync(...)`, and immediate `fs.promises.readdir(...)` / `realpath(...)` / `readlink(...)` / `mkdtemp(...)` accept earlier static `const` aliases for supported string `encoding` option values; readdir supports literal `"hex"` / `"base64"` string and object encodings for string filename arrays, and realpath/readlink/mkdtemp support those encodings for string results. Tests: `fs_readdir_buffer_options`, `fs_readdir_encoded_options`, `fs_link_path_encoding_options`, `fs_mkdtemp_encoding_options`, `fs_path_result_encoded_options`
...
- `fs.realpathSync(path[, "utf8" | "hex" | "base64" | "buffer" | { encoding }], ...ignored)` and immediate-settled `fs.promises.realpath(path[, "utf8" | "hex" | "base64" | "buffer" | { encoding }], ...ignored)` resolve paths through the host filesystem for the bounded UTF-8/hex/base64 and explicit Buffer/null object encoding subset, treating explicit `undefined` options as defaults while evaluating ignored extras. Tests: `fs_realpath`, `fs_link_path_encoding_options`, `fs_link_mkdtemp_ignored_arguments`, `fs_path_result_encoded_options`
- `fs.readlinkSync(path[, "utf8" | "hex" | "base64" | "buffer" | { encoding }], ...ignored)` and immediate-settled `fs.promises.readlink(path[, "utf8" | "hex" | "base64" | "buffer" | { encoding }], ...ignored)` read symlink targets through the host filesystem for the bounded UTF-8/hex/base64 and explicit Buffer/null object encoding subset, treating explicit `undefined` options as defaults while evaluating ignored extras. Tests: `fs_readlink`, `fs_link_path_encoding_options`, `fs_link_mkdtemp_ignored_arguments`, `fs_path_result_encoded_options`
...
- `fs.mkdtempSync(prefix[, "utf8" | "hex" | "base64" | "buffer" | { encoding }], ...ignored)` and immediate-settled `fs.promises.mkdtemp(prefix[, "utf8" | "hex" | "base64" | "buffer" | { encoding }], ...ignored)` create temporary directories from a prefix for the bounded UTF-8/hex/base64 and explicit Buffer/null object encoding subset, treating explicit `undefined` options as defaults while evaluating ignored extras. Tests: `fs_mkdtemp`, `fs_mkdtemp_encoding_options`, `fs_link_mkdtemp_ignored_arguments`, `fs_path_result_encoded_options`
...
| `fs_link_path_encoding_options` | fs realpath/readlink UTF-8 and explicit Buffer/null object encoding options for sync and promises |
| `fs_mkdtemp` | fs.mkdtempSync and immediate-settled fs.promises mkdtemp |
| `fs_mkdtemp_encoding_options` | fs mkdtemp UTF-8 and explicit Buffer/null object encoding options for sync and promises |
