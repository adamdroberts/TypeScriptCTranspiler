...
- Standard method decorators on named instance and static methods are evaluated during module initialization. Direct decorator functions and dynamic callable/proxy-backed decorators receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; static method decorators can return replacement functions used by direct `Class.method(...)` calls. Instance method replacement and computed non-literal decorator names remain deferred. Tests: `method_decorator_basic`, `static_method_decorator_replacement`, `decorator_add_initializer_basic`, `decorator_proxy_basic`
