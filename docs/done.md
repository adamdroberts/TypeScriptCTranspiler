...
- Generated-C DCE uses exact static fresh and repeated const object-value `Array.from(new Set(...))` length proofs, enabling unused in-range `.with(...)` calls on multi-entry object Sets to be pruned after object identity proofs. Test: `generated_c_dce_const`
