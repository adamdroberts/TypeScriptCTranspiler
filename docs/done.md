...
- CommonJS package sources treat read-only `module` metadata destructuring such as `const { filename, path, loaded, paths } = module` as static wrapper metadata. Test: `node_modules_commonjs_module_metadata_destructure`
