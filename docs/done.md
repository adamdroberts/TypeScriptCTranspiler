...
- `Object.create(proto)` and `Object.create(proto, descriptors)`, `Object.getPrototypeOf`, `Object.setPrototypeOf`, `Reflect.getPrototypeOf`, and `Reflect.setPrototypeOf` support dynamic object prototype chains; dynamic property reads, `in`, and `Reflect.has` walk prototypes while `Object.keys`, `Reflect.ownKeys`, descriptors, and `Object.hasOwn` remain own-property based. `Object.setPrototypeOf` rejects nullish targets, invalid prototype values before proxy trap dispatch, failed non-extensible updates, and falsy proxy `setPrototypeOf` trap results while preserving primitive target compatibility. Tests: `object_prototypes`, `object_create_descriptors`, `object_set_prototype_of_validation`
