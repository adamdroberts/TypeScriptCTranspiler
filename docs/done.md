...
- Generated-C DCE uses exact static `Object.values(...)` Set source lengths for fresh and repeated const object values, including `Object.fromEntries(...)` and `Object.assign(...)` sources, enabling unused in-range `.with(...)` calls after object identity proofs to be pruned. Test: `generated_c_dce_const`
