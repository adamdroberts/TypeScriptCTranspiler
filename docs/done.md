...
- ES2024 static `Map.groupBy(items: T[] | Set<T> | string, keyFn: (value: T, index: number) => K): Map<K, T[]>` groups a typed array, typed Set, or string source into a typed `Map<K, T[]>` by the callback's return value. Set sources preserve insertion order and string sources use code-point order for callback index assignment. The callback may be an inline arrow/function expression with an expression body or single-return block body, or a function reference (including generic functions); callbacks that declare `this: any` receive the JavaScript default `undefined` receiver. Tests: `map_group_by`, `group_by_this_param`
