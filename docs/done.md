...
- Standard method decorators on named instance and static methods are evaluated during module initialization. Direct decorator functions and dynamic callable/proxy-backed decorators receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; multiple decorator expressions on the same method evaluate top-down and invoke bottom-up; static method decorators can return replacement functions used by direct `Class.method(...)` calls, and instance method decorators can return replacement functions used by direct `obj.method(...)` calls with the actual instance passed to typed class `this` receivers. Instance method replacements can call the original method value through `Reflect.apply(value, this, args)`. Literal and const-literal computed method names are covered; computed non-literal decorator names remain deferred. Tests: `method_decorator_basic`, `static_method_decorator_replacement`, `instance_method_decorator_replacement`, `instance_method_decorator_this_receiver`, `instance_method_decorator_original`, `decorator_computed_members`, `decorator_add_initializer_basic`, `decorator_proxy_basic`, `decorator_replacement_order`, `decorator_factory_order`
...
- Standard getter and setter decorators on named instance and static accessors are evaluated during module initialization. Direct decorator functions receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; static getter decorators can return replacement getter functions used by direct `Class.property` reads, static setter decorators can return replacement setter functions used by direct `Class.property = value` writes, instance getter and setter decorators can return replacement functions used by direct `obj.property` reads/writes with the actual instance passed to typed class `this` receivers. Instance getter and setter replacements can call the original accessor value through `Reflect.apply(value, this, args)`. Literal and const-literal computed accessor names are covered; computed non-literal decorator names remain deferred. Tests: `accessor_decorator_basic`, `static_getter_decorator_replacement`, `static_setter_decorator_replacement`, `instance_getter_decorator_replacement`, `instance_getter_decorator_this_receiver`, `instance_getter_decorator_original`, `instance_setter_decorator_replacement`, `instance_setter_decorator_this_receiver`, `instance_setter_decorator_original`, `decorator_computed_members`, `decorator_proxy_basic`
