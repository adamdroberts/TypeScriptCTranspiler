...
- Typed object-key `WeakMap` supports `.set(k, v)`, `.get(k)`, `.has(k)`, `.delete(k)`, and construction from typed `Map<K, V>` sources with matching object-key/value types. Test: `weak_collections`
...
- Generated-C DCE uses exact static string-key `Array.from(new Map(...))` lengths through `Object.entries(...)`, `Array.of(...)`, one-argument `Array.from(...)`, const entry arrays, and array-spread constructor sources, enabling unused in-range `.with(...)` calls after duplicate key elimination to be pruned. Test: `generated_c_dce_const`
...
- Generated-C DCE uses exact static string-literal `Array.from(new Set(...))` / string-key `Array.from(new Map(...))` length proofs beyond singleton collections, enabling unused in-range `.with(...)` calls on multi-entry collections to be pruned. Test: `generated_c_dce_const`
