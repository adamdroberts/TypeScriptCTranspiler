...
- Standard method decorators on named instance and static methods are evaluated during module initialization. Direct decorator functions and dynamic callable/proxy-backed decorators receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; multiple decorator expressions on the same method evaluate top-down and invoke bottom-up; static method decorators can return replacement functions used by direct `Class.method(...)` calls, and instance method decorators can return replacement functions used by direct `obj.method(...)` calls when the replacement does not declare a typed `this` receiver. Instance replacement `this` receiver bridging, original-method replacement calls, and computed non-literal decorator names remain deferred. Tests: `method_decorator_basic`, `static_method_decorator_replacement`, `instance_method_decorator_replacement`, `decorator_add_initializer_basic`, `decorator_proxy_basic`, `decorator_replacement_order`, `decorator_factory_order`
...
| `instance_method_decorator_replacement` | standard instance method decorators returning replacement functions without typed this |
