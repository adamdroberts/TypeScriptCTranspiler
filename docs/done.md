...
- Generated-C DCE prunes unused `Object.groupBy(...)` / `Map.groupBy(...)` over statically empty returned array, typed Set, string, and typed Map sources, including earlier-const empty string sources plus earlier-const and copy-constructed typed Set and Map sources. Test: `generated_c_dce_const`
