...
- `fs.constants.COPYFILE_EXCL`, `COPYFILE_FICLONE`, and `COPYFILE_FICLONE_FORCE` are exposed. `fs.copyFileSync(src, dest, mode?)` and immediate `fs.promises.copyFile(src, dest, mode?)` accept optional flags, with `COPYFILE_EXCL` enforcing no overwrite and earlier static `const` aliases for `undefined` flags treated as default copy mode. Test: `fs_copy_flags`
