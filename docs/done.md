...
- Generated-C DCE also recognizes unused pure no-argument `Map` / `Set` / `WeakMap` / `WeakSet` constructors, fresh-array `pop()` / `shift()` / `reverse()` calls with pure ignored extra arguments, fresh-array `fill(...)` / `copyWithin(...)` calls with pure value/index inputs, fresh-array `push(...)` / `unshift(...)` calls with pure inserted values, default `sort()` calls on fresh string-array literals, comparator `sort(...)` / `toSorted(...)` calls on fresh arrays with at most one element, `Array.from(empty, mapper[, thisArg])` calls over statically empty array/string sources, `Object.groupBy(...)` / `Map.groupBy(...)` calls over statically empty array sources, `map(...)` / `flatMap(...)` / `filter(...)` / `forEach(...)`, predicate/search HOF calls, and `reduce(...)` / `reduceRight(...)` calls with explicit pure initial values on fresh empty array literals with pure callback and `thisArg` expressions where applicable, string `match(...)` / `matchAll(...)` / `search(...)` calls with statically valid string RegExp patterns, string `split(...)` calls with fresh RegExp separators and optional primitive numeric limits, string `replace(...)` / `replaceAll(...)` calls with fresh RegExp search patterns and string-coercible replacements, plus unused pure fresh-RegExp `test(...)` / `exec(...)` calls with string-coercible inputs and fresh-RegExp `toString()` / `toLocaleString()` / `valueOf()` calls. Test: `generated_c_dce_const`
