...
- Generated-C DCE uses exact static string, numeric, and boolean key `Array.from(new Map(...))` lengths through literal entry arrays, and string-key lengths through `Object.entries(...)`, `Array.of(...)`, one-argument `Array.from(...)`, const entry arrays, and array-spread constructor sources, enabling unused in-range `.with(...)` calls after duplicate key elimination to be pruned. Test: `generated_c_dce_const`
