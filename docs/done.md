...
- Generated-C DCE prunes unused `Promise.all(...)` / `Promise.allSettled(...)` / `Promise.any(...)` / `Promise.race(...)` over statically empty returned array, earlier-const/copied typed array, and earlier-const/copy-constructed typed Set sources. Test: `generated_c_dce_const`
