...
- `fs.readdirSync(path[, "utf8" | "hex" | "base64" | "buffer" | { encoding }], ...ignored)` → `tsc_fs_readdir_sync` / `tsc_fs_readdir_buffer_sync`, returning string arrays for the bounded UTF-8/hex/base64 encoding subset and Buffer arrays for explicit Buffer encoding; `fs.readdirSync(path, { recursive: true })` and immediate-settled `fs.promises.readdir(path, { recursive: true })` return recursive relative string results, with Buffer results for `{ encoding: "buffer", recursive: true }` and encoded string results for `{ encoding: "hex" | "base64", recursive: true }`; `fs.readdirSync(path, { withFileTypes: true })` and immediate-settled `fs.promises.readdir(path, { withFileTypes: true })` return a bounded `Dirent` subset with `name`, `isFile(...ignored)`, `isDirectory(...ignored)`, `isSymbolicLink(...ignored)`, `isBlockDevice(...ignored)`, `isCharacterDevice(...ignored)`, `isFIFO(...ignored)`, and `isSocket(...ignored)`, including recursive Dirent results for `{ withFileTypes: true, recursive: true }`. Sync and immediate-promise calls treat explicit `undefined` options as defaults while evaluating ignored extras. Tests: `fs_readdir_options`, `fs_readdir_buffer_options`, `fs_readdir_encoded_options`, `fs_readdir_recursive`, `fs_readdir_dirents`, `fs_readdir_ignored_arguments`
