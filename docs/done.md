...
- CommonJS `require(...)` package edges resolve package `exports` and package-private `imports` condition maps through `node-addons`, `require`, `node`, and `default` branches in package order while building the AOT module graph, including wildcard target substitution and package-internal `#...` requires from the selected CommonJS source. Tests: `node_modules_package_require_conditions`, `node_modules_package_require_node_addons_conditions`, `node_modules_package_require_pattern_conditions`
