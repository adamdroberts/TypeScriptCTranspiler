...
- Synchronous `function*` declarations with ordinary `yield expr` statements and bounded `yield*` over arrays, strings, or dynamic iterable values lower to materialized array-backed `Iterator<T>` / `IterableIterator<T>` results consumable by existing `for...of` lowering. The materialized iterator supports `.next(value?)`, `.return(value)`, and `.throw(error)` cursor interactions; optional `.next(value)` inputs are evaluated and ignored in this eager subset, and explicit generator `return value` is exposed once through the first completed `.next()`. Async generators and lazy suspend/resume state machines remain deferred. Tests: `generator_functions`, `generator_next_argument`, `generator_return_value`
...
### Promise subset
- Settled `Promise<T>` values lower to `tsc_promise_t*`. `new Promise<T>(executor)` calls the executor synchronously with generated `resolve` / `reject` callbacks that settle at most once, converts executor throws into rejections unless the promise is already settled, and returns pending when the executor does not settle. `Promise.resolve(value, ...ignored)` resolves typed values, adopts existing native Promise records after evaluating ignored extras, and assimilates immediate dynamic thenables with callable `then` properties, including immediately settled nested thenables, self-resolution rejection, and immediate cycle rejection; `Promise.reject(reason, ...ignored)` rejects after evaluating ignored extras, `Promise.try(callback)` invokes the callback synchronously and turns returned values, returned native Promise records, or immediate dynamic thenables into fulfilled/adopted results while throws become rejected records, synchronous `.then(...)`, `.then(undefined, onRejected)`, `.catch(...)`, `.finally(...)`, and settled-array `Promise.all` / `allSettled` / `race` / `any` are implemented for compiler-known callbacks and promise arrays or typed `Set<Promise<T>>` sources. Omitted fulfillment handlers in `.then()` pass fulfilled values through, omitted/undefined `.catch()` / `.finally()` handlers preserve fulfilled, rejected, and pending state, immediate `.then` / `.catch` callbacks adopt returned native Promise records, assimilate returned immediate dynamic thenables including immediately settled nested thenables, self-resolution rejection, and immediate cycle rejection, or convert throws into rejected Promise records, and immediate `.finally()` callbacks preserve the original state unless they throw or return rejected/pending native Promise records. Top-level `async function` declarations, class `async` methods, and async function values return immediate Promise records for fulfilled values, adopted returned promises, implicit `undefined` returns, synchronous uncaught throw rejections, and immediate `await` over native Promise records or non-Promise values; fulfilled awaits unwrap values including typed `FSStats` and `Buffer` side-channel results, non-Promise awaits evaluate once and continue with that value, local `try`/`catch` can catch rejected awaits, uncaught rejected awaits return rejected Promise records, and pending awaits return pending Promise records without scheduling a continuation. `Promise.any` rejects with an AggregateError-shaped dynamic object containing `name`, `message`, and `errors` when every array or typed Set input rejects, including empty inputs; empty `Promise.race([])` and `Promise.race(new Set())` return pending promises; and pending records propagate through immediate `all` / `allSettled` / `race` / `any` over arrays or typed Set sources without synchronously invoking chained callbacks. `queueMicrotask(callback)` supports a bounded before-exit zero-argument callback queue drained after `process.nextTick`; `setTimeout(callback, 0, ...args)`, `setTimeout(callback, undefined, ...args)`, `setTimeout(callback, void 0, ...args)`, and `setTimeout(callback, -0, ...args)` support a bounded zero-delay queue with typed callback arguments and no fixed arity cap, drained after next ticks and microtasks, return numeric handles, and can be canceled before drain with `clearTimeout(handle)` or `clearInterval(handle)`; `setImmediate(callback, ...args)` supports a bounded before-exit queue with typed callback arguments and no fixed arity cap, drained after next ticks, microtasks, and zero-delay timeouts, returns a numeric handle, and can be canceled before drain with `clearImmediate(handle)`. Omitted or undefined-valued `clearTimeout` / `clearInterval` / `clearImmediate` handles are no-op clears that still evaluate argument side effects. This is a settled/immediate subset; suspend/resume state machines, Promise microtask ordering, real timer integration, broader thenable assimilation edge cases outside the immediate dynamic settled/nested/self-resolution/cycle subset, and broader awaited rejection edge cases remain deferred. Tests: `promise_executor`, `promise_resolve_adopt`, `promise_thenable_assimilation`, `promise_thenable_recursive`, `promise_thenable_self_resolution`, `promise_try`, `promise_callback_adopt`, `promise_finally_adopt`, `promise_settled`, `promise_then_passthrough`, `promise_empty_handlers`, `promise_callback_throw`, `async_function_immediate`, `async_methods_immediate`, `async_function_values_immediate`, `async_throw_rejection`, `async_await_immediate`, `async_await_values_immediate`, `async_await_try_catch`, `promise_any_aggregate`, `promise_race_empty_pending`, `promise_pending_combinators`, `queue_microtask`, `set_timeout_zero`, `set_immediate`, `timers_clear`, `scheduler_callback_this`, `scheduler_many_args`, `fs_read_file_buffer_options`, `fs_read_file_null_buffer_options`
...
- ES2024 static `Map.groupBy(...)` groups typed array, typed Set, typed Map entry, or string sources by the callback's return value, returning typed `Map<K, T[]>` groups or `Map<K, ObjectEntry<MV, MK>[]>` for Map sources. Set and Map sources preserve insertion order and string sources use code-point order for callback index assignment. The callback may be an inline arrow/function expression with an expression body or single-return block body, or a function reference (including generic functions); callbacks that declare `this: any` receive the JavaScript default `undefined` receiver. Tests: `map_group_by`, `group_by_this_param`
...
- Dynamic array `forEach`, `map`, `flatMap`, `filter`, `reduce`/`reduceRight`, `find`, `findIndex`, `findLast`, `findLastIndex`, `some`, and `every` work with inline expression-body or single-return block-body callbacks and named callback references over `tsc_value_t` elements; callbacks receive element, index, receiver array, and accumulator arguments where JavaScript specifies them, optional `thisArg` values are evaluated once and bound for callbacks that declare `this: any`, and named callbacks receive values coerced to their declared parameter types. Tests: `dynamic_array_hof`, `dynamic_array_hof_more`, `dynamic_array_hof_refs`, `dynamic_array_find_last`, `dynamic_array_reduce`, `dynamic_array_reduce_right`, `dynamic_array_reduce_no_initial`, `dynamic_array_flatmap`, `array_hof_this_arg`
...
- `Object.create(proto)` and `Object.create(proto, descriptors)`, `Object.getPrototypeOf`, `Object.setPrototypeOf`, `Reflect.getPrototypeOf`, and `Reflect.setPrototypeOf` support dynamic object prototype chains; dynamic property reads, `in`, and `Reflect.has` walk prototypes while `Object.keys`, `Reflect.ownKeys`, descriptors, and `Object.hasOwn` remain own-property based. `Object.create(...)` rejects invalid prototypes through the catchable runtime exception path. `Object.setPrototypeOf` rejects nullish targets, invalid prototype values before proxy trap dispatch, failed non-extensible updates, and falsy proxy `setPrototypeOf` trap results while preserving primitive target compatibility. Tests: `object_prototypes`, `object_create_descriptors`, `object_set_prototype_of_validation`, `dynamic_object_iteration_errors`
...
- `fs.constants.COPYFILE_EXCL`, `COPYFILE_FICLONE`, and `COPYFILE_FICLONE_FORCE` are exposed. `fs.copyFileSync(src, dest, mode?)` and immediate `fs.promises.copyFile(src, dest, mode?)` accept optional flags, with `COPYFILE_EXCL` enforcing no overwrite and earlier static `const` aliases for `undefined` flags plus direct side-effect-free `void` defaults such as `void 0` treated as default copy mode. Test: `fs_copy_flags`
- `fs.cpSync(src, dest[, { recursive, force, errorOnExist, dereference, verbatimSymlinks, mode, preserveTimestamps }], ...ignored)` and immediate-settled `fs.promises.cp(src, dest[, options], ...ignored)` copy regular files, recursive directory trees, and symlinks in a bounded subset, with `force: false` skip behavior, `errorOnExist` errors only when `force` is false, numeric copy flags such as `COPYFILE_EXCL` for regular files, literal `preserveTimestamps` support for regular-file copies, earlier static `const` aliases for supported boolean option values, default resolved symlink targets, verbatim symlink target preservation, dereferenced symlink copies, and ignored trailing argument evaluation. Tests: `fs_cp_recursive`, `fs_cp_options`, `fs_cp_symlink_options`, `fs_mutation_ignored_arguments`
- `fs.readdirSync(path[, "utf8" | "hex" | "base64" | "buffer" | null | { encoding }], ...ignored)` → `tsc_fs_readdir_sync` / `tsc_fs_readdir_buffer_sync`, returning string arrays for the bounded UTF-8/hex/base64 encoding subset plus direct null and `{ encoding: null }` defaults, and Buffer arrays for explicit Buffer encoding; `fs.readdirSync(path, { recursive: true })` and immediate-settled `fs.promises.readdir(path, { recursive: true })` return recursive relative string results, with Buffer results for `{ encoding: "buffer", recursive: true }` and encoded string results for `{ encoding: "hex" | "base64", recursive: true }`; `fs.readdirSync(path, { withFileTypes: true })` and immediate-settled `fs.promises.readdir(path, { withFileTypes: true })` return a bounded `Dirent` subset with `name`, `isFile(...ignored)`, `isDirectory(...ignored)`, `isSymbolicLink(...ignored)`, `isBlockDevice(...ignored)`, `isCharacterDevice(...ignored)`, `isFIFO(...ignored)`, and `isSocket(...ignored)`, including recursive Dirent results for `{ withFileTypes: true, recursive: true }`. Sync and immediate-promise calls treat explicit `undefined` and side-effect-free `void` options as defaults while evaluating ignored extras. Tests: `fs_readdir_options`, `fs_readdir_buffer_options`, `fs_readdir_encoded_options`, `fs_readdir_null_encoding_options`, `fs_readdir_recursive`, `fs_readdir_dirents`, `fs_readdir_ignored_arguments`
- `fs.mkdirSync(...)`, immediate `fs.promises.mkdir(...)`, `fs.readdirSync(...)`, immediate `fs.promises.readdir(...)`, `fs.writeFileSync(...)`, `fs.appendFileSync(...)`, immediate `fs.promises.writeFile(...)`, and immediate `fs.promises.appendFile(...)` accept earlier static `const` aliases for supported boolean option values such as `recursive`, `withFileTypes`, and `flush`. Tests: `fs_mkdir_mode_options`, `fs_readdir_options`, `fs_write_append_mode_options`
...
- `fs.readdirSync(...)`, `fs.realpathSync(...)`, `fs.readlinkSync(...)`, `fs.mkdtempSync(...)`, and immediate `fs.promises.readdir(...)` / `realpath(...)` / `readlink(...)` / `mkdtemp(...)` accept earlier static `const` aliases for supported string `encoding` option values; readdir supports literal `"hex"` / `"base64"` string and object encodings for string filename arrays plus null/default string encodings, and realpath/readlink/mkdtemp support those encodings for string results plus null/default string encodings. Tests: `fs_readdir_buffer_options`, `fs_readdir_encoded_options`, `fs_readdir_null_encoding_options`, `fs_link_path_encoding_options`, `fs_mkdtemp_encoding_options`, `fs_path_result_encoded_options`
...
- `fs.rmSync(...)`, `fs.rmdirSync(...)`, and immediate `fs.promises.rm(...)` / `rmdir(...)` accept earlier static `const` aliases for supported numeric `maxRetries` / `retryDelay` compatibility options. Test: `fs_recursive_options`
- `fs.statSync(path[, { bigint: false, throwIfNoEntry }], ...ignored)` and immediate-settled `fs.promises.stat(path[, { bigint: false, throwIfNoEntry }], ...ignored)` return a small typed `Stats` subset with `dev`, `ino`, `size`, `mode`, `nlink`, `uid`, `gid`, `rdev`, `blksize`, `blocks`, numeric `atimeMs` / `mtimeMs` / `ctimeMs` / `birthtimeMs` timestamps, Date-object `atime` / `mtime` / `ctime` / `birthtime` timestamps, `isFile(...ignored)`, `isDirectory(...ignored)`, `isSymbolicLink(...ignored)`, `isBlockDevice(...ignored)`, `isCharacterDevice(...ignored)`, `isFIFO(...ignored)`, and `isSocket(...ignored)`. Literal `{ throwIfNoEntry: false }` returns / fulfills with `undefined` for missing paths, supported boolean option values can use earlier static `const` aliases, and sync plus immediate-promise calls treat explicit `undefined` and direct side-effect-free `void` options such as `void 0` as defaults while evaluating ignored extras. Tests: `fs_stat`, `fs_stats_kinds`, `fs_stat_options`, `fs_stat_ignored_arguments`, `fs_promises_stat_access_ignored_arguments`, `fs_stats_times`, `fs_stats_metadata`
- `fs.lstatSync(path[, { bigint: false, throwIfNoEntry }], ...ignored)` and immediate-settled `fs.promises.lstat(path[, { bigint: false, throwIfNoEntry }], ...ignored)` return the same small typed `Stats` subset without following symlinks. Literal `{ throwIfNoEntry: false }` returns / fulfills with `undefined` for missing paths, supported boolean option values can use earlier static `const` aliases, and sync plus immediate-promise calls treat explicit `undefined` options as defaults while evaluating ignored extras. Tests: `fs_lstat`, `fs_stats_kinds`, `fs_stat_options`, `fs_stat_ignored_arguments`, `fs_promises_stat_access_ignored_arguments`, `fs_stats_times`, `fs_stats_metadata`
- Supported fs option objects treat explicit `undefined` and side-effect-free `void` property values as defaults across the bounded read/write/append/readdir/stat/mkdir/rm/rmdir/cp option subsets for sync and immediate-promise calls. Test: `fs_option_property_undefined`
- `fs.realpathSync(path[, "utf8" | "hex" | "base64" | "buffer" | null | { encoding }], ...ignored)` and immediate-settled `fs.promises.realpath(path[, "utf8" | "hex" | "base64" | "buffer" | null | { encoding }], ...ignored)` resolve paths through the host filesystem for the bounded UTF-8/hex/base64 string encodings, explicit Buffer encodings, and direct/null-object default string encodings, treating explicit `undefined` and side-effect-free `void` options as defaults while evaluating ignored extras. Tests: `fs_realpath`, `fs_link_path_encoding_options`, `fs_link_mkdtemp_ignored_arguments`, `fs_path_result_encoded_options`
- `fs.readlinkSync(path[, "utf8" | "hex" | "base64" | "buffer" | null | { encoding }], ...ignored)` and immediate-settled `fs.promises.readlink(path[, "utf8" | "hex" | "base64" | "buffer" | null | { encoding }], ...ignored)` read symlink targets through the host filesystem for the bounded UTF-8/hex/base64 string encodings, explicit Buffer encodings, and direct/null-object default string encodings, treating explicit `undefined` and side-effect-free `void` options as defaults while evaluating ignored extras. Tests: `fs_readlink`, `fs_link_path_encoding_options`, `fs_link_mkdtemp_ignored_arguments`, `fs_path_result_encoded_options`
...
- `fs.mkdtempSync(prefix[, "utf8" | "hex" | "base64" | "buffer" | null | { encoding }], ...ignored)` and immediate-settled `fs.promises.mkdtemp(prefix[, "utf8" | "hex" | "base64" | "buffer" | null | { encoding }], ...ignored)` create temporary directories from a prefix for the bounded UTF-8/hex/base64 string encodings, explicit Buffer encodings, and direct/null-object default string encodings, treating explicit `undefined` and side-effect-free `void` options as defaults while evaluating ignored extras. Tests: `fs_mkdtemp`, `fs_mkdtemp_encoding_options`, `fs_link_mkdtemp_ignored_arguments`, `fs_path_result_encoded_options`
- `fs.truncateSync(path, len?, ...ignored)` and immediate-settled `fs.promises.truncate(path, len?, ...ignored)` truncate files by path. Tests: `fs_truncate`, `fs_mutation_ignored_arguments`
...
### `net`
- `net.isIP(input, ...ignored)`, `net.isIPv4(input, ...ignored)`, and `net.isIPv6(input, ...ignored)` are implemented as a pure address-classification subset for global `net`, named imports, and namespace imports from `"net"` / `"node:net"`, backed by host `inet_pton`, with ignored extra-argument evaluation. Test: `net_is_ip`
...
- Supported `child_process` option objects treat explicit `undefined` and side-effect-free `void` property values as defaults across callback `exec`/`execFile`, sync `execSync`/`execFileSync`, and `spawnSync` subsets. Test: `child_process_option_property_undefined`
...
- Supported `child_process` numeric and string `killSignal` values can use earlier static `const` aliases across the callback, sync, and `spawnSync` subsets. Tests: `child_process_numeric_kill_signal`, `child_process_exec_sync_timeout_options`, `child_process_spawn_sync_kill_signal`
- Supported `child_process` string `encoding` option values can use earlier static `const` aliases across callback `exec`/`execFile`, sync `execSync`/`execFileSync`, and `spawnSync` subsets. Tests: `child_process_exec_callback_encoding`, `child_process_exec_sync_encoding`, `child_process_spawn_sync`
...
- `Buffer.byteLength(value, encoding?, ...ignored)`, `Buffer.isEncoding(encoding, ...ignored)`, static `Buffer.compare(a, b, ...ignored)`, and instance `buf.compare(other)` are implemented for the supported UTF-8/hex/base64 Buffer subset, including `"utf-8"` aliases, explicit `undefined` or side-effect-free `void` default encoding, and catchable unsupported-encoding validation. Tests: `buffer_static_more`, `buffer_errors`
- Buffer instances expose `.length`, numeric byte indexing/get-set, `.toString("utf8" | "hex")`, `.slice()`, `.subarray()`, `.fill()`, `.write()`, signed/unsigned integer and float/double reads/writes, `.swap16(...ignored)` / `.swap32(...ignored)` / `.swap64(...ignored)`, `.copy()`, number/string/Buffer `.indexOf()` / `.lastIndexOf()` / `.includes()`, and `.equals(other, ...ignored)`, with catchable validation failures for unsupported `toString` encodings, out-of-range read/write offsets, and invalid swap widths. Tests: `buffer`, `buffer_fill`, `buffer_write`, `buffer_uint8_io`, `buffer_uint_multi_io`, `buffer_int_io`, `buffer_float_io`, `buffer_swap`, `buffer_copy`, `buffer_search`, `buffer_search_more`, `buffer_errors`
...
- Generated-C DCE prunes unused valid `os.userInfo(...)` calls and direct field reads, including named/namespace imports, literal or earlier static `const` UTF-8 options, explicit `undefined` defaults, and primitive `Promise.resolve(...)` inputs. Test: `generated_c_dce_const`
...
- Generated-C DCE prunes unused pure primitive-returning `Buffer.byteLength(...)`, `Buffer.isEncoding(...)`, and `Buffer.isBuffer(...)` calls, including earlier static `const` aliases for supported UTF-8 encodings and primitive `Promise.resolve(...)` inputs. Test: `generated_c_dce_const`
- Generated-C DCE prunes unused pure `Buffer.from(...)`, `Buffer.alloc(...)`, `Buffer.allocUnsafe(...)`, and `Buffer.allocUnsafeSlow(...)` allocations when their bounded arguments cannot throw, including earlier static `const` aliases for supported UTF-8 encodings. Test: `generated_c_dce_const`
...
- Generated-C DCE treats pure string-returning fresh-Buffer methods as side-effect-free `.length`, string-method, and indexable operands, including primitive `Promise.resolve(...)` inputs. Test: `generated_c_dce_const`
...
- Generated-C DCE prunes unused pure `EventEmitter`, `EventTarget`, and bounded `Event` constructor calls, including named `EventEmitter` imports. Test: `generated_c_dce_const`
...
- Generated-C DCE prunes unused static/module `EventEmitter.listenerCount(...)`, `events.listenerCount(...)`, `events.getMaxListeners(...)`, and `events.getEventListeners(...)` helper calls over fresh empty emitters, including named imports and primitive `Promise.resolve(...)` inputs for primitive-returning calls. Test: `generated_c_dce_const`
...
- Generated-C DCE prunes unused static/module `events.once(new EventEmitter(), eventName, options)` calls with defaulted options, including named imports. Test: `generated_c_dce_const`
...
- Generated-C DCE prunes unused deterministic `crypto.createHash(...)` allocation, `Hash.update(...)`, and `Hash.digest(...)` chains over directly fresh hash objects with supported static algorithms/encodings, including named and namespace imports plus primitive `Promise.resolve(...)` inputs. Test: `generated_c_dce_const`
...
- Generated-C DCE prunes unused direct-fresh `EventTarget.addEventListener(...)` / `removeEventListener(...)` calls with side-effect-free listener operands and supported literal options, including primitive `Promise.resolve(...)` inputs for void results. Test: `generated_c_dce_const`
...
- Generated-C DCE prunes unused `process.stdin` / `stdout` / `stderr` metadata reads such as `fd`, `isTTY`, `readable`, and `writable`, including false-valued output-stream `readable` reads and primitive `Promise.resolve(...)` inputs. Test: `generated_c_dce_const`
...
| `dns_lookup_hints` | dns.lookup and dns.promises.lookup hints constants and static const aliases |
...
| `fs_option_property_undefined` | fs option objects treat explicit undefined property values as defaults |
...
| `fs_cp_options` | fs.cpSync and immediate-settled fs.promises.cp force/errorOnExist/mode/preserveTimestamps and const boolean options |
...
| `proxy_array_extensibility_trap_invariants` | array Proxy isExtensible/preventExtensions trap results validate against target extensibility |
| `proxy_array_ownkeys_invariants` | array Proxy ownKeys traps enforce length, sealed-index, and non-extensible exact-key invariants |
| `proxy_array_descriptor_invariants` | array Proxy getOwnPropertyDescriptor traps enforce non-configurable, non-extensible, and frozen descriptor invariants |
| `proxy_array_define_invariants` | array Proxy defineProperty traps enforce non-extensible, non-configurable, and frozen fixed-data invariants |
| `proxy_array_get_set_invariants` | array Proxy get/set traps enforce frozen index and frozen length fixed-data invariants |
| `proxy_array_has_delete_invariants` | array Proxy has/deleteProperty traps enforce non-configurable and non-extensible key invariants |
...
| `map_group_by` | ES2024 Map.groupBy over typed arrays, typed Set sources, and strings with arrow/block-body and named callback references |
...
| `object_group_by` | ES2024 Object.groupBy over typed arrays, typed Set sources, and strings returning a dynamic null-prototype object with expression/block-body callbacks |
