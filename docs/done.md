...
- Standard method decorators on named instance and static methods are evaluated during module initialization. Direct decorator functions and dynamic callable/proxy-backed decorators receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; multiple decorator expressions on the same method evaluate top-down and invoke bottom-up; static method decorators can return replacement functions used by direct `Class.method(...)` calls, and instance method decorators can return replacement functions used by direct `obj.method(...)` calls with the actual instance passed to typed class `this` receivers. Original-method replacement calls and computed non-literal decorator names remain deferred. Tests: `method_decorator_basic`, `static_method_decorator_replacement`, `instance_method_decorator_replacement`, `instance_method_decorator_this_receiver`, `decorator_add_initializer_basic`, `decorator_proxy_basic`, `decorator_replacement_order`, `decorator_factory_order`
...
- Standard getter and setter decorators on named instance and static accessors are evaluated during module initialization. Direct decorator functions receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; static getter decorators can return replacement getter functions used by direct `Class.property` reads, static setter decorators can return replacement setter functions used by direct `Class.property = value` writes, instance getter decorators can return replacement functions used by direct `obj.property` reads with the actual instance passed to typed class `this` receivers, and instance setter decorators can return replacement functions used by direct `obj.property = value` writes when the replacement does not declare a typed `this` receiver. Instance setter `this` receiver bridging, instance accessor original-member bridging, and computed non-literal decorator names remain deferred. Tests: `accessor_decorator_basic`, `static_getter_decorator_replacement`, `static_setter_decorator_replacement`, `instance_getter_decorator_replacement`, `instance_getter_decorator_this_receiver`, `instance_setter_decorator_replacement`, `decorator_proxy_basic`
...
| `instance_setter_decorator_replacement` | standard instance setter decorators returning replacement functions without typed this |
