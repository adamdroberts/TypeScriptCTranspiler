...
- `fs.writeFileSync(path, data[, options], ...ignored)` → `tsc_fs_write_file_sync`, including explicit `undefined` default options and ignored-extra evaluation. Test: `fs_write_append_ignored_arguments`
...
- `fs.readFileSync(...)`, `fs.writeFileSync(...)`, `fs.appendFileSync(...)`, and immediate `fs.promises.readFile(...)` / `writeFile(...)` / `appendFile(...)` accept earlier static `const` aliases for supported string `encoding` and `flag` option values; readFile additionally supports literal `"hex"` / `"base64"` string and object encodings for string results. Tests: `fs_file_flag_options`, `fs_read_file_encoded_options`
