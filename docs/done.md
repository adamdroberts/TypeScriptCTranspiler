...
- `setTimeout(callback, 0, ...args)` plus explicit `undefined` / `void 0` / `-0` delay forms support a bounded zero-delay callback queue with up to three typed callback arguments, drained after `process.nextTick` and `queueMicrotask` callbacks before `setImmediate` and process exit. It returns a numeric handle accepted by `clearTimeout(handle)` before the drain; omitted or undefined-valued clear handles are no-op clears that still evaluate ignored arguments.
