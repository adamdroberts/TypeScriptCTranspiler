...
### `fs`
- `fs.readFileSync(path[, options], ...ignored)` → `tsc_fs_read_file_sync` / `tsc_fs_read_file_buffer_sync`, treating explicit `undefined` options as defaults while evaluating ignored extras. Test: `fs_read_file_ignored_arguments`
...
- `fs.readFileSync(...)`, `fs.writeFileSync(...)`, `fs.appendFileSync(...)`, and immediate `fs.promises.readFile(...)` / `writeFile(...)` / `appendFile(...)` accept earlier static `const` aliases for supported string `encoding` and `flag` option values. Test: `fs_file_flag_options`
