...
- Standard class decorators on named classes are evaluated during module initialization. Direct decorator functions receive a dynamic context object with `kind: "class"` and `name`; class replacement, metadata, `addInitializer`, and member decorators remain deferred. Test: `class_decorator_basic`
