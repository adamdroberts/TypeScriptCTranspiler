...
- Generated-C DCE prunes unused `Object.groupBy(...)` / `Map.groupBy(...)` over statically empty returned array, typed Set, string, and typed Map sources, including earlier-const and copied typed array sources, earlier-const empty string sources, and earlier-const/copy-constructed typed Set and Map sources. It also prunes `Array.from(empty, mapper)` through earlier-const and copied empty typed array sources. Test: `generated_c_dce_const`
