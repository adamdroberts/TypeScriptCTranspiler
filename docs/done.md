...
- `fs.readdirSync(path[, "utf8" | "buffer" | { encoding }], ...ignored)` → `tsc_fs_readdir_sync` / `tsc_fs_readdir_buffer_sync`, returning string arrays for the bounded UTF-8 encoding subset and Buffer arrays for explicit Buffer encoding; `fs.readdirSync(path, { recursive: true })` and immediate-settled `fs.promises.readdir(path, { recursive: true })` return recursive relative string results, with Buffer results for `{ encoding: "buffer", recursive: true }`; `fs.readdirSync(path, { withFileTypes: true })` and immediate-settled `fs.promises.readdir(path, { withFileTypes: true })` return a bounded `Dirent` subset with `name`, `isFile(...ignored)`, `isDirectory(...ignored)`, `isSymbolicLink(...ignored)`, `isBlockDevice(...ignored)`, `isCharacterDevice(...ignored)`, `isFIFO(...ignored)`, and `isSocket(...ignored)`, including recursive Dirent results for `{ withFileTypes: true, recursive: true }`. Sync and immediate-promise calls treat explicit `undefined` options as defaults while evaluating ignored extras. Tests: `fs_readdir_options`, `fs_readdir_buffer_options`, `fs_readdir_recursive`, `fs_readdir_dirents`, `fs_readdir_ignored_arguments`
...
- `fs.readdirSync(...)`, `fs.realpathSync(...)`, `fs.readlinkSync(...)`, `fs.mkdtempSync(...)`, and immediate `fs.promises.readdir(...)` / `realpath(...)` / `readlink(...)` / `mkdtemp(...)` accept earlier static `const` aliases for supported string `encoding` option values. Tests: `fs_readdir_buffer_options`, `fs_link_path_encoding_options`, `fs_mkdtemp_encoding_options`
...
- `fs.realpathSync(path[, "utf8" | "buffer" | { encoding }], ...ignored)` and immediate-settled `fs.promises.realpath(path[, "utf8" | "buffer" | { encoding }], ...ignored)` resolve paths through the host filesystem for the bounded UTF-8 and explicit Buffer/null object encoding subset, treating explicit `undefined` options as defaults while evaluating ignored extras. Tests: `fs_realpath`, `fs_link_path_encoding_options`, `fs_link_mkdtemp_ignored_arguments`
- `fs.readlinkSync(path[, "utf8" | "buffer" | { encoding }], ...ignored)` and immediate-settled `fs.promises.readlink(path[, "utf8" | "buffer" | { encoding }], ...ignored)` read symlink targets through the host filesystem for the bounded UTF-8 and explicit Buffer/null object encoding subset, treating explicit `undefined` options as defaults while evaluating ignored extras. Tests: `fs_readlink`, `fs_link_path_encoding_options`, `fs_link_mkdtemp_ignored_arguments`
...
- `fs.mkdtempSync(prefix[, "utf8" | "buffer" | { encoding }], ...ignored)` and immediate-settled `fs.promises.mkdtemp(prefix[, "utf8" | "buffer" | { encoding }], ...ignored)` create temporary directories from a prefix for the bounded UTF-8 and explicit Buffer/null object encoding subset, treating explicit `undefined` options as defaults while evaluating ignored extras. Tests: `fs_mkdtemp`, `fs_mkdtemp_encoding_options`, `fs_link_mkdtemp_ignored_arguments`
