...
- Standard getter and setter decorators on named instance and static accessors are evaluated during module initialization. Direct decorator functions receive a dynamic context object with `kind`, `name`, `static`, `private`, `metadata`, and `addInitializer`; static getter decorators can return replacement getter functions used by direct `Class.property` reads, static setter decorators can return replacement setter functions used by direct `Class.property = value` writes, and instance getter decorators can return replacement getter functions used by direct `obj.property` reads when the replacement does not declare a typed `this` receiver. Instance getter `this` receiver/original-getter bridging, instance setter replacement, and computed non-literal decorator names remain deferred. Tests: `accessor_decorator_basic`, `static_getter_decorator_replacement`, `static_setter_decorator_replacement`, `instance_getter_decorator_replacement`, `decorator_proxy_basic`
