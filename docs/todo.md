...
- **Phase 14 — npm integration**
  ...
  - Basic `allowJs: true` package-source loading is implemented for pure ESM JavaScript packages, including transitive package-local side-effect/default/namespace imports, transitive default/namespace re-export barrels over package-local JavaScript modules, and default imports of package-local CommonJS object-literal helpers emitted as finite named members; broader JS package patterns remain active work.
