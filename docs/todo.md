...
- **Phase 6 — `async`/`await` + libuv event loop**
  - Remaining work: general multi-suspension `await` state-machine lowering beyond the current direct tail-await, transparent const/let awaited-alias, transparent initialized and uninitialized direct assignment-awaited-alias, bounded single-await, embedded return-await, transparent local initializer-return, transparent initialized and uninitialized assignment-return, expression-bodied async-arrow direct/embedded-await, source `try`/`catch`/`finally`, and arbitrary-length leading awaited-local continuation subsets; broader `await` inside nested control-flow constructs and loops beyond the covered post-await synchronous statement subsets; broader awaited expressions outside the current leading local declaration, transparent local initializer-return, transparent const/let awaited-alias, transparent initialized and uninitialized direct assignment-awaited-alias, transparent initialized and uninitialized assignment-return, expression-statement, `return await`, expression-bodied async-arrow direct/embedded-await, source `try`/`catch`, and source `try`/`finally` recognizers; broader locals that must survive across general suspension points beyond the current primitive, dynamic, typed Promise, typed array/object/class/function, typed collection, typed RegExp, typed Node crypto/events/fs object, Event/EventTarget, Date/Error/AggregateError, URL/URLSearchParams, Buffer/ArrayBuffer/DataView, TextEncoder/TextDecoder, and symbol prelude captures; broader thenable assimilation edge cases outside the immediate dynamic settled/nested/self-resolution/cycle and executor-resolved thenable/nested-promise subset; full async function edge cases beyond the current declaration, method, lifted function-value, closure function-value, and expression-bodied async-arrow continuation routing; and future async I/O on top of libuv.
