...
- **Phase 6 — `async`/`await` + libuv event loop**
  - Remaining work: general multi-suspension `await` state-machine lowering beyond the current direct tail-await, direct awaited-alias return including parenthesized awaited initializer, single awaited local including parenthesized awaited initializer plus final return-expression or expressionless-`return` continuation subsets, bare or parenthesized expression-statement `await` plus final return subset, final return-expression subset containing one embedded pending `await`, direct `return await` including parenthesized direct return-await inside bounded `if` / fallthrough async control flow, bounded `if` / fallthrough branch return expressions including parenthesized direct return-await containing one embedded pending `await`, bounded nested `if` / fallthrough branch return trees whose return expressions each contain one embedded pending `await`, bounded conditional return expressions including parenthesized outer conditionals whose true and false expressions each contain one embedded pending `await`, bounded conditional return expressions including parenthesized outer conditionals with one pending-`await` branch and one synchronous branch, bounded `if` / fallthrough branch return trees that mix one pending-`await` branch with one synchronous branch, bounded nested conditional return-expression trees including parenthesized outer conditionals that mix pending-`await` and synchronous leaves, bounded `if` / fallthrough return branches whose return expression is a conditional tree mixing pending-`await` and synchronous leaves, bounded `&&` / `||` / `??` return expressions including parenthesized outer logical expressions whose right side contains a pending `await`, bounded leading local capture before final return expressions containing a pending `await` outside the direct `return await` adoption and final `if` / fallthrough / nested `if` / conditional / parenthesized conditional / conditional-branch / logical / parenthesized logical / expression-statement-prelude / expression-statement-direct-return-await / local-prelude-direct-return-await / expression-statement-awaited-local / local-prelude-awaited-local / expression-statement-leading-awaited-chain / local-prelude-leading-awaited-chain subsets, broader `await` inside nested control-flow constructs, `await` in loops, broader source `try`/`catch`/`finally` suspension beyond the current declaration/method/function-value single awaited-local including parenthesized awaited initializer, bare-await, parenthesized bare-await, and direct-return-await `try` plus synchronous catch-return, catch-throw, catch-prelude-return, catch-prelude-throw, catch-prelude-local-return, catch-prelude-local-throw, catch-fallthrough-return, catch-expressionless-return, finally-return, finally-expressionless-return, finally-local-return, finally-local-fallthrough-return, finally-fallthrough-return, bare-await finally-return, parenthesized bare-await finally-return, bare-await finally-fallthrough-return, and parenthesized bare-await finally-fallthrough-return subsets, broader awaited expressions outside the leading local declaration pattern beyond the parenthesized awaited-local initializer, bare or parenthesized expression-statement, bare or parenthesized try/catch, and bare or parenthesized try/finally subsets, broader locals that must survive across general suspension points, broader thenable assimilation edge cases outside the immediate dynamic settled/nested/self-resolution/cycle and executor-resolved thenable/nested-promise subset, full async function edge cases beyond the current declaration, method, lifted function-value, and closure function-value continuation routing, and future async I/O on top of libuv.
