...
4. Map standard feature tags to pinned normative clause IDs in `feature-map.json`. Map current normative `esid` values through the generated anchor catalog. Repair legacy metadata with one reviewed `esidAliases` or semantic-directory `pathRules` entry in `mapping-overrides.json`; qualify a directory rule by `esid` when pinned upstream metadata points at informative material. Use a source-hashed exact test override only for genuine ambiguity. Never create a file-by-file fixture ladder, infer mappings invisibly, use globs, or leave an unused mapping rule.
...
The manual **ECMAScript 2026 conformance evidence** workflow is the release path. Dispatch it for the exact revision under review. Its preflight intentionally fails red and uploads `ecmascript-2026-claim-<source-sha>` while the native Test262 host is ineligible; today the absent property specifications and any unavailable required E2E dependency also remain explicit local-gate blockers. Only a reviewed host with `claimEligible: true`, no semantic delegation, and every required capability enabled proceeds to exhaustive shards, exact merges, and the final claim job.
