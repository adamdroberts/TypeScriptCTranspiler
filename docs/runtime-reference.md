...
| Symbol | Signature | JS equivalent |
|--------|-----------|---------------|
| `tsc_http_request(options, response_listener)` | `tsc_value_t` | `http.request(options, callback?)` — opens a native TCP connection, buffers bounded string/Buffer request writes until `end`, encodes `Transfer-Encoding: chunked` requests when selected, and parses one close-delimited HTTP/1.1 response with status/version/headers/body metadata including decoded chunks; explicit chunked responses also expose `data` / `end` events; keep-alive, general streaming/backpressure, `https`, and `http2` remain deferred |
...
| `tsc_http_create_server(request_listener)` | `tsc_value_t` | `http.createServer(requestListener?)` — creates an HTTP/1.1 server over the native TCP runtime, parsing one bounded request including chunked bodies and exposing request metadata plus response status/header/body methods; explicit chunked response writes flush framed chunks; keep-alive, general streaming/backpressure, `https`, and `http2` remain deferred |
