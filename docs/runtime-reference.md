...
| `tsc_fs_promises_opendir_async(path, recursive, encoding, buffer_size)` | `tsc_promise_t*` | Nonrecursive libuv-backed `fs.promises.opendir(...)` implementation for Promise-chain call sites; opens with `uv_fs_opendir`, services bounded `Dir.read()` and async-iterator requests with `uv_fs_readdir`, and disposes with `uv_fs_closedir`; recursive calls and direct-await call sites retain the POSIX fallback until broader pending-await lowering is available |
