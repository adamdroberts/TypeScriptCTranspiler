...
| `tsc_fs_promises_opendir_async(path, recursive, encoding, buffer_size)` | `tsc_promise_t*` | Libuv-backed `fs.promises.opendir(...)` implementation for Promise-chain call sites; opens root and recursive child handles with `uv_fs_opendir`, services bounded `Dir.read()` and async-iterator requests with `uv_fs_readdir`, restores parent frames depth-first, and disposes active/parent handles with `uv_fs_closedir`; direct-await call sites retain the POSIX fallback until broader pending-await lowering is available |
