...
| `tsc_fs_opendir_sync(path, recursive)` | `tsc_value_t` | `fs.opendirSync(path[, { recursive }])` — bounded POSIX directory handle with sync/Promise reads, depth-first recursive traversal, async-iterator methods, and idempotent sync/async disposal |
