...
| `tsc_fs_promises_read_file_encoded_async(path, encoding)` | `tsc_promise_t*` | Libuv-backed `fs.promises.readFile(path, "hex" \| "base64")`; opens, reads in chunks, closes, encodes the bytes, then fulfills or rejects the pending Promise |
| `tsc_fs_promises_write_file_string_async(path, data, append, exclusive, update, mode)` | `tsc_promise_t*` | Libuv-backed `fs.promises.writeFile` / `appendFile` for UTF-8 string data and the supported flag/mode combinations |
| `tsc_fs_promises_write_file_buffer_async(path, data, append, exclusive, update, mode)` | `tsc_promise_t*` | Libuv-backed `fs.promises.writeFile` / `appendFile` for Buffer data, including emitter-decoded hex/base64 strings |
| `tsc_fs_promises_readdir_async(path, want_buffer)` | `tsc_promise_t*` | Libuv-backed non-recursive `fs.promises.readdir` for UTF-8 string or Buffer filename results; scans asynchronously and fulfills or rejects the pending Promise |
| `tsc_fs_promises_readdir_recursive_async(path, want_buffer)` | `tsc_promise_t*` | Libuv-backed recursive `fs.promises.readdir` for UTF-8 string or Buffer relative filename results; walks directory entries with sequential `uv_fs_scandir` requests without following symlink entries |
| `tsc_fs_promises_readdir_encoded_async(path, encoding)` | `tsc_promise_t*` | Libuv-backed non-recursive `fs.promises.readdir` for hex/base64 string filename results; scans raw names asynchronously, applies the requested encoding at completion, and fulfills or rejects the pending Promise |
| `tsc_fs_promises_readdir_recursive_encoded_async(path, encoding)` | `tsc_promise_t*` | Libuv-backed recursive `fs.promises.readdir` for hex/base64 relative filename results; walks directory entries with sequential `uv_fs_scandir` requests without following symlink entries, applies encoding at completion, and fulfills or rejects the pending Promise |
| `tsc_fs_promises_readdir_dirents_async(path, encoding)` | `tsc_promise_t*` | Libuv-backed non-recursive `fs.promises.readdir(path, { withFileTypes: true })`; scans typed `FSDirent` records asynchronously, applies the requested name encoding, and fulfills or rejects the pending Promise |
| `tsc_fs_promises_readdir_recursive_dirents_async(path, encoding)` | `tsc_promise_t*` | Libuv-backed recursive `fs.promises.readdir(path, { recursive: true, withFileTypes: true })`; walks typed directory entries with sequential `uv_fs_scandir` requests without following symlink entries, applies name encoding, and fulfills or rejects the pending Promise |
| `tsc_fs_promises_access_async(path, mode)` | `tsc_promise_t*` | Libuv-backed `fs.promises.access(path[, mode])`; performs the access check asynchronously and fulfills or rejects the pending Promise |
| `tsc_fs_promises_stat_async(path, throw_if_no_entry)` | `tsc_promise_t*` | Libuv-backed `fs.promises.stat(path)`; fulfills the typed `FSStats` side-channel or, when `throwIfNoEntry` is false, a null side-channel for a missing path |
| `tsc_fs_promises_lstat_async(path, throw_if_no_entry)` | `tsc_promise_t*` | Libuv-backed `fs.promises.lstat(path)` with the same missing-entry behavior, without following the final symbolic link |
| `tsc_fs_promises_statfs_async(path)` | `tsc_promise_t*` | Libuv-backed `fs.promises.statfs(path)`; fulfills a dynamic object with numeric `bsize`, `frsize`, `blocks`, `bfree`, `bavail`, `files`, and `ffree` fields or rejects |
...
| `tsc_fs_promises_realpath_async(path, encoding)` | `tsc_promise_t*` | Libuv-backed `fs.promises.realpath(path[, options])`; fulfills a UTF-8, encoded-string, or Buffer result after asynchronous resolution, or rejects with the bounded realpath error contract |
| `tsc_fs_promises_readlink_async(path, encoding)` | `tsc_promise_t*` | Libuv-backed `fs.promises.readlink(path[, options])`; fulfills a UTF-8, encoded-string, or Buffer target after asynchronous resolution, or rejects with the bounded readlink error contract |
| `tsc_fs_promises_mkdtemp_async(prefix, encoding)` | `tsc_promise_t*` | Libuv-backed `fs.promises.mkdtemp(prefix[, options])`; fulfills a UTF-8, encoded-string, or Buffer directory path after asynchronous creation, or rejects with the bounded mkdtemp error contract |
...
| `tsc_fs_promises_mkdir_async(path, mode)` | `tsc_promise_t*` | Libuv-backed nonrecursive `fs.promises.mkdir(path[, mode])`; fulfills after asynchronous directory creation or rejects with the bounded mkdir error contract |
| `tsc_fs_promises_mkdir_recursive_async(path, mode)` | `tsc_promise_t*` | Libuv-backed recursive `fs.promises.mkdir(path, { recursive: true, mode })`; walks path prefixes with sequential `uv_fs_mkdir` requests, accepts existing components, and rejects with the bounded recursive mkdir error contract |
...
| `tsc_fs_promises_rmdir_async(path)` | `tsc_promise_t*` | Libuv-backed nonrecursive `fs.promises.rmdir(path)`; fulfills after asynchronous empty-directory removal or rejects with the bounded rmdir error contract |
| `tsc_fs_promises_rmdir_recursive_async(path)` | `tsc_promise_t*` | Libuv-backed recursive `fs.promises.rmdir(path, { recursive: true })`; walks directories with sequential lstat/scandir/unlink/rmdir requests, rejects a non-directory root, or rejects with the bounded rmdir error contract |
| `tsc_fs_promises_rm_async(path, force)` | `tsc_promise_t*` | Libuv-backed nonrecursive `fs.promises.rm(path[, { force }])`; fulfills after asynchronous file/symlink removal, resolves a missing path when `force` is true, or rejects with the bounded rm error contract |
| `tsc_fs_promises_rm_recursive_async(path, force)` | `tsc_promise_t*` | Libuv-backed recursive `fs.promises.rm(path[, { recursive, force }])`; walks directories with sequential lstat/scandir/unlink/rmdir requests, resolves missing paths when `force` is true, or rejects with the bounded rm error contract |
...
| `tsc_fs_readdir_dirents_sync(path)` | `tsc_array_t*` | `fs.readdirSync(path, { withFileTypes: true })` — array of bounded `Dirent` records |
...
`fs.promises.readFile` uses a libuv-backed open/read/close chain for default UTF-8, hex/base64, and explicit Buffer/null results; encoded strings are produced after the asynchronous byte read completes. `fs.promises.writeFile` and `appendFile` use libuv-backed open/write/close chains for the supported string/Buffer data, encoding, flag, exclusive, update, and mode combinations. Non-recursive and recursive UTF-8/Buffer, hex/base64, and `withFileTypes` `fs.promises.readdir` calls use libuv-backed scanners. Default/mode `fs.promises.access` calls, `fs.promises.stat` / `lstat` / `statfs`, `fs.promises.realpath` / `readlink` / `mkdtemp`, `fs.promises.symlink` / `link`, `fs.promises.truncate`, nonrecursive and recursive `fs.promises.rm` / `rmdir`, `fs.promises.utimes` / `lutimes` / `chmod` / `chown` / `lchown`, nonrecursive and recursive `fs.promises.mkdir`, `fs.promises.unlink`, `fs.promises.copyFile` / `rename`, and all supported recursive/nonrecursive `fs.promises.cp` option forms use libuv-backed requests or state machines; recursive `readdir` walks directory entries sequentially without following symlink entries, recursive `mkdir` walks path prefixes sequentially, recursive `cp` walks directories sequentially, copies regular files with `uv_fs_copyfile`, and recreates symlinks through asynchronous link-target requests. Recursive `rmdir` rejects non-directory roots. Stat/lstat, the libuv Buffer-returning read, and the libuv string/Dirent-returning readdir paths use typed promise side-channel records. Real AbortSignal cancellation and broader libuv-backed filesystem I/O remain deferred.
