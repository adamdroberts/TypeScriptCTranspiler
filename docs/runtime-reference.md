...
`fs.promises.readFile` uses a libuv-backed open/read/close chain for default UTF-8 and explicit Buffer/null results; hex/base64 forms remain immediate wrappers over the sync runtime. `fs.promises.writeFile` and `appendFile` use libuv-backed open/write/close chains for the supported string/Buffer data, encoding, flag, exclusive, update, and mode combinations. Simple non-recursive `fs.promises.readdir` calls use a libuv-backed scanner for UTF-8 string and Buffer filename results; recursive, `withFileTypes`, and hex/base64 forms remain immediate wrappers over the sync runtime. Default/mode `fs.promises.access` calls and `fs.promises.stat` / `lstat` / `statfs` calls use libuv-backed requests; `realpath`, `readlink`, `symlink`, `link`, `mkdtemp`, `truncate`, `utimes`, `lutimes`, `chown`, `lchown`, `chmod`, `mkdir`, `unlink`, `rm`, `rmdir`, `cp`, `copyFile`, and `rename` remain emitter-level wrappers over sync runtime calls plus immediate Promise records. Stat/lstat and the libuv Buffer-returning read use typed promise side-channel records. Real AbortSignal cancellation and broader libuv-backed filesystem I/O remain deferred.
