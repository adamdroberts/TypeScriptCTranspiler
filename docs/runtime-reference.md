...
| `tsc_fs_opendir_sync(path)` | `tsc_value_t` | `fs.opendirSync(path)` — bounded POSIX directory handle with sync/Promise reads, async-iterator methods, and idempotent sync/async disposal |
