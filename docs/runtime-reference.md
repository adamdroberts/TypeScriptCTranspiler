...
| `tsc_http_create_server(request_listener)` | `tsc_value_t` | `http.createServer(requestListener?)` — creates an HTTP/1.1 server over the native TCP runtime, parsing one bounded request including chunked bodies and exposing request metadata plus response status/header/body methods; keep-alive, streaming, `https`, and `http2` remain deferred |
