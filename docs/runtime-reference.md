...
| `tsc_fs_rmdir_sync_opts(path, recursive)` | `void` | `fs.rmdirSync(path, { recursive })` and the recursive immediate-settled `fs.promises.rmdir(path, { recursive: true })` bounded options subset |
