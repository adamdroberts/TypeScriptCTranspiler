...
| `tsc_net_connect(port, host, connect_listener)` | `tsc_value_t` | `net.connect` / `net.createConnection` — creates a nonblocking POSIX IPv4 socket, emits `connect` / `data` / `end` / `close` / `error`, and exposes UTF-8, string/Buffer write, half-close, destroy, endpoint metadata, byte counters, and `setNoDelay` / `setKeepAlive` controls |
