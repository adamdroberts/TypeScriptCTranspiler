...
| `tsc_fs_promises_write_file_string_async(path, data, append, exclusive, update, mode, signal)` | `tsc_promise_t*` | Libuv-backed `fs.promises.writeFile` / `appendFile` for UTF-8 string data and the supported flag/mode combinations, with in-flight AbortSignal rejection/cancellation |
| `tsc_fs_promises_write_file_buffer_async(path, data, append, exclusive, update, mode, signal)` | `tsc_promise_t*` | Libuv-backed `fs.promises.writeFile` / `appendFile` for Buffer data, including emitter-decoded hex/base64 strings, with in-flight AbortSignal rejection/cancellation |
