...
| `tsc_fs_promises_cp_async(src, dest, recursive, force, errorOnExist, dereference, verbatimSymlinks, mode, preserveTimestamps)` | `tsc_promise_t*` | Libuv-preflighted `fs.promises.cp(src, dest, options)` for the bounded nonrecursive, regular-file-compatible subset; regular files use `uv_fs_copyfile`, while non-regular sources fall back to the existing copy helper before Promise settlement |
...
| `tsc_fs_rm_sync_opts(path, recursive, force)` | `void` | `fs.rmSync(path, { recursive, force })` and the recursive immediate-settled `fs.promises.rm(path, { recursive: true, force })` bounded options subset |
