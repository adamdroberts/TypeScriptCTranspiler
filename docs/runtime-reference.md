...
| `tsc_fs_promises_cp_async(src, dest, recursive, force, errorOnExist, dereference, verbatimSymlinks, mode, preserveTimestamps)` | `tsc_promise_t*` | Libuv-preflighted nonrecursive `fs.promises.cp(src, dest, options)`; regular files use `uv_fs_copyfile` with destination-existence and optional timestamp requests, while non-regular sources fall back to the existing copy helper before Promise settlement |
...
| `tsc_fs_mkdir_sync_opts(path, recursive, mode)` | `void` | `fs.mkdirSync(path, mode \| { recursive, mode })` and the recursive immediate-settled `fs.promises.mkdir(path, { recursive: true, mode })` bounded options subset |
...
| `tsc_fs_cp_sync_opts(src, dest, recursive, force, errorOnExist, dereference, verbatimSymlinks, mode, preserveTimestamps)` | `void` | `fs.cpSync(src, dest, { recursive, force, errorOnExist, dereference, verbatimSymlinks, mode, preserveTimestamps })` and the remaining immediate-settled `fs.promises.cp(src, dest, options)` option forms |
...
`fs.promises.readFile` uses a libuv-backed open/read/close chain for default UTF-8 and explicit Buffer/null results; hex/base64 forms remain immediate wrappers over the sync runtime. `fs.promises.writeFile` and `appendFile` use libuv-backed open/write/close chains for the supported string/Buffer data, encoding, flag, exclusive, update, and mode combinations. Simple non-recursive `fs.promises.readdir` calls use a libuv-backed scanner for UTF-8 string and Buffer filename results; recursive, `withFileTypes`, and hex/base64 forms remain immediate wrappers over the sync runtime. Default/mode `fs.promises.access` calls, `fs.promises.stat` / `lstat` / `statfs`, `fs.promises.realpath` / `readlink` / `mkdtemp`, `fs.promises.symlink` / `link`, `fs.promises.truncate`, nonrecursive and recursive `fs.promises.rm` / `rmdir`, `fs.promises.utimes` / `lutimes` / `chmod` / `chown` / `lchown`, nonrecursive `fs.promises.mkdir` / `unlink`, `fs.promises.copyFile` / `rename`, and all supported nonrecursive `fs.promises.cp` option forms use libuv-backed requests or state machines; recursive `rmdir` rejects non-directory roots, while regular-file cp's `uv_fs_copyfile` / destination-existence / timestamp path and symlink/directory cp fallbacks remain sync-backed before Promise settlement. Stat/lstat and the libuv Buffer-returning read use typed promise side-channel records. Real AbortSignal cancellation and broader libuv-backed filesystem I/O remain deferred.
