...
| Symbol | Signature | JS equivalent |
|--------|-----------|---------------|
| `tsc_http_request(options, response_listener)` | `tsc_value_t` | `http.request(options, callback?)` — opens or reuses an endpoint-keyed native TCP connection, accepts bounded `hostname` / `host` / `port` / `path` / `method` / `headers` / `auth` / `signal` options, synthesizes Basic authorization unless an explicit authorization header is present, exposes case-insensitive `ClientRequest` header mutation/introspection before headers are sent plus synchronized `headersSent` and `aborted`, supports validated `addTrailers(...)` for chunked requests, chainable `ClientRequest.setTimeout(timeout, callback?)`, `setNoDelay(noDelay?)`, `setSocketKeepAlive(enable?, initialDelay?)`, and `abort()` forwarding native socket controls/cancellation, buffers ordinary bounded string/Buffer request writes until `end`, preserves individual writes as framed chunks when `Transfer-Encoding: chunked` is selected, propagates bounded 16 KiB `ClientRequest` writable state and `drain` for queued chunked writes, and parses one HTTP/1.1 response with status/version/headers/body/trailers metadata including decoded chunks; pre-aborted and in-flight signals emit the signal reason once as a request error, explicit chunked responses also expose `data` / `end` events; sequential pooling covers ordinary and framed chunked requests/responses, while concurrent pooling, pipelining, broader streaming/backpressure, and `http2` remain deferred |
