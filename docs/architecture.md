...
Static literal `require(...)` calls are collected throughout a source file, including function bodies and local `require` / `module.require` aliases, so function-scoped bindings can resolve to package-source exports. Dependencies still execute eagerly through `mod_init`; this is not Node's lazy require timing.
