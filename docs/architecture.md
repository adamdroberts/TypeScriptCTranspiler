...
| Module requests | `src/module-request.ts` | derives static requests and finite dynamic-request worklists, normalizes sorted import attributes, proves request equality/deduplication, and enforces the resource-type policy |
| Module graph | `src/resolve.ts` | walks `ts.Program.getSourceFiles()`, resolves canonical static and dynamic Module Requests via `ts.resolveModuleName`, and separates startup from lazy edges |
...
Dynamic `import()` uses the same canonical ModuleRequest identity. A source-tree worklist derives a finite set of statically provable specifier alternatives and static import attributes; resolution includes those targets in the native program without adding them to the startup dependency order. The emitted import job evaluates the specifier once, queues module loading as a microtask, walks the selected target's static dependency graph in dependency-first order, and resolves to the module's stable namespace object. Every `mod_init` is guarded by one unstarted/evaluating/evaluated/failed state machine, so repeated requests share evaluation and failures. Runtime-shaped options/attribute enumeration and specifiers without a finite AOT proof fail closed rather than guessing a host load.
