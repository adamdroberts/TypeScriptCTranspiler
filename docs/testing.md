...
- **Inline-cache baselines** — `TSC_DYNAMIC_STATS=1` makes compiled binaries print dynamic property-operation counters, object shape-update counters, and dynamic property cache hit/miss counters to stderr at process exit. `dynamic_runtime_stats` covers the opt-in path plus repeated cached literal property and string-key element reads, but broader receiver-aware read/write cache sites remain Phase 15 work.
