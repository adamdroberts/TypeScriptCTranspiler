...
- **Inline-cache baselines** — `TSC_DYNAMIC_STATS=1` makes compiled binaries print dynamic property-operation counters, object shape-update counters, and literal dynamic property cache hit/miss counters to stderr at process exit. `dynamic_runtime_stats` covers the opt-in path and repeated cached reads, but broader generated-code cache sites remain Phase 15 work.
