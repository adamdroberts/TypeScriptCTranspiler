...
| `--dynamic-require-manifest <path>` | — | JSON allow list for non-finite `require(variable)` sites. Shape: `{ "requires": ["./specifier"] }`, `{ "requires": { "name": "./specifier" } }`, or caller-scoped maps such as `{ "requires": { "src/loader.ts": ["./specifier"] } }`. |
