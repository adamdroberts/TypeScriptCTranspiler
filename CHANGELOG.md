...
### Added
- Standard class decorators now evaluate during module initialization for named classes, with direct decorator functions receiving a dynamic context object containing `kind: "class"` and `name`; class replacement, metadata, and `addInitializer` remain deferred. Test: `class_decorator_basic`.
