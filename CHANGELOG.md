...
- `Error`, `TypeError`, `RangeError`, `SyntaxError`, and `AggregateError` now have a narrow object subset for constructor/callable forms, exposing `.name`, `.message`, `.toString()`, `.toLocaleString()`, and `.valueOf()`. `AggregateError` stores `.errors`, while `throw` still stringifies errors to the existing exception channel. Tests: `error_instances`, `error_constructors`, `aggregate_error_constructor`.
