...
- `fs.mkdtempSync` and immediate `fs.promises.mkdtemp` now accept literal UTF-8 encoding options. Test: `fs_mkdtemp_encoding_options`.
...
- `fs.realpathSync` / `fs.readlinkSync` and immediate `fs.promises.realpath` / `readlink` now accept literal UTF-8 encoding options. Test: `fs_link_path_encoding_options`.
...
- `process.nextTick(callback)` now supports a bounded zero-argument callback queue drained before program exit. Test: `process_next_tick`.
