...
`strict-equality.property.test.ts` derives comparisons from one canonical collection of ECMAScript language Types and independently applies the Strict Equality algorithm. The generated program verifies every represented Type pairing, identity/value partitions, `NaN`, signed zero, negation, and left-to-right operand evaluation in both runtime memory modes; individual pair counts are not completion evidence.

`html-dda.property.test.ts` derives the Test262 host-global and normative Annex B `[[IsHTMLDDA]]` behavior from one semantic-probe collection. It independently checks `$262`/`print` global identity and descriptors plus the callable exotic object's `typeof`, truthiness, abstract/strict/SameValue equality, nullish behavior, and required call results in both runtime memory modes; the probe inventory is a partition specification, not a coverage denominator.
