...
function subjectSource(): string {
    return `
        ...
        var intrinsic = String.prototype.substring;
        var descriptor = Object.getOwnPropertyDescriptor(String.prototype, "substring");
        ...
        check(typeof intrinsic === "function" && intrinsic === String.prototype.substring,
        ...
        check(intrinsic.name === "substring" && !nameDescriptor.writable && !nameDescriptor.enumerable &&
        ...
        check(intrinsic.call(ordinaryReceiver, 2, 6) === "dina" && receiverCalls === 1,
            "ordinary-receiver-once");
        Number.prototype.substring = intrinsic;
        var numericReceiver = 11.001002;
        check(numericReceiver.substring() === "11.001002", "number-prototype-borrowed-direct-call");
        delete Number.prototype.substring;
        Boolean.prototype.substring = intrinsic;
        var booleanReceiver = true;
        check(booleanReceiver.substring(3, 1) === "ru", "boolean-prototype-borrowed-direct-call");
        delete Boolean.prototype.substring;
        ...
        for (var rangeIndex = 0; rangeIndex < ranges.length; rangeIndex++) {
            ...
            check(intrinsic.call("012345", range[0], range[1]) === range[2],
                "clamped-swapped-range-" + rangeIndex);
        }
        ...
        check(intrinsic.call(utf16, 1, 3) === face, "utf16-scalar-range");
        check(intrinsic.call(utf16, 4, 5).charCodeAt(0) === 0xd800,
        ...
        check("abc".substring((argumentEvaluated = true, 1), 2) === "abc:1:2" && argumentEvaluated,
        ...
        check("abc".substring((argumentEvaluated = true, 1)) === "getter-selected" && argumentEvaluated,
        ...
        check("abc".substring(1) === "bc" && String.prototype.substring === intrinsic,
            "surface-restored");

        print(failures.length === 0 ? "string-substring-ok" : failures.join(","));
    `;
}

test("String.prototype.substring follows one generic UTF-16 clamped-range algorithm", async () => {
    const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "tsc2c-string-substring-property-"));
    const entry = path.join(temporary, "subject.js");
    const scenarioId = "property/string-substring.js#sloppy";
    try {
