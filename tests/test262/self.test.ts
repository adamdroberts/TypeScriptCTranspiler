...
describe("host result contract", () => {
    ...
    test("prepares separate global Scripts as one runner-owned native observation", async () => {
        ...
        const testSource = [
            ...
            "if (globalThis.HarnessError !== HarnessError || globalThis.setupGlobal !== 20) throw new TypeError('bad setup globals');",
            "var reflected = 1;",
            "if (globalThis.reflected !== 1) throw new TypeError('bad identifier-to-property reflection');",
            "globalThis.reflected = 2;",
            "if (reflected !== 2) throw new TypeError('bad property-to-identifier reflection');",
            "reflected += 3;",
            "if (globalThis.reflected !== 5 || reflected++ !== 5 || reflected !== 6) throw new TypeError('bad global updates');",
            "if (true) { var nestedGlobal = 7; }",
            "if (globalThis.nestedGlobal !== 7) throw new TypeError('bad nested var hoist');",
            "let lexicalOnly = 8;",
            "if ('lexicalOnly' in globalThis) throw new TypeError('lexical binding leaked');",
            "var reflectedDescriptor = Object.getOwnPropertyDescriptor(globalThis, 'reflected');",
            "if (!reflectedDescriptor || !reflectedDescriptor.writable || !reflectedDescriptor.enumerable || reflectedDescriptor.configurable) throw new TypeError('bad global var descriptor');",
            "if (delete globalThis.reflected) throw new TypeError('global var was configurable');",
            "function sloppyReceiver() { return this; }",
            "function strictReceiver() { 'use strict'; return this; }",
            "if (sloppyReceiver() !== globalThis || strictReceiver() !== undefined) throw new TypeError('bad function this normalization');",
            ...
            "if (!abstractHookDescriptor || abstractHookDescriptor.value !== abstractModuleSource || !abstractHookDescriptor.writable || abstractHookDescriptor.enumerable || !abstractHookDescriptor.configurable) throw new TypeError('bad AbstractModuleSource hook descriptor');",
            "if (typeof abstractModuleSource !== 'function' || abstractModuleSource.length !== 0 || abstractModuleSource.name !== 'AbstractModuleSource') throw new TypeError('bad AbstractModuleSource metadata');",
            "if (Object.getPrototypeOf(abstractModuleSource) !== Function.prototype || 'AbstractModuleSource' in globalThis) throw new TypeError('bad AbstractModuleSource intrinsic placement');",
            "var abstractPrototypeDescriptor = Object.getOwnPropertyDescriptor(abstractModuleSource, 'prototype');",
            "if (!abstractPrototypeDescriptor || abstractPrototypeDescriptor.value !== abstractModuleSource.prototype || abstractPrototypeDescriptor.writable || abstractPrototypeDescriptor.enumerable || abstractPrototypeDescriptor.configurable) throw new TypeError('bad AbstractModuleSource prototype descriptor');",
            "if (Object.getPrototypeOf(abstractModuleSource.prototype) !== Object.prototype) throw new TypeError('bad AbstractModuleSource prototype chain');",
            "var abstractConstructorDescriptor = Object.getOwnPropertyDescriptor(abstractModuleSource.prototype, 'constructor');",
            "if (!abstractConstructorDescriptor || abstractConstructorDescriptor.value !== abstractModuleSource || !abstractConstructorDescriptor.writable || abstractConstructorDescriptor.enumerable || !abstractConstructorDescriptor.configurable) throw new TypeError('bad AbstractModuleSource constructor descriptor');",
            "var abstractTagDescriptor = Object.getOwnPropertyDescriptor(abstractModuleSource.prototype, Symbol.toStringTag);",
            "if (!abstractTagDescriptor || typeof abstractTagDescriptor.get !== 'function' || abstractTagDescriptor.get.name !== 'get [Symbol.toStringTag]' || abstractTagDescriptor.set !== undefined || abstractTagDescriptor.enumerable || !abstractTagDescriptor.configurable) throw new TypeError('bad AbstractModuleSource toStringTag descriptor');",
            "if (abstractTagDescriptor.get.call(262) !== undefined || abstractTagDescriptor.get.call(abstractModuleSource.prototype) !== undefined) throw new TypeError('bad AbstractModuleSource toStringTag brand handling');",
            ...
            "if (!abstractCallThrows || !abstractConstructThrows) throw new TypeError('AbstractModuleSource did not throw TypeError');",
            "var detach = $262.detachArrayBuffer;",
            "if (typeof detach !== 'function' || detach.length !== 1 || detach.name !== 'detachArrayBuffer') throw new TypeError('bad detach hook');",
            "var detachDescriptor = Object.getOwnPropertyDescriptor($262, 'detachArrayBuffer');",
            "if (!detachDescriptor || detachDescriptor.value !== detach || !detachDescriptor.writable || detachDescriptor.enumerable || !detachDescriptor.configurable) throw new TypeError('bad detach hook descriptor');",
            "var storage = new ArrayBuffer(8);",
            "var detachedView = new DataView(storage, 2, 4);",
            "detach(storage);",
            "if (storage.byteLength !== 0 || detachedView.buffer !== storage) throw new TypeError('bad detached buffer state');",
            "var detachedLengthThrows = false;",
            "try { detachedView.byteLength; } catch (error) { detachedLengthThrows = error instanceof TypeError; }",
            "if (!detachedLengthThrows) throw new TypeError('detached DataView length did not throw TypeError');",
            "var detachedOffsetThrows = false;",
            "try { detachedView.byteOffset; } catch (error) { detachedOffsetThrows = error instanceof TypeError; }",
            "if (!detachedOffsetThrows) throw new TypeError('detached DataView offset did not throw TypeError');",
