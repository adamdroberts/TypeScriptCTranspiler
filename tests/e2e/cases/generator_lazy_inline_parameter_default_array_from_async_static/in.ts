function* values(): Generator<number, number, number> {
    return (yield 1) + ((items: Promise<number[]> = Array.fromAsync([1, 2]), valid: boolean = items === items) => valid ? 1 : 0)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((items: Promise<number[]> = Array.fromAsync([1, 2]), valid: boolean = items === items) => (valid ? 1 : 0) + offset)() + (yield 2);
    };
}
