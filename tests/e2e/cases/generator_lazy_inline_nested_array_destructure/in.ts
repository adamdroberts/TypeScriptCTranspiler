function* values(): Generator<number, number, number> {
    return (yield 1) + (([[first, second], [third, fourth]]: number[][]) => first + second + third + fourth)([[7, 8], [9, 10]]) + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + (([[first, second], [third, fourth]]: number[][]) => first + second + third + fourth + offset)([[7, 8], [9, 10]]) + (yield 2);
    };
}
