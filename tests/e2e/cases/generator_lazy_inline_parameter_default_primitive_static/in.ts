function* values(): Generator<number, number, number> {
    return (yield 1) + ((value: number = 7, text: string = String(value), numeric: number = Number(text), truthy: boolean = Boolean(numeric)) => text === "7" && numeric === 7 && truthy ? 1 : 0)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((value: number = 7, text: string = String(value), numeric: number = Number(text), truthy: boolean = Boolean(numeric)) => (text === "7" && numeric === 7 && truthy ? 1 : 0) + offset)() + (yield 2);
    };
}
