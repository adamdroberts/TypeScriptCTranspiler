function* values(): Generator<number, number, number> {
    return (yield 1) + ((record: any = Promise.withResolvers<number>(), valid: boolean = record.promise === record.promise) => valid ? 1 : 0)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((record: any = Promise.withResolvers<number>(), valid: boolean = record.promise === record.promise) => (valid ? 1 : 0) + offset)() + (yield 2);
    };
}
