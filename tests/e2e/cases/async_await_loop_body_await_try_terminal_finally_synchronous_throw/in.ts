...
async function run(sourceReject: boolean, cleanupReject: boolean): Promise<string> {
    let count = 0;
...
report("success", run(false, false))
    .then((_value) => report("source-reject", run(true, false)))
    .then((_value) => report("cleanup-reject", run(false, true)));
