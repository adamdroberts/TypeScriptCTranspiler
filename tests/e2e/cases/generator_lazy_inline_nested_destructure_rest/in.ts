function* values(): Generator<number, number, number> {
    return (yield 1) + (({ outer: { first, ...outerRest }, tail: [second, ...tailRest] }: { outer: { first: number; second: number }; tail: number[] }) => first + outerRest.second + second + tailRest[0])({ outer: { first: 7, second: 8 }, tail: [9, 10] }) + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + (({ outer: { first, ...outerRest }, tail: [second, ...tailRest] }: { outer: { first: number; second: number }; tail: number[] }) => first + outerRest.second + second + tailRest[0] + offset)({ outer: { first: 7, second: 8 }, tail: [9, 10] }) + (yield 2);
    };
}
