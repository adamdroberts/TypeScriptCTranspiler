let pushArraySpread: any = ["push-before-spread"];
let pushArrayTail: any = 2;
let unshiftObjectSpread: any = { base: "unshift-before-base" };
let unshiftObjectValue: any = "unshift-before-value";
let unshiftObjectCount: any = 1;

function* multiYieldPushLiteralSpreadArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push([...pushArraySpread, pushArrayTail], yield "push-item", ...(yield "push-spread"))[yield "push-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
function* multiYieldUnshiftLiteralSpreadArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift({ ...unshiftObjectSpread, value: unshiftObjectValue, count: unshiftObjectCount }, yield "unshift-item", ...(yield "unshift-spread"))[yield "unshift-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const pushIterator = multiYieldPushLiteralSpreadArgumentAccessDeclarationInitializer();
...
pushArraySpread = ["push-after-spread-one", "push-after-spread-two"];
pushArrayTail = 8;
...
console.log("push-literal", pushReceiver[1].join("|"));

const unshiftReceiver: any = ["unshift-first"];
const unshiftSpread: any = ["unshift-spread-one", "unshift-spread-two"];
const unshiftIterator = multiYieldUnshiftLiteralSpreadArgumentAccessDeclarationInitializer();
...
unshiftObjectSpread = { base: "unshift-after-base" };
unshiftObjectValue = "unshift-after-value";
unshiftObjectCount = 9;
...
console.log("unshift-literal", unshiftReceiver[0].base, unshiftReceiver[0].value, unshiftReceiver[0].count);
