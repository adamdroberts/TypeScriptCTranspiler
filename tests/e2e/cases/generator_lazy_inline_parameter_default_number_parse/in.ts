function* values(): Generator<number, number, number> {
    return (yield 1) + ((raw: string = "42.5", integer: number = Number.parseInt(raw, 10), decimal: number = Number.parseFloat(raw)) => integer + decimal)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((raw: string = "42.5", integer: number = Number.parseInt(raw, 10), decimal: number = Number.parseFloat(raw)) => integer + decimal + offset)() + (yield 2);
    };
}
