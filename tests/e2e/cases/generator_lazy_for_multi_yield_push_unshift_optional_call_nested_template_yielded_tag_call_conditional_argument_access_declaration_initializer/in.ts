...
function pushConditionalTag(strings: TemplateStringsArray, value: any): any {
    return "push-conditional-" + strings[0] + value + strings[1];
}

function unshiftConditionalTag(strings: TemplateStringsArray, value: any): any {
    return "unshift-conditional-" + strings[0] + value + strings[1];
}

function getPushConditionalTag(seed: any): any {
    pushTagEvents.push("tag-" + seed);
    return pushConditionalTag;
}

function getUnshiftConditionalTag(seed: any): any {
    unshiftTagEvents.push("tag-" + seed);
    return unshiftConditionalTag;
}

function pushConditionalTagCallback(value: any): any {
    return "push-tagged-" + value;
}

function unshiftConditionalTagCallback(value: any): any {
    return "unshift-tagged-" + value;
}

function* multiYieldPushConditionalTagArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushConditionalTagCallback?.(`push-${getPushConditionalTag((yield "push-selector") ? (yield "push-selected") : "push-fallback")`inner-${yield "push-inner"}`}-${yield "push-outer"}`),
            yield "push-item"
        )[yield "push-key"],
        count = 0;
        count < 1;
...
function* multiYieldUnshiftConditionalTagArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftConditionalTagCallback?.(String.raw`unshift-${getUnshiftConditionalTag((yield "unshift-selector") ? (yield "unshift-selected") : "unshift-fallback")`inner-${yield "unshift-inner"}`}-${yield "unshift-outer"}`),
            yield "unshift-item"
        )[yield "unshift-key"],
        count = 0;
        count < 1;
...
const pushIterator = multiYieldPushConditionalTagArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftConditionalTagArgumentAccessDeclarationInitializer();
