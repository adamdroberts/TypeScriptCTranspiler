...
async function sixAwait(prefix: string): Promise<string> {
    const first = await delay(306, "one");
    const second = await delay(307, prefix + first + "-two");
    const third = await delay(308, first + ":" + second + "-three");
    const fourth = await delay(309, first + ":" + second + ":" + third + "-four");
...
async function deeplyNestedConditionalLeadingChain(kind: number, prefix: string): Promise<string> {
    let first = prefix + "seed";
    if (kind === 0) {
        first = await delay(1410, prefix + "zero-one");
    } else if (kind === 1) {
        first = await delay(1411, prefix + "one-one");
    } else if (kind === 2) {
        first = await delay(1412, prefix + "two-one");
    } else {
        first = await delay(1413, prefix + "other-one");
    }
    const second = await delay(1414, first + ":two");
    return first + ":" + second;
}

class DeeplyNestedConditionalLeadingChainMethod {
    private readonly prefix: string;
    ...
    async run(kind: number): Promise<string> {
        let first = this.prefix + "seed";
        if (kind === 0) {
            first = await delay(1415, this.prefix + "zero-one");
        } else if (kind === 1) {
            first = await delay(1416, this.prefix + "one-one");
        } else if (kind === 2) {
            first = await delay(1417, this.prefix + "two-one");
        } else {
            first = await delay(1418, this.prefix + "other-one");
        }
        const second = await delay(1419, first + ":two");
        return this.prefix + first + ":" + second;
    }
}

const deeplyNestedConditionalLeadingChainValue = async function(kind: number, prefix: string): Promise<string> {
    let first = prefix + "seed";
    if (kind === 0) {
        first = await delay(1420, prefix + "zero-one");
    } else if (kind === 1) {
        first = await delay(1421, prefix + "one-one");
    } else if (kind === 2) {
        first = await delay(1422, prefix + "two-one");
    } else {
        first = await delay(1423, prefix + "other-one");
    }
    const second = await delay(1424, first + ":two");
    return prefix + first + ":" + second;
};
...
async function fiveLeafConditionalLeadingChain(kind: number, prefix: string): Promise<string> {
    let first = prefix + "seed";
    let marker = "initial";
    if (kind === 0) {
        marker = marker + "|zero-pre";
        first = await delay(1490, prefix + "zero-one");
        marker = marker + "|zero-post";
    } else if (kind === 1) {
        marker = marker + "|one-pre";
        first = await delay(1491, prefix + "one-one");
        marker = marker + "|one-post";
    } else if (kind === 2) {
        marker = marker + "|two-pre";
        first = await delay(1492, prefix + "two-one");
        marker = marker + "|two-post";
    } else if (kind === 3) {
        marker = marker + "|three-pre";
        first = await delay(1493, prefix + "three-one");
        marker = marker + "|three-post";
    } else {
        marker = marker + "|other-pre";
        first = await delay(1494, prefix + "other-one");
        marker = marker + "|other-post";
    }
    const second = await delay(1495, first + ":" + marker);
    return first + ":" + second + ":" + marker;
}

class FiveLeafConditionalLeadingChainMethod {
    private readonly prefix: string;
    ...
    async run(kind: number): Promise<string> {
        ...
        if (kind === 0) {
            marker = marker + "|zero-pre";
            first = await delay(1496, this.prefix + "zero-one");
            marker = marker + "|zero-post";
        } else if (kind === 1) {
            marker = marker + "|one-pre";
            first = await delay(1497, this.prefix + "one-one");
            marker = marker + "|one-post";
        } else if (kind === 2) {
            marker = marker + "|two-pre";
            first = await delay(1498, this.prefix + "two-one");
            marker = marker + "|two-post";
        } else if (kind === 3) {
            marker = marker + "|three-pre";
            first = await delay(1499, this.prefix + "three-one");
            marker = marker + "|three-post";
        } else {
            marker = marker + "|other-pre";
            first = await delay(1500, this.prefix + "other-one");
            marker = marker + "|other-post";
        }
        const second = await delay(1501, first + ":" + marker);
        return this.prefix + first + ":" + second + ":" + marker;
    }
}

const fiveLeafConditionalLeadingChainValue = async function(kind: number, prefix: string): Promise<string> {
    let first = prefix + "seed";
    let marker = "initial";
    if (kind === 0) {
        marker = marker + "|zero-pre";
        first = await delay(1502, prefix + "zero-one");
        marker = marker + "|zero-post";
    } else if (kind === 1) {
        marker = marker + "|one-pre";
        first = await delay(1503, prefix + "one-one");
        marker = marker + "|one-post";
    } else if (kind === 2) {
        marker = marker + "|two-pre";
        first = await delay(1504, prefix + "two-one");
        marker = marker + "|two-post";
    } else if (kind === 3) {
        marker = marker + "|three-pre";
        first = await delay(1505, prefix + "three-one");
        marker = marker + "|three-post";
    } else {
        marker = marker + "|other-pre";
        first = await delay(1506, prefix + "other-one");
        marker = marker + "|other-post";
    }
    const second = await delay(1507, first + ":" + marker);
    return prefix + first + ":" + second + ":" + marker;
};
...
class BranchSixLeafConditionalLeadingChainMethod {
    private readonly prefix: string;
    ...
    async run(outer: boolean, kind: number): Promise<string> {
        if (outer) {
            ...
            if (kind === 0) {
                marker = marker + "|zero-pre";
                first = await delay(1550, this.prefix + "zero-one");
                marker = marker + "|zero-post";
            } else if (kind === 1) {
                marker = marker + "|one-pre";
                first = await delay(1551, this.prefix + "one-one");
                marker = marker + "|one-post";
            } else if (kind === 2) {
                marker = marker + "|two-pre";
                first = await delay(1552, this.prefix + "two-one");
                marker = marker + "|two-post";
            } else if (kind === 3) {
                marker = marker + "|three-pre";
                first = await delay(1553, this.prefix + "three-one");
                marker = marker + "|three-post";
            } else if (kind === 4) {
                marker = marker + "|four-pre";
                first = await delay(1554, this.prefix + "four-one");
                marker = marker + "|four-post";
            } else {
                marker = marker + "|other-pre";
                first = await delay(1555, this.prefix + "other-one");
                marker = marker + "|other-post";
            }
            const second = await delay(1556, first + ":" + marker);
            return this.prefix + first + ":" + second + ":" + marker;
        }
        return this.prefix + "fallthrough";
    }
}

const branchSixLeafConditionalLeadingChainValue = async function(outer: boolean, kind: number, prefix: string): Promise<string> {
    if (outer) {
        let first = prefix + "seed";
        let marker = "initial";
        if (kind === 0) {
            marker = marker + "|zero-pre";
            first = await delay(1560, prefix + "zero-one");
            marker = marker + "|zero-post";
        } else if (kind === 1) {
            marker = marker + "|one-pre";
            first = await delay(1561, prefix + "one-one");
            marker = marker + "|one-post";
        } else if (kind === 2) {
            marker = marker + "|two-pre";
            first = await delay(1562, prefix + "two-one");
            marker = marker + "|two-post";
        } else if (kind === 3) {
            marker = marker + "|three-pre";
            first = await delay(1563, prefix + "three-one");
            marker = marker + "|three-post";
        } else if (kind === 4) {
            marker = marker + "|four-pre";
            first = await delay(1564, prefix + "four-one");
            marker = marker + "|four-post";
        } else {
            marker = marker + "|other-pre";
            first = await delay(1565, prefix + "other-one");
            marker = marker + "|other-post";
        }
        const second = await delay(1566, first + ":" + marker);
        return prefix + first + ":" + second + ":" + marker;
    }
    return prefix + "fallthrough";
};
...
async function loopControlDeclarationAfterAwait(prefix: string): Promise<string> {
    const value = await delay(342, "decl-loop");
    let result = prefix;
    let index = 0;
    while (index < 5) {
        if (index === 1) {
...
async function tryCatchDeclarationAfterAwait(prefix: string): Promise<string> {
    const value = await delay(344, "decl-try");
    let result = prefix;
    try {
        throw value + "!";
    } catch (e) {
...
class Worker {
    ...
    async sixAwaitMethod(prefix: string): Promise<string> {
        const first = await delay(312, "method-one");
        const second = await delay(313, prefix + this.prefix + first);
        const third = await delay(314, first + ":" + second + "-three");
        const fourth = await delay(315, first + ":" + second + ":" + third + "-four");
        const fifth = await delay(316, first + ":" + second + ":" + third + ":" + fourth + "-five");
        const sixth = await delay(317, first + ":" + second + ":" + third + ":" + fourth + ":" + fifth + "-six");
        return first + ":" + second + ":" + third + ":" + fourth + ":" + fifth + ":" + sixth + "!";
    }
...
const arrowExpressionBodyParenthesizedDirectAwait = async (prefix: string): Promise<string> =>
    (await delay(33, prefix + "arrow-expression-body-parenthesized-direct"));

const arrowExpressionBodyAssertedInlineAwaitReturn = async (prefix: string): Promise<string> =>
    (prefix + await delay(34, "arrow-expression-body-asserted-inline") + "!") as string;

function expressionBodyClosureDirectAwait(prefix: string): Promise<string> {
    const captured = prefix + "closure-";
    const run = async (suffix: string): Promise<string> =>
        await delay(29, captured + suffix);
    return run("direct");
}

function expressionBodyClosureInlineAwaitReturn(prefix: string): Promise<string> {
    const captured = prefix + "closure-";
    const run = async (suffix: string): Promise<string> =>
        captured + await delay(30, "inline") + suffix;
    return run("!");
}
...
function expressionBodyClosureParenthesizedDirectAwait(prefix: string): Promise<string> {
    const captured = prefix + "closure-transparent-";
    const run = async (suffix: string): Promise<string> =>
        (await delay(35, captured + suffix));
    return run("direct");
}

function expressionBodyClosureAssertedInlineAwaitReturn(prefix: string): Promise<string> {
    const captured = prefix + "closure-transparent-";
    const run = async (suffix: string): Promise<string> =>
        (captured + await delay(36, "inline") + suffix) as string;
    return run("!");
}
...
const arrowSixAwait = async (prefix: string): Promise<string> => {
    const first = await delay(318, "arrow-one");
    const second = await delay(319, prefix + first);
    const third = await delay(320, first + ":" + second + "-three");
    const fourth = await delay(321, first + ":" + second + ":" + third + "-four");
    const fifth = await delay(322, first + ":" + second + ":" + third + ":" + fourth + "-five");
    const sixth = await delay(323, first + ":" + second + ":" + third + ":" + fourth + ":" + fifth + "-six");
    return first + ":" + second + ":" + third + ":" + fourth + ":" + fifth + ":" + sixth + "!";
};
...
const arrowLoopControlAfterAwait = async (prefix: string): Promise<string> => {
    const value = await delay(345, "arrow-loop");
    let result = prefix;
    let index = 0;
    while (index < 5) {
        if (index === 1) {
...
deeplyNestedConditionalLeadingChain(0, "deep-").then((value: string): void => {
    console.log("deeply-nested-conditional-leading-chain-zero:", value);
});
deeplyNestedConditionalLeadingChain(1, "deep-").then((value: string): void => {
    console.log("deeply-nested-conditional-leading-chain-one:", value);
});
deeplyNestedConditionalLeadingChain(2, "deep-").then((value: string): void => {
    console.log("deeply-nested-conditional-leading-chain-two:", value);
});
deeplyNestedConditionalLeadingChain(3, "deep-").then((value: string): void => {
    console.log("deeply-nested-conditional-leading-chain-other:", value);
});
const deeplyNestedConditionalLeadingChainMethodInstance = new DeeplyNestedConditionalLeadingChainMethod("method-deep-");
deeplyNestedConditionalLeadingChainMethodInstance.run(0).then((value: string): void => {
    console.log("deeply-nested-conditional-leading-chain-method-zero:", value);
});
deeplyNestedConditionalLeadingChainMethodInstance.run(1).then((value: string): void => {
    console.log("deeply-nested-conditional-leading-chain-method-one:", value);
});
deeplyNestedConditionalLeadingChainMethodInstance.run(2).then((value: string): void => {
    console.log("deeply-nested-conditional-leading-chain-method-two:", value);
});
deeplyNestedConditionalLeadingChainMethodInstance.run(3).then((value: string): void => {
    console.log("deeply-nested-conditional-leading-chain-method-other:", value);
});
deeplyNestedConditionalLeadingChainValue(0, "value-deep-").then((value: string): void => {
    console.log("deeply-nested-conditional-leading-chain-value-zero:", value);
});
deeplyNestedConditionalLeadingChainValue(1, "value-deep-").then((value: string): void => {
    console.log("deeply-nested-conditional-leading-chain-value-one:", value);
});
deeplyNestedConditionalLeadingChainValue(2, "value-deep-").then((value: string): void => {
    console.log("deeply-nested-conditional-leading-chain-value-two:", value);
});
...
fiveLeafConditionalLeadingChain(0, "five-").then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-zero:", value);
});
fiveLeafConditionalLeadingChain(1, "five-").then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-one:", value);
});
fiveLeafConditionalLeadingChain(2, "five-").then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-two:", value);
});
fiveLeafConditionalLeadingChain(3, "five-").then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-three:", value);
});
fiveLeafConditionalLeadingChain(4, "five-").then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-other:", value);
});
const fiveLeafConditionalLeadingChainMethodInstance = new FiveLeafConditionalLeadingChainMethod("method-five-");
fiveLeafConditionalLeadingChainMethodInstance.run(0).then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-method-zero:", value);
});
fiveLeafConditionalLeadingChainMethodInstance.run(1).then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-method-one:", value);
});
fiveLeafConditionalLeadingChainMethodInstance.run(2).then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-method-two:", value);
});
fiveLeafConditionalLeadingChainMethodInstance.run(3).then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-method-three:", value);
});
fiveLeafConditionalLeadingChainMethodInstance.run(4).then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-method-other:", value);
});
fiveLeafConditionalLeadingChainValue(0, "value-five-").then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-value-zero:", value);
});
fiveLeafConditionalLeadingChainValue(1, "value-five-").then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-value-one:", value);
});
fiveLeafConditionalLeadingChainValue(2, "value-five-").then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-value-two:", value);
});
fiveLeafConditionalLeadingChainValue(3, "value-five-").then((value: string): void => {
    console.log("five-leaf-conditional-leading-chain-value-three:", value);
});
...
const branchSixLeafConditionalLeadingChainMethodInstance = new BranchSixLeafConditionalLeadingChainMethod("method-branch-six-");
branchSixLeafConditionalLeadingChainMethodInstance.run(true, 0).then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-method-zero:", value);
});
branchSixLeafConditionalLeadingChainMethodInstance.run(true, 1).then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-method-one:", value);
});
branchSixLeafConditionalLeadingChainMethodInstance.run(true, 2).then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-method-two:", value);
});
branchSixLeafConditionalLeadingChainMethodInstance.run(true, 3).then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-method-three:", value);
});
branchSixLeafConditionalLeadingChainMethodInstance.run(true, 4).then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-method-four:", value);
});
branchSixLeafConditionalLeadingChainMethodInstance.run(true, 5).then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-method-other:", value);
});
branchSixLeafConditionalLeadingChainMethodInstance.run(false, 0).then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-method-fallthrough:", value);
});
branchSixLeafConditionalLeadingChainValue(true, 0, "value-branch-six-").then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-value-zero:", value);
});
branchSixLeafConditionalLeadingChainValue(true, 1, "value-branch-six-").then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-value-one:", value);
});
branchSixLeafConditionalLeadingChainValue(true, 2, "value-branch-six-").then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-value-two:", value);
});
branchSixLeafConditionalLeadingChainValue(true, 3, "value-branch-six-").then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-value-three:", value);
});
branchSixLeafConditionalLeadingChainValue(true, 4, "value-branch-six-").then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-value-four:", value);
});
branchSixLeafConditionalLeadingChainValue(true, 5, "value-branch-six-").then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-value-other:", value);
});
branchSixLeafConditionalLeadingChainValue(false, 0, "value-branch-six-").then((value: string): void => {
    console.log("branch-six-leaf-conditional-leading-chain-value-fallthrough:", value);
});
...
loopControlDeclarationAfterAwait("fn-").then((value: string): void => {
...
tryCatchDeclarationAfterAwait("fn-try-").then((value: string): void => {
    console.log("declaration-try-catch-after-await:", value);
});
...
new Worker("method-six-").sixAwaitMethod("class-").then((value: string): void => {
...
arrowExpressionBodyParenthesizedDirectAwait("value-").then((value: string): void => {
    console.log("arrow-expression-body-parenthesized-direct-await:", value);
});

arrowExpressionBodyAssertedInlineAwaitReturn("value-").then((value: string): void => {
    console.log("arrow-expression-body-asserted-inline-await-return:", value);
});

expressionBodyClosureDirectAwait("value-").then((value: string): void => {
    console.log("closure-expression-body-direct-await:", value);
});
...
expressionBodyClosureParenthesizedDirectAwait("value-").then((value: string): void => {
    console.log("closure-expression-body-parenthesized-direct-await:", value);
});

expressionBodyClosureAssertedInlineAwaitReturn("value-").then((value: string): void => {
    console.log("closure-expression-body-asserted-inline-await-return:", value);
});
...
arrowLoopControlAfterAwait("value-").then((value: string): void => {
...
arrowTryCatchAfterAwait("value-try-").then((value: string): void => {
    console.log("arrow-try-catch-after-await:", value);
});
