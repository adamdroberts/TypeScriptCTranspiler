...
async function consume(iterator: any): Promise<string> {
    for await (const item of iterator) {
        ...
        await Promise.reject("twenty-nine-rejection");
        break;
    }
...
consume(iterator).then((value: string): void => {
    console.log("body-twenty-nine-awaits-rejected: unexpected", value);
}, (reason: any): void => {
    console.log("body-twenty-nine-awaits-rejected:", reason);
});
