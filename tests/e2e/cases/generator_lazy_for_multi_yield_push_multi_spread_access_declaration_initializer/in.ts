function* multiYieldPushMultiSpreadAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "receiver").push(...(yield "first-items"), ...(yield "second-items"))[yield "key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const iterator = multiYieldPushMultiSpreadAccessDeclarationInitializer();
