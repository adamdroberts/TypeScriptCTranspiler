function* values(): Generator<number, number, number> {
    return (yield 1) + ((raw: string = "{\"ok\":true}", parsed: any = JSON.parse(raw), valid: boolean = parsed.ok === true) => valid ? 1 : 0)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((raw: string = "{\"ok\":true}", parsed: any = JSON.parse(raw), valid: boolean = parsed.ok === true) => (valid ? 1 : 0) + offset)() + (yield 2);
    };
}
