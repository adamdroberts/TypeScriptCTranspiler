...
class Worker {
    async run(kind: number): Promise<string> {
        ...
        else first = await delay(1, "method-six");
        const second = await delay(1, first + "-second");
        return second;
    }
}
...
const value = async function(kind: number): Promise<string> {
    ...
    else first = await delay(1, "value-six");
    const second = await delay(1, first + "-second");
    return second;
};
...
async function branch(outer: boolean, kind: number): Promise<string> {
    if (outer) {
        ...
        else first = await delay(1, "branch-six");
        const second = await delay(1, first + "-second");
        return second;
    }
    return "fallthrough";
...
declaration(0).then((value: string): void => console.log("declaration-zero:", value));
declaration(6).then((value: string): void => console.log("declaration-six:", value));
const worker = new Worker();
worker.run(3).then((value: string): void => console.log("method-three:", value));
value(5).then((result: string): void => console.log("value-five:", result));
branch(true, 2).then((value: string): void => console.log("branch-two:", value));
branch(false, 0).then((value: string): void => console.log("branch-fallthrough:", value));
