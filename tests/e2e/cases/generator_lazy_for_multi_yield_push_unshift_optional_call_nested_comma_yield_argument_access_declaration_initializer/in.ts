function pushNestedCommaCallback(value: any): any {
    return "push-nested-" + value;
}

function unshiftNestedCommaCallback(value: any): any {
    return "unshift-nested-" + value;
}

function* multiYieldPushOptionalCallNestedCommaYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedCommaCallback?.((yield "push-optional", yield "push-comma")),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftOptionalCallNestedCommaYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedCommaCallback?.((yield "unshift-optional", yield "unshift-comma")),
            yield "unshift-item",
...
const pushIterator = multiYieldPushOptionalCallNestedCommaYieldArgumentAccessDeclarationInitializer();
...
const pushFourth: any = pushIterator.next(20);
...
const unshiftIterator = multiYieldUnshiftOptionalCallNestedCommaYieldArgumentAccessDeclarationInitializer();
...
const unshiftThird: any = unshiftIterator.next(30);
console.log("unshift-third", unshiftReceiver.join(","), unshiftThird.done, unshiftThird.value);
const unshiftFourth: any = unshiftIterator.next(40);
