function* values(): Generator<number, number, number> {
    return (yield 1) + ((raw: string = "2020-01-02", year: number = 2020, parsed: number = Date.parse(raw), stamp: number = Date.UTC(year, 0, 2), valid: boolean = parsed > 0 && stamp > 0) => valid ? 1 : 0)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((raw: string = "2020-01-02", year: number = 2020, parsed: number = Date.parse(raw), stamp: number = Date.UTC(year, 0, 2), valid: boolean = parsed > 0 && stamp > 0) => (valid ? 1 : 0) + offset)() + (yield 2);
    };
}
