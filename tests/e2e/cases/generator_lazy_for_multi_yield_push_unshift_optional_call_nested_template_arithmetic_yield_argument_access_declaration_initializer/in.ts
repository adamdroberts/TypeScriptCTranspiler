function pushNestedTemplateArithmeticCallback(value: any): any {
    return "push-template-" + value;
}

function unshiftNestedTemplateArithmeticCallback(value: any): any {
    return "unshift-template-" + value;
}

function* multiYieldPushNestedTemplateArithmeticArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedTemplateArithmeticCallback?.(`push-${(yield "push-arithmetic-one") + 1}-${(yield "push-arithmetic-two") * 2}`),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftNestedTemplateArithmeticArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedTemplateArithmeticCallback?.(String.raw`unshift-${(yield "unshift-arithmetic-one") | 1}-${(yield "unshift-arithmetic-two") << 1}`),
            yield "unshift-item",
...
const pushIterator = multiYieldPushNestedTemplateArithmeticArgumentAccessDeclarationInitializer();
...
const pushFourth: any = pushIterator.next(3);
...
const unshiftIterator = multiYieldUnshiftNestedTemplateArithmeticArgumentAccessDeclarationInitializer();
...
const unshiftFourth: any = unshiftIterator.next(3);
