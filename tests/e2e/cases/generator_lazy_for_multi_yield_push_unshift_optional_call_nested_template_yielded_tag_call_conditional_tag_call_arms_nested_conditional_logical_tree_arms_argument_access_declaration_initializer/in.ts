...
function pushTrueNested(strings: TemplateStringsArray, value: any): any {
    return "push-true-nested-" + strings[0] + value + strings[1];
}

function pushFalseNested(strings: TemplateStringsArray, value: any): any {
    return "push-false-nested-" + strings[0] + value + strings[1];
}

function unshiftTrueNested(strings: TemplateStringsArray, value: any): any {
    return "unshift-true-nested-" + strings[0] + value + strings[1];
}

function unshiftFalseNested(strings: TemplateStringsArray, value: any): any {
...
function getPushTrueNested(seed: any): any {
    pushEvents.push("true-nested-" + seed);
    return pushTrueNested;
}

function getPushFalseNested(seed: any): any {
    pushEvents.push("false-nested-" + seed);
    return pushFalseNested;
}

function getUnshiftTrueNested(seed: any): any {
    unshiftEvents.push("true-nested-" + seed);
    return unshiftTrueNested;
}

function getUnshiftFalseNested(seed: any): any {
    unshiftEvents.push("false-nested-" + seed);
    return unshiftFalseNested;
}
...
function* pushNestedConditionalLogicalTrees(): Generator<string, string, any> {
    for (
                ...
                (yield "push-selector")
                    ? getPushArm(
                        (yield "push-nested-selector")
                            ? ((yield "push-true-left") && (
                                (yield "push-true-middle") ||
                                getPushTrueNested((yield "push-true-seed"))`nested-${yield "push-true-value"}`
                            ))
                            : ((yield "push-false-left") || (
                                (yield "push-false-middle") ??
                                getPushFalseNested((yield "push-false-seed"))`nested-${yield "push-false-value"}`
                            ))
                    )`arm-${yield "push-arm-value"}`
                    : getPushArm("push-outer-fallback")`arm-${yield "push-fallback-arm-value"}`
...
function* unshiftNestedConditionalLogicalTrees(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftCallback?.(String.raw`unshift-${getUnshiftOuter(
                (yield "unshift-selector")
                    ? getUnshiftArm("unshift-outer-fallback")`arm-${yield "unshift-fallback-arm-value"}`
                    : getUnshiftArm(
                        (yield "unshift-nested-selector")
                            ? ((yield "unshift-true-left") && (
                                (yield "unshift-true-middle") ||
                                getUnshiftTrueNested((yield "unshift-true-seed"))`nested-${yield "unshift-true-value"}`
                            ))
                            : ((yield "unshift-false-left") || (
                                (yield "unshift-false-middle") ??
                                getUnshiftFalseNested((yield "unshift-false-seed"))`nested-${yield "unshift-false-value"}`
                            ))
...
const pushIterator = pushNestedConditionalLogicalTrees();
...
const pushSixth: any = pushIterator.next(false);
console.log("push-sixth", pushEvents.join(","), pushReceiver.join(","), pushSixth.done, pushSixth.value);
const pushSeventh: any = pushIterator.next("push-true-seed-value");
console.log("push-seventh", pushEvents.join(","), pushReceiver.join(","), pushSeventh.done, pushSeventh.value);
const pushEighth: any = pushIterator.next("push-true-value");
console.log("push-eighth", pushEvents.join(","), pushReceiver.join(","), pushEighth.done, pushEighth.value);
const pushNinth: any = pushIterator.next("push-arm-value");
console.log("push-ninth", pushEvents.join(","), pushReceiver.join(","), pushNinth.done, pushNinth.value);
const pushTenth: any = pushIterator.next("push-inner-value");
console.log("push-tenth", pushEvents.join(","), pushReceiver.join(","), pushTenth.done, pushTenth.value);
const pushEleventh: any = pushIterator.next("push-outer-value");
console.log("push-eleventh", pushEvents.join(","), pushReceiver.join(","), pushEleventh.done, pushEleventh.value);
const pushTwelfth: any = pushIterator.next("push-item-value");
console.log("push-twelfth", pushEvents.join(","), pushReceiver.join(","), pushTwelfth.done, pushTwelfth.value);
const pushThirteenth: any = pushIterator.next("push-key-value");
console.log("push-thirteenth", pushEvents.join(","), pushReceiver.join(","), pushThirteenth.done, pushThirteenth.value);
...
const unshiftIterator = unshiftNestedConditionalLogicalTrees();
...
const unshiftFifth: any = unshiftIterator.next(false);
console.log("unshift-fifth", unshiftEvents.join(","), unshiftReceiver.join(","), unshiftFifth.done, unshiftFifth.value);
const unshiftSixth: any = unshiftIterator.next(null);
console.log("unshift-sixth", unshiftEvents.join(","), unshiftReceiver.join(","), unshiftSixth.done, unshiftSixth.value);
const unshiftSeventh: any = unshiftIterator.next("unshift-false-seed-value");
console.log("unshift-seventh", unshiftEvents.join(","), unshiftReceiver.join(","), unshiftSeventh.done, unshiftSeventh.value);
const unshiftEighth: any = unshiftIterator.next("unshift-false-value");
console.log("unshift-eighth", unshiftEvents.join(","), unshiftReceiver.join(","), unshiftEighth.done, unshiftEighth.value);
const unshiftNinth: any = unshiftIterator.next("unshift-arm-value");
console.log("unshift-ninth", unshiftEvents.join(","), unshiftReceiver.join(","), unshiftNinth.done, unshiftNinth.value);
const unshiftTenth: any = unshiftIterator.next("unshift-inner-value");
console.log("unshift-tenth", unshiftEvents.join(","), unshiftReceiver.join(","), unshiftTenth.done, unshiftTenth.value);
const unshiftEleventh: any = unshiftIterator.next("unshift-outer-value");
console.log("unshift-eleventh", unshiftEvents.join(","), unshiftReceiver.join(","), unshiftEleventh.done, unshiftEleventh.value);
const unshiftTwelfth: any = unshiftIterator.next("unshift-item-value");
console.log("unshift-twelfth", unshiftEvents.join(","), unshiftReceiver.join(","), unshiftTwelfth.done, unshiftTwelfth.value);
const unshiftThirteenth: any = unshiftIterator.next("unshift-key-value");
console.log("unshift-thirteenth", unshiftEvents.join(","), unshiftReceiver.join(","), unshiftThirteenth.done, unshiftThirteenth.value);
