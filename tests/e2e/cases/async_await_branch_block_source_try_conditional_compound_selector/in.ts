...
async function compoundSelectorCatch(flag: boolean, inner: boolean): Promise<string> {
    try {
...
class CompoundSelectorRunner {
    async method(flag: boolean, inner: boolean): Promise<string> {
        try {
            return (flag || inner) ? await delay(3, "method-ok") : await delayedRejectAfter(4, "method-bad");
        } finally {
            trace += "M";
        }
    }
...
const compoundSelectorCatchFinally = async (flag: boolean | null, inner: boolean): Promise<string> => {
    try {
...
compoundSelectorCatch(true, false)
    .then((value) => {
        console.log("fn-reject", value, trace);
        return compoundSelectorCatch(true, true);
    })
    .then((value) => {
        console.log("fn-ok", value, trace);
        return runner.method(false, false).then(
    ...
    .then((value) => {
        ...
        return compoundSelectorCatchFinally(false, false);
    })
    .then((value) => {
        console.log("arrow-reject", value, trace);
        return compoundSelectorCatchFinally(null, true);
    })
