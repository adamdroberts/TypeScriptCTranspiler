function pushCallbackImpl(first: any, second: any): any {
    return "push-spread-" + first + "-" + second;
}

let pushCallback: any = null;
pushCallback = pushCallbackImpl;
let unshiftCallback: any = null;
const pushCallArguments: any = ["push-optional", 7];
const unshiftCallArguments: any = ["unshift-optional", 8];

function* multiYieldPushOptionalCallSpreadArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushCallback?.(...pushCallArguments),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftOptionalCallSpreadArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftCallback?.(...unshiftCallArguments),
            yield "unshift-item",
...
const pushIterator = multiYieldPushOptionalCallSpreadArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftOptionalCallSpreadArgumentAccessDeclarationInitializer();
