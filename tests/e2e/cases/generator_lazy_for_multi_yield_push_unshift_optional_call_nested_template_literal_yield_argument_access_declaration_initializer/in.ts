function pushNestedTemplateLiteralCallback(value: any): any {
    return "push-template-literal-" + value;
}

function unshiftNestedTemplateLiteralCallback(value: any): any {
    return "unshift-template-literal-" + value;
}

function* multiYieldPushNestedTemplateLiteralArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedTemplateLiteralCallback?.(
                `push-${[yield "push-array-first", yield "push-array-second"]}-${({ "push-label": yield "push-object-value" } as any)}`
            ),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
...
function* multiYieldUnshiftNestedTemplateLiteralArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedTemplateLiteralCallback?.(
                String.raw`unshift-${[yield "unshift-array-first", yield "unshift-array-second"]}-${({ "unshift-label": yield "unshift-object-value" } as any)}`
            ),
            yield "unshift-item",
...
const pushIterator = multiYieldPushNestedTemplateLiteralArgumentAccessDeclarationInitializer();
...
const pushThird: any = pushIterator.next("push-array-one");
console.log("push-third", pushReceiver.join(","), pushThird.done, pushThird.value);
const pushFourth: any = pushIterator.next("push-array-two");
console.log("push-fourth", pushReceiver.join(","), pushFourth.done, pushFourth.value);
const pushFifth: any = pushIterator.next("push-object");
...
const unshiftIterator = multiYieldUnshiftNestedTemplateLiteralArgumentAccessDeclarationInitializer();
...
const unshiftThird: any = unshiftIterator.next("unshift-array-one");
console.log("unshift-third", unshiftReceiver.join(","), unshiftThird.done, unshiftThird.value);
const unshiftFourth: any = unshiftIterator.next("unshift-array-two");
console.log("unshift-fourth", unshiftReceiver.join(","), unshiftFourth.done, unshiftFourth.value);
const unshiftFifth: any = unshiftIterator.next("unshift-object");
