...
function pushArmCall(value: any): any {
    return "push-call-" + value;
}

function unshiftArmCall(value: any): any {
    return "unshift-call-" + value;
}
...
function* pushNestedCallArms(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushCallback?.(`push-${getPushOuter(
                (yield "push-selector")
                    ? getPushArm(pushArmCall?.(yield "push-call-argument"))`arm-${yield "push-arm-value"}`
...
function* unshiftNestedCallArms(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftCallback?.(String.raw`unshift-${getUnshiftOuter(
                (yield "unshift-selector")
                    ...
                    : getUnshiftArm(unshiftArmCall?.(yield "unshift-call-argument"))`arm-${yield "unshift-arm-value"}`
...
const pushIterator = pushNestedCallArms();
...
const unshiftIterator = unshiftNestedCallArms();
