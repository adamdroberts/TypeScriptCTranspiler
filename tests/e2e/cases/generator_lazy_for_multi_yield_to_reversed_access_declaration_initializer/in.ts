function* multiYieldToReversedAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "receiver").toReversed()[yield "key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const iterator = multiYieldToReversedAccessDeclarationInitializer();
