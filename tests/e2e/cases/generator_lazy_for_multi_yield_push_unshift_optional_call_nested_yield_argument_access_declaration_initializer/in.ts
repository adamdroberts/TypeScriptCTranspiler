function pushNestedCallback(value: any): any {
    return "push-nested-" + value;
}

function unshiftNestedCallback(value: any): any {
    return "unshift-nested-" + value;
}

function* multiYieldPushOptionalCallNestedYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedCallback?.(yield "push-optional"),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftOptionalCallNestedYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedCallback?.(yield "unshift-optional"),
            yield "unshift-item",
...
const pushIterator = multiYieldPushOptionalCallNestedYieldArgumentAccessDeclarationInitializer();
...
const pushThird: any = pushIterator.next("push-callback");
...
const unshiftIterator = multiYieldUnshiftOptionalCallNestedYieldArgumentAccessDeclarationInitializer();
...
const unshiftThird: any = unshiftIterator.next("unshift-callback");
