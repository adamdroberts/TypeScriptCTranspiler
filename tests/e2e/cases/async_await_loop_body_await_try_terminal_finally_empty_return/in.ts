...
async function runFinal(sourceReject: boolean, cleanupReject: boolean): Promise<string | undefined> {
    let count = 0;
...
async function runCatch(stageReject: boolean, sourceReject: boolean, recoveryReject: boolean, catchTerminalReject: boolean, cleanupReject: boolean): Promise<string | undefined> {
    let count = 0;
    while (await laterBoolean(count < 1)) {
        count = 1;
        try {
