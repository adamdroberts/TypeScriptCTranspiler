function* values(): Generator<number, number, number> {
    return (yield 1) + ((value: number = 7, kind: string = typeof value) => kind.length + value)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((value: number = 7, kind: string = typeof value) => kind.length + value + offset)() + (yield 2);
    };
}
