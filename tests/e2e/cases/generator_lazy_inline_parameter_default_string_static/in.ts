function* values(): Generator<number, number, number> {
    return (yield 1) + ((code: number = 65, char: string = String.fromCharCode(code), point: string = String.fromCodePoint(0x1f600)) => char === "A" && point === "😀" ? 3 : 0)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((code: number = 65, char: string = String.fromCharCode(code), point: string = String.fromCodePoint(0x1f600)) => (char === "A" && point === "😀" ? 3 : 0) + offset)() + (yield 2);
    };
}
