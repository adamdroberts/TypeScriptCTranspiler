...
function pushTagCallExpressionTag(strings: TemplateStringsArray, value: any): any {
    return "push-expression-" + strings[0] + value + strings[1];
}

function unshiftTagCallExpressionTag(strings: TemplateStringsArray, value: any): any {
    return "unshift-expression-" + strings[0] + value + strings[1];
}

function getPushTagCallExpressionTag(seed: any): any {
    pushTagEvents.push("tag-" + seed);
    return pushTagCallExpressionTag;
}

function getUnshiftTagCallExpressionTag(seed: any): any {
    unshiftTagEvents.push("tag-" + seed);
    return unshiftTagCallExpressionTag;
}

function pushTagCallExpressionCallback(value: any): any {
    return "push-tagged-" + value;
}

function unshiftTagCallExpressionCallback(value: any): any {
    return "unshift-tagged-" + value;
}

function* multiYieldPushTagCallExpressionArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushTagCallExpressionCallback?.(`push-${getPushTagCallExpressionTag((yield "push-tag-left") + (yield "push-tag-right"))`inner-${yield "push-inner"}`}-${yield "push-outer"}`),
            yield "push-item"
        )[yield "push-key"],
        count = 0;
        count < 1;
...
function* multiYieldUnshiftTagCallExpressionArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftTagCallExpressionCallback?.(String.raw`unshift-${getUnshiftTagCallExpressionTag((yield "unshift-tag-left") && (yield "unshift-tag-right"))`inner-${yield "unshift-inner"}`}-${yield "unshift-outer"}`),
            yield "unshift-item"
        )[yield "unshift-key"],
        count = 0;
        count < 1;
...
const pushIterator = multiYieldPushTagCallExpressionArgumentAccessDeclarationInitializer();
...
const pushThird: any = pushIterator.next("push-left-value");
console.log("push-third", pushTagEvents.join(","), pushReceiver.join(","), pushThird.done, pushThird.value);
const pushFourth: any = pushIterator.next("push-right-value");
...
const unshiftIterator = multiYieldUnshiftTagCallExpressionArgumentAccessDeclarationInitializer();
...
const unshiftThird: any = unshiftIterator.next("");
