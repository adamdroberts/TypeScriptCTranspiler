...
let directOutput = "";
for (const { [outerKey()]: { [innerKey()]: label, values: [first, ...tail] } } of directRows) {
    directOutput += `${label}:${first}:${tail.length}:${String(tail[0])};`;
}
console.log("direct", directOutput, outerReads, innerReads);
...
let customOutput = "";
for (const { [outerKey()]: { [innerKey()]: label, values: [first, ...tail] } } of new ObjectIterator([
...
console.log("custom", customOutput, outerReads, innerReads);

function* lazyNestedComputed(): Generator<string, string, number> {
    const values: any[] = [
        { payload: { label: "lazy-a", values: ["r", 3, "tail"] as any[] } },
        { payload: { label: "lazy-b", values: ["s"] as any[] } },
    ];
    for (const { [outerKey()]: { [innerKey()]: label, values: [first, ...tail] } } of values) {
        yield `${label}:${first}:${tail.length}:${String(tail[0])}`;
    }
    return "nested-computed-done";
}

const lazy = lazyNestedComputed();
...
console.log("keys", outerReads, innerReads);
