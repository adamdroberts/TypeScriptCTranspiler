function* values(): Generator<number, number, number> {
    return (yield 1) + (({ outer: { first, second }, third }: { outer: { first: number; second: number }; third: number }) => first + second + third)({ outer: { first: 7, second: 8 }, third: 9 }) + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + (({ outer: { first, second }, third }: { outer: { first: number; second: number }; third: number }) => first + second + third + offset)({ outer: { first: 7, second: 8 }, third: 9 }) + (yield 2);
    };
}
