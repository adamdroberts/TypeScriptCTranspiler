let pushDiscarded: any = "push-before-discarded";
let pushValue: any = "push-before-value";
let unshiftDiscarded: any = "unshift-before-discarded";
let unshiftValue: any = "unshift-before-value";

function* multiYieldPushCommaArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push((void pushDiscarded, pushValue), yield "push-item", ...(yield "push-spread"))[yield "push-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
function* multiYieldUnshiftCommaArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift((void unshiftDiscarded, unshiftValue), yield "unshift-item", ...(yield "unshift-spread"))[yield "unshift-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const pushIterator = multiYieldPushCommaArgumentAccessDeclarationInitializer();
...
pushDiscarded = "push-after-discarded";
pushValue = "push-after-value";
...
const unshiftIterator = multiYieldUnshiftCommaArgumentAccessDeclarationInitializer();
...
unshiftDiscarded = "unshift-after-discarded";
unshiftValue = "unshift-after-value";
