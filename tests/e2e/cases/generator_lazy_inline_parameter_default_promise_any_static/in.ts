function* values(): Generator<number, number, number> {
    return (yield 1) + ((winner: Promise<number> = Promise.any([Promise.resolve(1), Promise.resolve(2)]), valid: boolean = winner === winner) => valid ? 1 : 0)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((winner: Promise<number> = Promise.any([Promise.resolve(1), Promise.resolve(2)]), valid: boolean = winner === winner) => (valid ? 1 : 0) + offset)() + (yield 2);
    };
}
