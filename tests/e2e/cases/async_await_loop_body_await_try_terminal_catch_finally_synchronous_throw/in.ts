...
async function run(stageReject: boolean, sourceReject: boolean, recoveryReject: boolean, catchTerminalReject: boolean, cleanupReject: boolean): Promise<string> {
    let count = 0;
...
report("success", run(false, false, false, false, false))
    .then((_value) => report("source-reject", run(false, true, false, false, false)))
    .then((_value) => report("stage-reject", run(true, false, false, false, false)))
    .then((_value) => report("recovery-reject", run(true, false, true, false, false)))
