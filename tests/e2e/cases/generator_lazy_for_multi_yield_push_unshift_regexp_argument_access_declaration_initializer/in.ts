function* multiYieldPushRegexpArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(/push-regexp/gi, yield "push-item", ...(yield "push-spread"))[yield "push-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
function* multiYieldUnshiftRegexpArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(/unshift-regexp/m, yield "unshift-item", ...(yield "unshift-spread"))[yield "unshift-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const pushIterator = multiYieldPushRegexpArgumentAccessDeclarationInitializer();
...
console.log("push-regexp-type", typeof pushReceiver[1]);

const unshiftReceiver: any = ["unshift-first"];
const unshiftSpread: any = ["unshift-spread-one", "unshift-spread-two"];
const unshiftIterator = multiYieldUnshiftRegexpArgumentAccessDeclarationInitializer();
...
console.log("unshift-regexp-type", typeof unshiftReceiver[0]);
