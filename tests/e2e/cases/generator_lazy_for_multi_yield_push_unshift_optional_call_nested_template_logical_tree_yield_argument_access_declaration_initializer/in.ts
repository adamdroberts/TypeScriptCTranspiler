function pushNestedTemplateLogicalTreeCallback(value: any): any {
    return "push-tree-" + value;
}

function unshiftNestedTemplateLogicalTreeCallback(value: any): any {
    return "unshift-tree-" + value;
}

function* multiYieldPushNestedTemplateLogicalTreeArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedTemplateLogicalTreeCallback?.(`push-${((yield "push-outer") && ((yield "push-inner-left") || (yield "push-inner-right")))}-${yield "push-tail"}`),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftNestedTemplateLogicalTreeArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedTemplateLogicalTreeCallback?.(String.raw`unshift-${((yield "unshift-outer") ?? ((yield "unshift-inner-left") && (yield "unshift-inner-right")))}-${yield "unshift-tail"}`),
            yield "unshift-item",
...
const pushIterator = multiYieldPushNestedTemplateLogicalTreeArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftNestedTemplateLogicalTreeArgumentAccessDeclarationInitializer();
...
const unshiftFifth: any = unshiftIterator.next("unshift-inner-right-value");
console.log("unshift-fifth", unshiftReceiver.join(","), unshiftFifth.done, unshiftFifth.value);
const unshiftSixth: any = unshiftIterator.next("unshift-tail-value");
