function* values(): Generator<number, number, number> {
    return (yield 1) + (({ first = 7, nested: { second = 8 }, values: [third = 9] }: { first?: number; nested: { second?: number }; values: Array<number | undefined> }) => first + second + third)({ nested: {}, values: [undefined] }) + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + (({ first = 7, nested: { second = 8 }, values: [third = 9] }: { first?: number; nested: { second?: number }; values: Array<number | undefined> }) => first + second + third + offset)({ nested: {}, values: [undefined] }) + (yield 2);
    };
}
