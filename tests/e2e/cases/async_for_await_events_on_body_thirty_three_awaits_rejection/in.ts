...
async function consume(iterator: any): Promise<string> {
    for await (const item of iterator) {
        ...
        await Promise.reject("thirty-three-rejection");
        break;
    }
...
consume(iterator).then((value: string): void => {
    console.log("body-thirty-three-awaits-rejected: unexpected", value);
}, (reason: any): void => {
    console.log("body-thirty-three-awaits-rejected:", reason);
});
