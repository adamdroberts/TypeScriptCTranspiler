function* multiYieldCopyWithinAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "receiver").copyWithin(yield "target", yield "start")[yield "key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const iterator = multiYieldCopyWithinAccessDeclarationInitializer();
