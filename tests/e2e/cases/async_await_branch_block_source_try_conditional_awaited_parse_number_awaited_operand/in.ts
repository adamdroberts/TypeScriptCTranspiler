...
function value(flag: number): Promise<string> {
    trace += "L";
    if (flag === 6) return Promise.reject("parse-bad");
...
async function declaration(flag: number): Promise<string> {
    try {
        return parseInt(await value(flag))
...
class ParseNumberSelectorRunner {
    async method(flag: number): Promise<string> {
        try {
            return parseFloat(await value(flag))
...
const runner = new ParseNumberSelectorRunner();
