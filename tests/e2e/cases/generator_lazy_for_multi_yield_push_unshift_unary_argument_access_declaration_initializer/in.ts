let pushNumber: any = 3;
let unshiftNumber: any = 4;

function* multiYieldPushUnaryArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(-pushNumber, yield "push-item", ...(yield "push-spread"))[yield "push-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
function* multiYieldUnshiftUnaryArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(+unshiftNumber, yield "unshift-item", ...(yield "unshift-spread"))[yield "unshift-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const pushIterator = multiYieldPushUnaryArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftUnaryArgumentAccessDeclarationInitializer();
