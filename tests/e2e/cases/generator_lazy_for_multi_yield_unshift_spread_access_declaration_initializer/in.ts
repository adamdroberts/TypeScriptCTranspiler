function* multiYieldUnshiftSpreadAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "receiver").unshift(...(yield "items"))[yield "key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const iterator = multiYieldUnshiftSpreadAccessDeclarationInitializer();
