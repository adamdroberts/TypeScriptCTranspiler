function pushNestedCallInner(value: any): any {
    return "push-inner-" + value;
}

function pushNestedCallOuter(value: any): any {
    return "push-outer-" + value;
}

function unshiftNestedCallInner(value: any): any {
    return "unshift-inner-" + value;
}

function unshiftNestedCallOuter(value: any): any {
    return "unshift-outer-" + value;
}

function* multiYieldPushNestedCallArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedCallOuter?.(pushNestedCallInner(yield "push-nested")),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftNestedCallArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedCallOuter?.(unshiftNestedCallInner(yield "unshift-nested")),
            yield "unshift-item",
...
const pushIterator = multiYieldPushNestedCallArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftNestedCallArgumentAccessDeclarationInitializer();
