...
function pattern(flag: number): Promise<string> {
    trace += "L";
    if (flag === 6) return Promise.reject("pattern-bad");
...
async function declaration(flag: number): Promise<string> {
    try {
        return RegExp(await pattern(flag))
...
const runner = new RegExpSelectorRunner();
...
declaration(1)
    ...
    .then((valueResult) => {
        console.log("declaration-empty-pattern", valueResult, trace);
        trace = "";
    ...
    .then((valueResult) => {
        console.log("method-empty-pattern", valueResult, trace);
        trace = "";
    ...
    .then((valueResult) => {
        console.log("arrow-empty-pattern", valueResult, trace);
        trace = "";
        return arrow(6);
    })
