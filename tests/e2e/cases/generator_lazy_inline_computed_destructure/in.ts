function* values(): Generator<number, number, number> {
    return (yield 1) + (({ ["first"]: first, nested: { ["second"]: second } }: { first: number; nested: { second: number } }) => first + second)({ first: 7, nested: { second: 8 } }) + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + (({ ["first"]: first, nested: { ["second"]: second } }: { first: number; nested: { second: number } }) => first + second + offset)({ first: 7, nested: { second: 8 } }) + (yield 2);
    };
}
