function* values(): Generator<number, number, number> {
    return (yield 1) + ((items: number[] = Array.of(1, 2), count: number = items.length) => count)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((items: number[] = Array.of(1, 2), count: number = items.length) => count + offset)() + (yield 2);
    };
}
