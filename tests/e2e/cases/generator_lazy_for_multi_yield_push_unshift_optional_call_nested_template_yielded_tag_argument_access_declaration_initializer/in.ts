function pushYieldedNestedTemplateTag(strings: TemplateStringsArray, first: any, second: any): any {
    return "push-yielded-tag-" + strings[0] + first + strings[1] + second + strings[2];
}

function unshiftYieldedNestedTemplateTag(strings: TemplateStringsArray, first: any, second: any): any {
    return "unshift-yielded-tag-" + strings[0] + first + strings[1] + second + strings[2];
}

function pushYieldedNestedTemplateTagCallback(value: any): any {
    return "push-tagged-" + value;
}

function unshiftYieldedNestedTemplateTagCallback(value: any): any {
    return "unshift-tagged-" + value;
}

function* multiYieldPushYieldedNestedTemplateTagArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushYieldedNestedTemplateTagCallback?.(`push-${(yield "push-tag")`inner-${yield "push-inner-one"}-${yield "push-inner-two"}`}-${yield "push-outer"}`),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftYieldedNestedTemplateTagArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftYieldedNestedTemplateTagCallback?.(String.raw`unshift-${(yield "unshift-tag")`inner-${yield "unshift-inner-one"}-${yield "unshift-inner-two"}`}-${yield "unshift-outer"}`),
            yield "unshift-item",
...
const pushIterator = multiYieldPushYieldedNestedTemplateTagArgumentAccessDeclarationInitializer();
...
const pushThird: any = pushIterator.next(pushYieldedNestedTemplateTag);
...
const unshiftIterator = multiYieldUnshiftYieldedNestedTemplateTagArgumentAccessDeclarationInitializer();
...
const unshiftThird: any = unshiftIterator.next(unshiftYieldedNestedTemplateTag);
