...
async function forAwaitIncrementorIfAwaitBreak(): Promise<string> {
    let index = 0, trace = 0;
    for (; await laterBoolean(index < 2); await laterNumber(index++)) {
        trace = trace * 10 + 1;
        if (await laterBoolean(index === 1)) break;
    }
    return await laterString(trace + "|" + index);
}

forAwaitIncrementorIfAwaitBreak().then((value) => console.log(value));
