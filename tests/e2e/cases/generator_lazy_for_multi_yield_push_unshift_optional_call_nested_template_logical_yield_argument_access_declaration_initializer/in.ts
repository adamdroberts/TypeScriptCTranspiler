function pushNestedTemplateLogicalCallback(value: any): any {
    return "push-logical-" + value;
}

function unshiftNestedTemplateLogicalCallback(value: any): any {
    return "unshift-logical-" + value;
}

function* multiYieldPushNestedTemplateLogicalArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedTemplateLogicalCallback?.(`push-${((yield "push-logical-left") && "yes")}-${yield "push-tail"}`),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftNestedTemplateLogicalArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedTemplateLogicalCallback?.(String.raw`unshift-${((yield "unshift-logical-left") ?? "fallback")}-${yield "unshift-tail"}`),
            yield "unshift-item",
...
const pushIterator = multiYieldPushNestedTemplateLogicalArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftNestedTemplateLogicalArgumentAccessDeclarationInitializer();
