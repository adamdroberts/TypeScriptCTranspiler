function* values(): Generator<number, number, number> {
    return (yield 1) + ((raw: string = "hello", encoded: string = btoa(raw), decoded: string = atob(encoded), valid: boolean = decoded === raw) => valid ? 1 : 0)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((raw: string = "hello", encoded: string = btoa(raw), decoded: string = atob(encoded), valid: boolean = decoded === raw) => (valid ? 1 : 0) + offset)() + (yield 2);
    };
}
