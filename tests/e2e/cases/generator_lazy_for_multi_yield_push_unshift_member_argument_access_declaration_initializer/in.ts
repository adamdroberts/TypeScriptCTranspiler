let pushSource: any = { label: "push-before" };
let pushKey: any = "label";
let unshiftSource: any = { label: "unshift-before" };
let unshiftKey: string = "label";

function* multiYieldPushMemberArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
...
function* multiYieldUnshiftMemberArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftSource[unshiftKey],
            yield "unshift-item",
...
const pushIterator = multiYieldPushMemberArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftMemberArgumentAccessDeclarationInitializer();
...
unshiftSource.label = "unshift-after";
