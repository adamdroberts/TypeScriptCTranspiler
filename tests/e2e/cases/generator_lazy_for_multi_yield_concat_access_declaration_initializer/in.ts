function* multiYieldConcatAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "receiver").concat()[yield "key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const iterator = multiYieldConcatAccessDeclarationInitializer();
