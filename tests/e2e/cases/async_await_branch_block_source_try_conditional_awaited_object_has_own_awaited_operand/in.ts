...
function objectSource(flag: number): Promise<any> {
    trace += "L";
    if (flag === 6) return Promise.reject("object-has-own-bad");
    return delay(1, flag === 1 ? { present: true } : flag === 2 ? { other: true } : { missing: true });
}
...
async function declaration(flag: number): Promise<string> {
    try {
        return Object.hasOwn(await objectSource(flag), "present")
...
class ObjectHasOwnSelectorRunner {
    async method(flag: number): Promise<string> {
        try {
            return Object.hasOwn(await objectSource(flag), "other")
...
const runner = new ObjectHasOwnSelectorRunner();
