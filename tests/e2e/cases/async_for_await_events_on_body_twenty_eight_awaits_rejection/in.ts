...
async function consume(iterator: any): Promise<string> {
    for await (const item of iterator) {
        ...
        await Promise.reject("twenty-eight-rejection");
        break;
    }
...
consume(iterator).then((value: string): void => {
    console.log("body-twenty-eight-awaits-rejected: unexpected", value);
}, (reason: any): void => {
    console.log("body-twenty-eight-awaits-rejected:", reason);
});
