...
function* multiYieldPushOptionalCallNestedConditionalYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedConditionalCallback?.((yield "push-condition") ? "push-true" : "push-false"),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftOptionalCallNestedConditionalYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedConditionalCallback?.((yield "unshift-condition") ? "unshift-true" : "unshift-false"),
            yield "unshift-item",
...
const pushIterator = multiYieldPushOptionalCallNestedConditionalYieldArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftOptionalCallNestedConditionalYieldArgumentAccessDeclarationInitializer();
