...
async function runOf(): Promise<string> {
    for (const item of ["of-a", "of-b"]) {
        try {
            ...
            const first = await recoverFirst(reason);
            const second = await recoverSecond(reason);
...
async function runFirstRecoveryRejected(): Promise<string> {
    for (const item of ["first"]) {
        try {
            ...
            const first = await rejectFirst(reason);
            console.log("unreachable-first-" + first);
            const second = await recoverSecond(reason);
...
async function runSecondRecoveryRejected(): Promise<string> {
    for (const item of ["second"]) {
        try {
            ...
            const first = await recoverFirst(reason);
            console.log(item + "-first-" + first + "|" + reason);
            const second = await rejectSecond(reason);
            console.log("unreachable-second-" + second);
        } finally {
            await later(item + "-cleanup");
            console.log(item + "-cleanup");
