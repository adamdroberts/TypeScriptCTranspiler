let pushWord: any = "push-before";
let pushCount: any = 1;
let unshiftWord: any = "unshift-before";
let unshiftCount: any = 2;

function* multiYieldPushTemplateArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(`push-${pushWord}-${pushCount}`, yield "push-item", ...(yield "push-spread"))[yield "push-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
function* multiYieldUnshiftTemplateArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(`unshift-${unshiftWord}-${unshiftCount}`, yield "unshift-item", ...(yield "unshift-spread"))[yield "unshift-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const pushIterator = multiYieldPushTemplateArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftTemplateArgumentAccessDeclarationInitializer();
