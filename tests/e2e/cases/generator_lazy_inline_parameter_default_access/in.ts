function* values(): Generator<number, number, number> {
    return (yield 1) + ((items: number[], index: number = 0, selected: number = items[index]) => selected + items.length)([7, 8]) + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((items: number[], index: number = 0, selected: number = items[index]) => selected + items.length + offset)([7, 8]) + (yield 2);
    };
}
