let pushCallTarget: any = {
    invoke: (first: any, second: any): any => "push-element-" + first + "-" + second
};
let unshiftCallTarget: any = null;

function* multiYieldPushOptionalCallElementCalleeAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushCallTarget["invoke"]?.("push-optional", 7),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftOptionalCallElementCalleeAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftCallTarget?.["invoke"]?.("unshift-optional", 8),
            yield "unshift-item",
...
const pushIterator = multiYieldPushOptionalCallElementCalleeAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftOptionalCallElementCalleeAccessDeclarationInitializer();
