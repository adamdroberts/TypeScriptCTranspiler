...
report("success", run(false, false, false))
    .then((_value) => report("terminal-reject", run(false, true, false)))
    .then((_value) => report("cleanup-reject", run(false, false, true)))
    .then((_value) => report("stage-reject", run(true, false, false)));
