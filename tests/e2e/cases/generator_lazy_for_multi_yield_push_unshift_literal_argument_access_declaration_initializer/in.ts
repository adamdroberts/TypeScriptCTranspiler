let pushArrayHead: any = "push-before-head";
...
function* multiYieldPushLiteralArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push([pushArrayHead, pushArrayTail], yield "push-item", ...(yield "push-spread"))[yield "push-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
function* multiYieldUnshiftLiteralArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift({ value: unshiftObjectValue, count: unshiftObjectCount }, yield "unshift-item", ...(yield "unshift-spread"))[yield "unshift-key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const pushIterator = multiYieldPushLiteralArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftLiteralArgumentAccessDeclarationInitializer();
...
console.log("unshift-literal", unshiftReceiver[0].value, unshiftReceiver[0].count);
