function* multiYieldSortAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "receiver").sort()[yield "key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const receiver: any = ["b", "a"];
const iterator = multiYieldSortAccessDeclarationInitializer();
