function* values(): Generator<number, number, number> {
    return (yield 1) + ((text: string = JSON.stringify({ ok: true, count: 1 }), valid: boolean = text === "{\"ok\":true,\"count\":1}") => valid ? 1 : 0)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((text: string = JSON.stringify({ ok: true, count: 1 }), valid: boolean = text === "{\"ok\":true,\"count\":1}") => (valid ? 1 : 0) + offset)() + (yield 2);
    };
}
