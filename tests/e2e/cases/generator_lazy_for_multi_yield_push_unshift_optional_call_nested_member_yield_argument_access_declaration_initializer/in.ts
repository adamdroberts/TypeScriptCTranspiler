function pushNestedMemberCallback(first: any, second: any): any {
    return "push-member-" + first + "-" + second;
}

function unshiftNestedMemberCallback(first: any, second: any): any {
    return "unshift-member-" + first + "-" + second;
}

function* multiYieldPushOptionalCallNestedMemberYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedMemberCallback?.(
                ({ "push-key": "push-value" } as any)[(yield "push-member-key")],
                (yield "push-member-object").label
            ),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
...
function* multiYieldUnshiftOptionalCallNestedMemberYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedMemberCallback?.(
                ({ "unshift-key": "unshift-value" } as any)[(yield "unshift-member-key")],
                (yield "unshift-member-object").label
            ),
            yield "unshift-item",
...
const pushIterator = multiYieldPushOptionalCallNestedMemberYieldArgumentAccessDeclarationInitializer();
...
const pushThird: any = pushIterator.next("push-key");
console.log("push-third", pushReceiver.join(","), pushThird.done, pushThird.value);
const pushFourth: any = pushIterator.next({ label: "push-label" });
...
const unshiftIterator = multiYieldUnshiftOptionalCallNestedMemberYieldArgumentAccessDeclarationInitializer();
...
const unshiftThird: any = unshiftIterator.next("unshift-key");
console.log("unshift-third", unshiftReceiver.join(","), unshiftThird.done, unshiftThird.value);
const unshiftFourth: any = unshiftIterator.next({ label: "unshift-label" });
