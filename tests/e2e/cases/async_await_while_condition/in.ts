...
async function chooseDirectAwaitMutableUninitializedVarPrelude(prefix: string): Promise<string> {
    var source: string;
    source = prefix + "-mutable-var";
...
async function chooseDirectAwaitUninitializedVarIfPrelude(prefix: string, flag: boolean): Promise<string> {
    var source: string;
    if (flag) source = prefix + "-if";
    else source = prefix + "-else";
    return await laterBodyValue(source);
}

async function chooseDirectAwaitUninitializedVarWhilePrelude(prefix: string, flag: boolean): Promise<string> {
    var source: any;
    while (flag) {
        source = prefix + "-while";
        break;
...
async function chooseDirectAwaitUninitializedVarDoPrelude(prefix: string): Promise<string> {
    var source: any;
    do {
        source = prefix + "-do";
    } while (false);
...
async function chooseDirectAwaitUninitializedVarForPrelude(prefix: string): Promise<string> {
    var source: any;
...
async function chooseDirectAwaitUninitializedVarForOfPrelude(prefix: string): Promise<string> {
    var source: any;
...
async function chooseDirectAwaitUninitializedVarForInPrelude(prefix: string): Promise<string> {
    var source: any;
    for (const key in { in: true }) {
        source = prefix + "-" + key;
    }
    return await laterBodyValue(source);
}

async function chooseDirectAwaitUninitializedVarSwitchPrelude(prefix: string): Promise<string> {
    var source: any;
    switch (1) {
        case 1:
            source = prefix + "-switch";
            break;
        default:
            source = prefix + "-unexpected";
            break;
    }
    return await laterBodyValue(source);
}

async function chooseDirectAwaitUninitializedVarTryFinallyPrelude(prefix: string): Promise<string> {
    var source: any;
    try {
        source = prefix + "-try";
    } finally {
...
async function chooseDirectAwaitUninitializedVarTryCatchFinallyPrelude(prefix: string): Promise<string> {
    var source: any;
    try {
        throw "catch-finally";
    } catch (reason) {
        void reason;
        source = prefix + "-catch-finally";
    } finally {
        void "finally";
    }
    return await laterBodyValue(source);
...
class UninitializedVarTryCatchFinallyChooser {
    async choose(prefix: string): Promise<string> {
        var source: any;
        try {
            throw "method-catch-finally";
        } catch (reason) {
            void reason;
            source = prefix + "-catch-finally";
        } finally {
...
const chooseUninitializedVarTryCatchFinallyValue = async (prefix: string): Promise<string> => {
    var source: any;
    try {
        throw "value-catch-finally";
    } catch (reason) {
        void reason;
        source = prefix + "-catch-finally";
    } finally {
...
async function chooseLoopTwoAwaitBreakAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat)) {
        value += "-two-break";
        break;
    }
...
async function chooseLoopTwoAwaitBreakInitializerAwait(value: string, repeat: boolean): Promise<string> {
    for (value += "-two-initializer"; await laterCondition(repeat) && await laterCondition(repeat); value += "-unexpected-incrementor") {
        value += "-two-initializer-body";
        break;
    }
    return await laterBodyValue(value);
}
...
async function chooseLoopTwoAwaitBreakDeclarationAwait(value: string, repeat: boolean): Promise<string> {
    for (var prefix: any, suffix: any; await laterCondition(repeat) && await laterCondition(repeat); suffix += "-unexpected-incrementor") {
        prefix = "-two-";
        suffix = "declaration";
        value += prefix + suffix;
        break;
...
async function chooseLoopTwoAwaitContinueInitializerIncrementorAwait(value: string, repeat: boolean): Promise<string> {
    for (value += "-two-initializer"; await laterCondition(repeat) && await laterCondition(repeat); value += "-two-incrementor") {
        value += "-two-continue";
        repeat = false;
        continue;
    }
...
async function chooseLoopThreeAwaitContinueInitializerIncrementorAwait(value: string, repeat: boolean): Promise<string> {
    for (value += "-three-initializer"; await laterCondition(repeat) && await laterCondition(repeat) && await laterCondition(repeat); value += "-three-incrementor") {
        value += "-three-continue";
        repeat = false;
        continue;
    }
...
async function chooseLoopThreeAwaitContinueDeclarationIncrementorAwait(value: string, repeat: boolean): Promise<string> {
    for (let suffix: any; await laterCondition(repeat) && await laterCondition(repeat) && await laterCondition(repeat); value += "-three-incrementor") {
        suffix = "-three-declaration";
        value += suffix;
        repeat = false;
        continue;
...
async function chooseLoopConditionalThreeAwaitContinueDeclarationIncrementorAwait(value: string, repeat: boolean): Promise<string> {
    for (let suffix: any; await laterCondition(repeat) ? await laterCondition(repeat) : await laterCondition(false); value += "-conditional-incrementor") {
        suffix = "-conditional-declaration";
        value += suffix;
        repeat = false;
        continue;
...
async function chooseLoopThreeAwaitAndContinueAwait(value: string, repeat: boolean): Promise<string> {
...
async function chooseLoopThreeAwaitOrContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) || await laterCondition(repeat) || await laterCondition(repeat)) {
        value += "-three-or";
        repeat = false;
        continue;
...
async function chooseLoopThreeAwaitNullishContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterNull() ?? await laterNull() ?? await laterCondition(repeat)) {
        value += "-three-nullish";
        repeat = false;
        continue;
    }
...
async function chooseLoopThreeAwaitAndContinueThrowAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat) && await laterCondition(repeat)) {
        value += "-three-and";
        repeat = false;
        continue;
...
async function chooseLoopConditionalThreeAwaitBreakAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) ? await laterCondition(repeat) : await laterCondition(repeat)) {
        value += "-conditional-break";
        break;
    }
    return await laterBodyValue(value);
}
...
async function chooseLoopThreeAwaitMixedAndOrBreakDeclarationAwait(value: string, repeat: boolean): Promise<string> {
    for (var suffix: any; await laterCondition(repeat) && await laterCondition(repeat) || await laterCondition(repeat); value += "-unexpected-incrementor") {
        suffix = "-mixed-break-declaration";
        value += suffix;
        break;
    }
    return await laterBodyValue(value);
...
async function chooseLoopThreeAwaitMixedOrAndBreakDeclarationAwait(value: string, repeat: boolean): Promise<string> {
    for (var suffix: any; await laterCondition(repeat) || await laterCondition(repeat) && await laterCondition(repeat); value += "-unexpected-incrementor") {
        suffix = "-mixed-or-and-break-declaration";
        value += suffix;
        break;
    }
    return await laterBodyValue(value);
...
async function chooseLoopThreeAwaitMixedOrNullishContinueAwait(value: string, repeat: boolean): Promise<string> {
    for (var suffix: any; (await laterCondition(repeat) || await laterCondition(repeat)) ?? await laterCondition(repeat); value += "-three-or-nullish-incrementor") {
        suffix = "-three-or-nullish";
        value += suffix;
        repeat = false;
        continue;
    }
...
async function chooseLoopThreeAwaitRightNullishOrContinueAwait(value: string, repeat: boolean): Promise<string> {
    for (var suffix: any; await laterNull() ?? (await laterCondition(repeat) || await laterCondition(repeat)); value += "-three-right-nullish-or-incrementor") {
        suffix = "-three-right-nullish-or";
        value += suffix;
        repeat = false;
        continue;
    }
...
class ThreeAwaitContinueChooser {
    private readonly suffix: string;
...
async function chooseLoopTwoAwaitForInContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat)) {
        for (const index in ["-for-in"]) {
            value += index;
        }
        repeat = false;
        continue;
...
async function chooseLoopTwoAwaitForContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat)) {
        for (let index = 0; index < 1; index++) {
            value += "-for";
        }
...
async function chooseLoopTwoAwaitIteratorControlContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat)) {
        for (const suffix of ["-skip", "-keep"]) {
...
async function chooseLoopTwoAwaitKeyControlContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat)) {
        for (const index in ["-first", "-second"]) {
            if (index === "0") continue;
            value += index;
        }
        repeat = false;
...
async function chooseLoopTwoAwaitForControlContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat)) {
        for (let index = 0; index < 2; index++) {
            if (index === 0) continue;
            value += "-keep";
...
async function chooseLoopTwoAwaitCatchContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat)) {
        try {
            if (repeat) throw "caught";
            value += "-try";
        } catch (reason) {
            value += "-" + reason;
        }
...
async function chooseLoopTwoAwaitCatchFinallyContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat)) {
        try {
            if (repeat) throw "caught";
            value += "-try";
...
async function chooseLoopTwoAwaitLocalContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat)) {
        const suffix = "-local";
        value += suffix;
        repeat = false;
...
async function chooseLoopTwoAwaitAssignedLocalContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat)) {
        let suffix: string;
        suffix = "-assigned";
        value += suffix;
...
async function chooseLoopTwoAwaitMultipleLocalContinueAwait(value: string, repeat: boolean): Promise<string> {
    while (await laterCondition(repeat) && await laterCondition(repeat)) {
        const first = "-first";
        const second = "-second";
        value += first + second;
...
async function chooseDirectAwaitEscapingVarTryCatchFinallyPrelude(prefix: string): Promise<string> {
    try {
        throw "escaping-catch";
    } catch (reason) {
        var source: any;
        void reason;
...
async function chooseOptionalIfAwaitPrelude(prefix: string, flag: boolean): Promise<string> {
    if (flag) {
...
class LoopChooser {
    ...
    async pickDirectAwaitEscapingVarTryCatchFinallyPrelude(prefix: string): Promise<string> {
        try {
            throw "method-escaping-catch";
        } catch (reason) {
            var source: any;
            void reason;
            source = prefix + "-escaping-catch-finally";
    ...
    async pickOptionalIfAwaitPrelude(prefix: string, flag: boolean): Promise<string> {
        if (flag) {
            await laterBodyValue(prefix + "-optional-branch");
        }
        return await laterBodyValue(prefix + "-optional-tail");
...
const chooseDirectAwaitEscapingVarTryCatchFinallyPreludeValue = async (prefix: string): Promise<string> => {
    try {
        throw "value-escaping-catch";
    } catch (reason) {
        var source: any;
        void reason;
        source = prefix + "-escaping-catch-finally";
...
const chooseOptionalIfAwaitPreludeValue = async (prefix: string, flag: boolean): Promise<string> => {
    if (flag) {
        await laterBodyValue(prefix + "-optional-branch");
    }
    return await laterBodyValue(prefix + "-optional-tail");
...
async function chooseMixedNestedIfAwaitPrelude(prefix: string, flag: boolean): Promise<string> {
    var source: any;
    if (flag) {
        if (prefix.length > 0) void "nested-branch-prelude";
        source = await laterBodyValue(prefix + "-nested-await");
    } else {
        source = prefix + "-nested-sync";
    }
    return await laterBodyValue(source);
}
...
chooseDirectAwaitUninitializedVarIfPrelude("direct-uninitialized-var-if", true).then((value) => console.log("await-direct-uninitialized-var-if-true", value));
chooseDirectAwaitUninitializedVarIfPrelude("direct-uninitialized-var-if", false).then((value) => console.log("await-direct-uninitialized-var-if-false", value));
chooseDirectAwaitUninitializedVarWhilePrelude("direct-uninitialized-var-while", true).then((value) => console.log("await-direct-uninitialized-var-while-true", value));
chooseDirectAwaitUninitializedVarDoPrelude("direct-uninitialized-var-do").then((value) => console.log("await-direct-uninitialized-var-do", value));
chooseDirectAwaitUninitializedVarForPrelude("direct-uninitialized-var-for").then((value) => console.log("await-direct-uninitialized-var-for", value));
chooseDirectAwaitUninitializedVarForOfPrelude("direct-uninitialized-var-for-of").then((value) => console.log("await-direct-uninitialized-var-for-of", value));
...
chooseOptionalIfAwaitPrelude("direct", true).then((value) => console.log("await-direct-optional-if-true", value));
chooseOptionalIfAwaitPrelude("direct", false).then((value) => console.log("await-direct-optional-if-false", value));
new LoopChooser("method-").pickOptionalIfAwaitPrelude("method", true).then((value) => console.log("await-method-optional-if-true", value));
...
chooseLoopTwoAwaitContinueDeclarationIncrementorAwait("loop-two-await-continue-declaration", true).then((value) => console.log("await-loop-two-await-continue-declaration", value));
...
chooseLoopTwoAwaitContinueInitializerIncrementorAwait("loop-two-await-continue-initializer-incrementor", true).then((value) => console.log("await-loop-two-await-continue-initializer-incrementor", value));
...
chooseThreeAwaitContinueValue("loop-three-await-value-continue", true).then((value) => console.log("await-loop-three-await-value-continue", value));
...
chooseLoopTwoAwaitForInContinueAwait("loop-two-await-for-in-continue", true).then((value) => console.log("await-loop-two-await-for-in-continue", value));
chooseLoopTwoAwaitForContinueAwait("loop-two-await-for-continue", true).then((value) => console.log("await-loop-two-await-for-continue", value));
chooseLoopTwoAwaitIteratorControlContinueAwait("loop-two-await-iterator-control-continue", true).then((value) => console.log("await-loop-two-await-iterator-control-continue", value));
chooseLoopTwoAwaitKeyControlContinueAwait("loop-two-await-key-control-continue", true).then((value) => console.log("await-loop-two-await-key-control-continue", value));
chooseLoopTwoAwaitForControlContinueAwait("loop-two-await-for-control-continue", true).then((value) => console.log("await-loop-two-await-for-control-continue", value));
chooseLoopTwoAwaitCatchContinueAwait("loop-two-await-catch-continue", true).then((value) => console.log("await-loop-two-await-catch-continue", value));
chooseLoopTwoAwaitCatchFinallyContinueAwait("loop-two-await-catch-finally-continue", true).then((value) => console.log("await-loop-two-await-catch-finally-continue", value));
chooseLoopTwoAwaitLocalContinueAwait("loop-two-await-local-continue", true).then((value) => console.log("await-loop-two-await-local-continue", value));
chooseLoopTwoAwaitAssignedLocalContinueAwait("loop-two-await-assigned-local-continue", true).then((value) => console.log("await-loop-two-await-assigned-local-continue", value));
chooseLoopTwoAwaitMultipleLocalContinueAwait("loop-two-await-multiple-local-continue", true).then((value) => console.log("await-loop-two-await-multiple-local-continue", value));
