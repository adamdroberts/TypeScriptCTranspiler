...
class Worker {
    async run(flag: boolean): Promise<string> {
        if (flag) {
            try {
                throw (await delayedRejectAfter(1, "method-rejection") + await delay(1, "unused-two") + await delay(1, "unused-three") + await delay(1, "unused-four") + await delay(1, "unused-five"));
            } catch (reason) {
                return "caught:" + reason;
            }
        }
