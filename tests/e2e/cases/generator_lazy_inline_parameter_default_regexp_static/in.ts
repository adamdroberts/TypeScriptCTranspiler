function* values(): Generator<number, number, number> {
    return (yield 1) + ((raw: string = "a+b", escaped: string = RegExp.escape(raw), valid: boolean = escaped === "\\x61\\+b") => valid ? 1 : 0)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((raw: string = "a+b", escaped: string = RegExp.escape(raw), valid: boolean = escaped === "\\x61\\+b") => (valid ? 1 : 0) + offset)() + (yield 2);
    };
}
