function* values(): Generator<number, number, number> {
    return (yield 1) + ((value: number = -7, magnitude: number = Math.abs(value), limited: number = Math.min(magnitude, 8)) => magnitude + limited)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((value: number = -7, magnitude: number = Math.abs(value), limited: number = Math.min(magnitude, 8)) => magnitude + limited + offset)() + (yield 2);
    };
}
