...
async function runOf(): Promise<string> {
    for (const item of ["of-a", "of-b"]) {
        try {
            ...
            let first: string, second: string = await recoverSecond(reason);
            console.log(item + "-between");
            first = await recoverFirst(reason);
...
async function runFirstRecoveryRejected(): Promise<string> {
    for (const item of ["first"]) {
        try {
            ...
            let first: string, second: string = await recoverSecond(reason);
            console.log(item + "-between");
            first = await rejectFirst(reason);
