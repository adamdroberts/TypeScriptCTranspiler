function pushNestedOptionalInner(value: any): any {
    return "push-inner-" + value;
}

function pushNestedOptionalOuter(value: any): any {
    return "push-outer-" + value;
}

function unshiftNestedOptionalInner(value: any): any {
    return "unshift-inner-" + value;
}

function unshiftNestedOptionalOuter(value: any): any {
    return "unshift-outer-" + value;
}

function* multiYieldPushNestedOptionalCallArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedOptionalOuter?.(pushNestedOptionalInner?.(yield "push-nested")),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftNestedOptionalCallArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedOptionalOuter?.(unshiftNestedOptionalInner?.(yield "unshift-nested")),
            yield "unshift-item",
...
const pushIterator = multiYieldPushNestedOptionalCallArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftNestedOptionalCallArgumentAccessDeclarationInitializer();
