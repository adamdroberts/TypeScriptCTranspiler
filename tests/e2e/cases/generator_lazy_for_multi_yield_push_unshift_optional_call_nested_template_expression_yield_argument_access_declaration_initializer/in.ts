function pushNestedTemplateExpressionCallback(value: any): any {
    return "push-template-expression-" + value;
}

function unshiftNestedTemplateExpressionCallback(value: any): any {
    return "unshift-template-expression-" + value;
}

function* multiYieldPushNestedTemplateExpressionArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedTemplateExpressionCallback?.(
                `push-${`inner-${yield "push-inner"}-${yield "push-inner-tail"}`}-${yield "push-outer"}`
            ),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
...
function* multiYieldUnshiftNestedTemplateExpressionArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedTemplateExpressionCallback?.(
                String.raw`unshift-${`inner-${yield "unshift-inner"}-${yield "unshift-inner-tail"}`}-${yield "unshift-outer"}`
            ),
            yield "unshift-item",
...
const pushIterator = multiYieldPushNestedTemplateExpressionArgumentAccessDeclarationInitializer();
...
const pushThird: any = pushIterator.next("push-inner-value");
console.log("push-third", pushReceiver.join(","), pushThird.done, pushThird.value);
const pushFourth: any = pushIterator.next("push-inner-tail-value");
...
const unshiftIterator = multiYieldUnshiftNestedTemplateExpressionArgumentAccessDeclarationInitializer();
...
const unshiftThird: any = unshiftIterator.next("unshift-inner-value");
console.log("unshift-third", unshiftReceiver.join(","), unshiftThird.done, unshiftThird.value);
const unshiftFourth: any = unshiftIterator.next("unshift-inner-tail-value");
