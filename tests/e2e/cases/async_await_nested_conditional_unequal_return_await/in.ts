...
async function declaration(outer: boolean, inner: boolean): Promise<string> {
    ...
    return await delay(5, outer ? (inner ? "declaration-two" : "declaration-one") : "declaration-fallback");
}
...
class Worker {
    async run(outer: boolean, inner: boolean): Promise<string> {
        ...
        return await delay(10, outer ? (inner ? "method-two" : "method-one") : "method-fallback");
    }
}
...
const value = async (outer: boolean, inner: boolean): Promise<string> => {
    ...
    return await delay(15, outer ? (inner ? "value-two" : "value-one") : "value-fallback");
};
