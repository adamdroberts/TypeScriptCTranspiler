function* values(): Generator<number, number, number> {
    return (yield 1) + ((extra: number[] = [8], base: { bonus: number } = { bonus: 2 }, items: number[] = [7, ...extra], config: { bonus: number } = { ...base, bonus: base.bonus + 1 }) => items[0] + items[1] + config.bonus)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((extra: number[] = [8], base: { bonus: number } = { bonus: 2 }, items: number[] = [7, ...extra], config: { bonus: number } = { ...base, bonus: base.bonus + 1 }) => items[0] + items[1] + config.bonus + offset)() + (yield 2);
    };
}
