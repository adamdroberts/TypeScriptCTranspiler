function pushNestedLiteralInner(value: any): any {
    return "push-inner-" + value;
}

function pushNestedLiteralCallback(value: any): any {
    return "push-literal-" + value.join("|");
}

function unshiftNestedLiteralInner(value: any): any {
    return "unshift-inner-" + value;
}

function unshiftNestedLiteralCallback(value: any): any {
    return "unshift-literal-" + value.first + "|" + value.second;
}

function* multiYieldPushNestedLiteralCallArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedLiteralCallback?.([
                pushNestedLiteralInner(yield "push-literal-one"),
                pushNestedLiteralInner(yield "push-literal-two")
            ]),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
...
function* multiYieldUnshiftNestedLiteralCallArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedLiteralCallback?.({
                first: unshiftNestedLiteralInner?.(yield "unshift-literal-one"),
                second: unshiftNestedLiteralInner(yield "unshift-literal-two")
            }),
            yield "unshift-item",
...
const pushIterator = multiYieldPushNestedLiteralCallArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftNestedLiteralCallArgumentAccessDeclarationInitializer();
