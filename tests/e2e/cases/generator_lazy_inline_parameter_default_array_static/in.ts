function* values(): Generator<number, number, number> {
    return (yield 1) + ((source: any = [1], array: boolean = Array.isArray(source), object: boolean = Array.isArray({})) => (array ? 1 : 0) + (object ? 2 : 0))() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((source: any = [1], array: boolean = Array.isArray(source), object: boolean = Array.isArray({})) => (array ? 1 : 0) + (object ? 2 : 0) + offset)() + (yield 2);
    };
}
