function* multiYieldUnshiftLiteralArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "receiver").unshift("literal", yield "direct-item", ...(yield "spread-items"), true)[yield "key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const iterator = multiYieldUnshiftLiteralArgumentAccessDeclarationInitializer();
