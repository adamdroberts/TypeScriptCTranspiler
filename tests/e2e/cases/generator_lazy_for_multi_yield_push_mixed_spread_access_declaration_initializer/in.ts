function* multiYieldPushMixedSpreadAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "receiver").push(yield "first-item", ...(yield "spread-items"), yield "last-item")[yield "key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const iterator = multiYieldPushMixedSpreadAccessDeclarationInitializer();
