function* multiYieldToSplicedInsertAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "receiver").toSpliced(0, yield "delete", yield "insert")[yield "key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const receiver: any = ["first", "third"];
const iterator = multiYieldToSplicedInsertAccessDeclarationInitializer();
