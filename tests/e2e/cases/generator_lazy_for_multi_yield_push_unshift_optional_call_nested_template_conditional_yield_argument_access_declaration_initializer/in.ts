function pushNestedTemplateConditionalCallback(value: any): any {
    return "push-conditional-" + value;
}

function unshiftNestedTemplateConditionalCallback(value: any): any {
    return "unshift-conditional-" + value;
}

function* multiYieldPushNestedTemplateConditionalArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedTemplateConditionalCallback?.(`push-${((yield "push-condition") ? "yes" : "no")}-${yield "push-tail"}`),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftNestedTemplateConditionalArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedTemplateConditionalCallback?.(String.raw`unshift-${((yield "unshift-condition") ? "yes" : "no")}-${yield "unshift-tail"}`),
            yield "unshift-item",
...
const pushIterator = multiYieldPushNestedTemplateConditionalArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftNestedTemplateConditionalArgumentAccessDeclarationInitializer();
