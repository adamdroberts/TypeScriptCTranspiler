...
async function declaration(kind: number): Promise<string> {
    ...
    else first = await delay(1, "eight");
...
class Worker {
    async run(kind: number): Promise<string> {
        ...
        else first = await delay(1, "method-eight");
        const second = await delay(1, first + "-second");
        return second;
    }
}
...
const value = async function(kind: number): Promise<string> {
    ...
    else first = await delay(1, "value-eight");
    const second = await delay(1, first + "-second");
    return second;
};
...
async function branch(outer: boolean, kind: number): Promise<string> {
    if (outer) {
        ...
        else first = await delay(1, "branch-eight");
        const second = await delay(1, first + "-second");
        return second;
    }
    return "fallthrough";
...
declaration(8).then((result: string): void => console.log("declaration-eight:", result));
new Worker().run(7).then((result: string): void => console.log("method-seven:", result));
value(6).then((result: string): void => console.log("value-six:", result));
branch(true, 8).then((result: string): void => console.log("branch-eight:", result));
