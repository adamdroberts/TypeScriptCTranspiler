function pushNestedTaggedTemplate(strings: TemplateStringsArray, first: any, second: any): any {
    return strings[0] + first + strings[1] + second + strings[2];
}

function pushNestedTaggedInner(value: any): any {
    return "push-inner-" + value;
}

function unshiftNestedTaggedInner(value: any): any {
...
function* multiYieldPushNestedTaggedTemplateCallArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedTaggedCallback?.(pushNestedTaggedTemplate`push-${pushNestedTaggedInner(yield "push-tag-one")}-${pushNestedTaggedInner?.(yield "push-tag-two")}`),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftNestedTaggedTemplateCallArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedTaggedCallback?.(String.raw`unshift-${unshiftNestedTaggedInner(yield "unshift-tag-one")}-${unshiftNestedTaggedInner?.(yield "unshift-tag-two")}`),
            yield "unshift-item",
...
const pushIterator = multiYieldPushNestedTaggedTemplateCallArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftNestedTaggedTemplateCallArgumentAccessDeclarationInitializer();
