function pushCallbackImpl(first: any, second: any): any {
    return "push-callback-" + first + "-" + second;
}

let pushCallback: any = pushCallbackImpl;
let unshiftCallback: any = null;

function* multiYieldPushOptionalCallArgumentsAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushCallback?.("push-optional", 7),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftOptionalCallArgumentsAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftCallback?.("unshift-optional", 8),
            yield "unshift-item",
...
const pushIterator = multiYieldPushOptionalCallArgumentsAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftOptionalCallArgumentsAccessDeclarationInitializer();
