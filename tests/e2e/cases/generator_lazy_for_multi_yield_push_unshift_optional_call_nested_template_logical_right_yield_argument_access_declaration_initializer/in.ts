function pushNestedTemplateLogicalRightCallback(value: any): any {
    return "push-right-" + value;
}

function unshiftNestedTemplateLogicalRightCallback(value: any): any {
    return "unshift-right-" + value;
}

function* multiYieldPushNestedTemplateLogicalRightArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedTemplateLogicalRightCallback?.(`push-${((yield "push-and-left") && (yield "push-and-right"))}-${((yield "push-or-left") || (yield "push-or-right"))}-${yield "push-tail"}`),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftNestedTemplateLogicalRightArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedTemplateLogicalRightCallback?.(String.raw`unshift-${((yield "unshift-nullish-left") ?? (yield "unshift-nullish-right"))}-${yield "unshift-tail"}`),
            yield "unshift-item",
...
const pushIterator = multiYieldPushNestedTemplateLogicalRightArgumentAccessDeclarationInitializer();
...
const pushFourth: any = pushIterator.next(false);
console.log("push-fourth", pushReceiver.join(","), pushFourth.done, pushFourth.value);
const pushFifth: any = pushIterator.next("push-or-right-value");
console.log("push-fifth", pushReceiver.join(","), pushFifth.done, pushFifth.value);
const pushSixth: any = pushIterator.next("push-tail-value");
...
const unshiftIterator = multiYieldUnshiftNestedTemplateLogicalRightArgumentAccessDeclarationInitializer();
...
const unshiftFourth: any = unshiftIterator.next("unshift-nullish-right-value");
