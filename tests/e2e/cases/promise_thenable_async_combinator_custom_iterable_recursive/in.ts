...
class ThenableBag {
    ...
    [Symbol.iterator](): IterableIterator<any> {
        return this.items as unknown as IterableIterator<any>;
    }
}
...
Promise.all(new ThenableBag([nestedFulfilled("all-a", "A"), nestedFulfilled("all-b", "B")]))
    .then((items: any[]): Promise<any> => {
        console.log("all custom:", items.join(","), events.join("|"));
        events = [];
    ...
    .then((items: any[]): any[] => {
        ...
        console.log("settled custom:", summary, events.join("|"));
        return items;
    });
