...
function* multiYieldPushOptionalCallNestedTemplateYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedTemplateCallback?.(`push-${yield "push-template-one"}-${yield "push-template-two"}`),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
        count = 0;
...
function* multiYieldUnshiftOptionalCallNestedTemplateYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedTemplateCallback?.(`unshift-${yield "unshift-template-one"}-${yield "unshift-template-two"}`),
            yield "unshift-item",
...
const pushIterator = multiYieldPushOptionalCallNestedTemplateYieldArgumentAccessDeclarationInitializer();
...
const unshiftIterator = multiYieldUnshiftOptionalCallNestedTemplateYieldArgumentAccessDeclarationInitializer();
