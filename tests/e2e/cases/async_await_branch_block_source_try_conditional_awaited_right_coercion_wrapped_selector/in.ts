...
async function declaration(flag: number): Promise<string> {
    try {
        return String(ready(flag) === await left(flag))
...
class CoercionRunner {
    async method(flag: number): Promise<string> {
        try {
            return Number(ready(flag) === await left(flag))
...
const runner = new CoercionRunner();

const arrow = async (flag: number): Promise<string> => {
    try {
        return Boolean(ready(flag) === await left(flag))
...
declaration(1)
    ...
    .then((value) => {
        console.log("declaration-false-string", value, trace);
        trace = "";
    ...
    .then((value) => {
        console.log("method-false-number", value, trace);
        trace = "";
    ...
    .then((value) => {
        console.log("arrow-false-boolean", value, trace);
        trace = "";
        return arrow(6);
    })
    .then((value) => {
