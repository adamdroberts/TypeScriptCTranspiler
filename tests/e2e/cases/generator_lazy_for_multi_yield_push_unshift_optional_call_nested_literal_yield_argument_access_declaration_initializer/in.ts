function pushNestedLiteralCallback(values: any[], record: any): any {
    return "push-literal-" + values[0] + "-" + record.value;
}

function unshiftNestedLiteralCallback(values: any[], record: any): any {
    return "unshift-literal-" + values[0] + "-" + record.value;
}

function* multiYieldPushOptionalCallNestedLiteralYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "push-receiver").push(
            pushNestedLiteralCallback?.(
                [(yield "push-array")],
                { value: yield "push-object" }
            ),
            yield "push-item",
            ...(yield "push-spread")
        )[yield "push-key"],
...
function* multiYieldUnshiftOptionalCallNestedLiteralYieldArgumentAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "unshift-receiver").unshift(
            unshiftNestedLiteralCallback?.(
                [(yield "unshift-array")],
                { value: yield "unshift-object" }
            ),
            yield "unshift-item",
...
const pushIterator = multiYieldPushOptionalCallNestedLiteralYieldArgumentAccessDeclarationInitializer();
...
const pushThird: any = pushIterator.next("push-array-value");
console.log("push-third", pushReceiver.join(","), pushThird.done, pushThird.value);
const pushFourth: any = pushIterator.next("push-object-value");
...
const unshiftIterator = multiYieldUnshiftOptionalCallNestedLiteralYieldArgumentAccessDeclarationInitializer();
...
const unshiftThird: any = unshiftIterator.next("unshift-array-value");
console.log("unshift-third", unshiftReceiver.join(","), unshiftThird.done, unshiftThird.value);
const unshiftFourth: any = unshiftIterator.next("unshift-object-value");
