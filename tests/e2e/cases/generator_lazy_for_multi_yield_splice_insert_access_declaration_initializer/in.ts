function* multiYieldSpliceInsertAccessDeclarationInitializer(): Generator<string, string, any> {
    for (
        let value: any = (yield "receiver").splice(0, yield "delete", yield "insert")[yield "key"],
        count = 0;
        count < 1;
        count++
    ) {
...
const iterator = multiYieldSpliceInsertAccessDeclarationInitializer();
...
const fourth: any = iterator.next("inserted");
