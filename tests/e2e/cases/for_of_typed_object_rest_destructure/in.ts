...
for (const {
    name,
    profile: { label, ...profileRest },
    score = 10,
    ...outerRest
} of directRows) {
    direct += name + ":" + label + ":" + profileRest.score + ":" + profileRest.note + ":" + score + ":" + outerRest.extra + ":" + String((outerRest as any).profile) + ";";
}
...
for (const {
    ...
    custom += name + ":" + profileRest.label + ":" + profileRest.score + ":" + profileRest.note + ":" + score + ":" + outerRest.extra + ";";
}
...
function* lazyRows(): Generator<string, string, number> {
    ...
    for (const {
        ...
        yield name + ":" + label + ":" + profileRest.score + ":" + profileRest.note + ":" + score + ":" + outerRest.extra;
    }
    return "typed-object-rest-done";
}
