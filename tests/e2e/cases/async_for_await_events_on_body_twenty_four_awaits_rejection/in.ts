...
async function consume(iterator: any): Promise<string> {
    for await (const item of iterator) {
        ...
        await Promise.reject("twenty-four-rejection");
        break;
    }
...
consume(iterator).then((value: string): void => {
    console.log("body-twenty-four-awaits-rejected: unexpected", value);
}, (reason: any): void => {
    console.log("body-twenty-four-awaits-rejected:", reason);
});
