function* values(): Generator<number, number, number> {
    return (yield 1) + ((value: number = 7, config: { value: number } = { value }) => value + config.value)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((value: number = 7, config: { value: number } = { value }) => value + config.value + offset)() + (yield 2);
    };
}
