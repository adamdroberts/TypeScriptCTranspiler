function* values(): Generator<number, number, number> {
    return (yield 1) + ((raw: string = "42.5", integer: number = parseInt(raw, 10), decimal: number = parseFloat(raw), finite: boolean = isFinite(decimal), nan: boolean = isNaN(decimal)) => integer === 42 && decimal === 42.5 && finite && !nan ? 1 : 0)() + (yield 2);
}
...
function makeCapturedValues(): () => Generator<number, number, number> {
    ...
    return function* (): Generator<number, number, number> {
        return (yield 1) + ((raw: string = "42.5", integer: number = parseInt(raw, 10), decimal: number = parseFloat(raw), finite: boolean = isFinite(decimal), nan: boolean = isNaN(decimal)) => (integer === 42 && decimal === 42.5 && finite && !nan ? 1 : 0) + offset)() + (yield 2);
    };
}
