...
static void tsc_child_process_abort(void* env) {
    tsc_child_process_async_t* child = (tsc_child_process_async_t*)env;
    if (!child || child->closed || child->exited) return;
    child->abort_requested = true;
    if (child->pid > 0 && !child->killed && kill(child->pid, child->kill_signal) == 0) {
        child->killed = true;
        tsc_value_set_prop(child->event.value, tsc_str_from_lit("killed", 6), tsc_value_bool(true));
    }
}
...
static void tsc_child_process_poll(void* env) {
    ...
    if (child->abort_requested && !child->error_emitted) {
        child->error_emitted = true;
        tsc_child_emit_one_value(child->event.emitter, "error", tsc_child_process_abort_reason(child));
    }
...
tsc_value_t tsc_child_process_spawn(const tsc_str_t* file, const tsc_array_t* args, const tsc_str_t* cwd, const tsc_array_t* env, const tsc_str_t* shell, const tsc_str_t* argv0, bool pipe_stdin, bool ignore_stdin, bool pipe_stdout, bool ignore_stdout, bool inherit_stdout, bool pipe_stderr, bool ignore_stderr, bool inherit_stderr, bool detached, double uid, double gid, double timeout_ms, int kill_signal, tsc_value_t abort_signal) {
    ...
    if (!tsc_value_is_nullish(abort_signal)) {
        tsc_abort_signal_add_callback(abort_signal, tsc_child_process_abort, child);
    }
...
static bool tsc_net_resolve_endpoint(const tsc_str_t* host, int family, struct sockaddr_storage* out, socklen_t* out_len) {
...
static tsc_value_t tsc_net_server_listen(void* env, tsc_value_t this_arg, tsc_array_t* args) {
    ...
    if (!tsc_net_resolve_endpoint(host, AF_UNSPEC, &endpoint, &endpoint_len)) {
...
static tsc_value_t tsc_net_connect_internal(double port, tsc_str_t* host, int address_family, tsc_str_t* local_address, double local_port, bool local_port_set, double timeout_ms, bool timeout_set, tsc_value_t abort_signal, const tsc_net_connect_socket_options_t* socket_options, tsc_value_t connect_listener, tsc_net_socket_t** out_socket) {
    ...
    struct sockaddr_storage endpoint;
    socklen_t endpoint_len = 0;
    if (!tsc_net_resolve_endpoint(host, address_family, &endpoint, &endpoint_len)) {
    ...
    if (local_bind) {
        if (local_address) {
            if (!tsc_net_resolve_endpoint(local_address, endpoint.ss_family, &local_endpoint, &local_endpoint_len)) {
...
tsc_value_t tsc_net_connect_options(tsc_value_t options, tsc_value_t connect_listener) {
    ...
    tsc_value_t family_value = tsc_value_get_prop(options, tsc_str_from_lit("family", 6));
    int address_family = AF_UNSPEC;
    if (!tsc_value_is_nullish(family_value)) {
        if (!tsc_value_number_is_finite(family_value) || !tsc_value_number_is_integer(family_value) ||
            (tsc_value_as_num(family_value) != 0.0 && tsc_value_as_num(family_value) != 4.0 && tsc_value_as_num(family_value) != 6.0)) {
            tsc_throw_str(tsc_str_from_cstr("net.connect options.family must be 0, 4, or 6"));
        }
        address_family = tsc_value_as_num(family_value) == 4.0 ? AF_INET :
            (tsc_value_as_num(family_value) == 6.0 ? AF_INET6 : AF_UNSPEC);
    }
...
typedef struct tsc_http_timeout_context tsc_http_timeout_context_t;
...
struct tsc_http_timeout_context {
...
static tsc_value_t tsc_http_message_set_timeout(void* env, tsc_value_t this_arg, tsc_array_t* args) {
    tsc_http_timeout_context_t* context = (tsc_http_timeout_context_t*)env;
    const char* api_name = context && context->api_name ? context->api_name : "http.Message.setTimeout";
    char message[192];
    if (!context || !args || args->len < 1 || !tsc_value_number_is_finite(TSC_ARR(tsc_value_t, args, 0)) ||
        tsc_value_as_num(TSC_ARR(tsc_value_t, args, 0)) < 0.0) {
        snprintf(message, sizeof(message), "%s timeout must be a non-negative finite number", api_name);
        tsc_throw_str(tsc_str_from_cstr(message));
    }
    if (args->len > 1 && !tsc_value_is_nullish(TSC_ARR(tsc_value_t, args, 1)) &&
        !tsc_value_is_callable(TSC_ARR(tsc_value_t, args, 1))) {
        snprintf(message, sizeof(message), "%s callback must be callable", api_name);
        tsc_throw_str(tsc_str_from_cstr(message));
    }
    if (args->len > 1 && !tsc_value_is_nullish(TSC_ARR(tsc_value_t, args, 1))) {
...
static tsc_value_t tsc_http_response_end(void* env, tsc_value_t this_arg, tsc_array_t* args) {
    ...
    if (tsc_http_header_value_contains(response->headers, "transfer-encoding", "chunked")) {
        ...
        response->ended = true;
        tsc_http_timeout_context_dispose(response->timeout_context);
...
static tsc_value_t tsc_http_make_request_response(tsc_http_connection_state_t* connection, const char* method, const char* target, const char* version, tsc_value_t headers, tsc_value_t trailers, const char* body, size_t body_len, size_t request_consumed, bool keep_alive) {
    ...
    response->timeout_context = tsc_http_timeout_context_new(&response->event, response->socket, "httpServerTimeout", "http.ServerResponse.setTimeout");
    ...
    tsc_object_set(response_object, tsc_str_from_lit("setTimeout", 10), tsc_value_function_generic_named(tsc_http_message_set_timeout, response->timeout_context, 1.0, tsc_str_from_lit("setTimeout", 10)));
...
static tsc_value_t tsc_http_request_internal(tsc_value_t options, tsc_value_t response_listener, bool force_get, bool tls) {
    ...
    client->socket_object = (value_is_box(client->socket) && value_tag(client->socket) == TSC_VALUE_TAG_OBJECT)
        ...
        : NULL;
    tsc_value_t socket = tsc_http_client_socket_value(client);
    client->timeout_context = tsc_http_timeout_context_new(&client->event, socket, "httpClientTimeout", "http.ClientRequest.setTimeout");
